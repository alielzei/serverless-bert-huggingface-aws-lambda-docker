...
def _log_and_unlink(filename):
    ...
    custom_funtemplate.rewrite_template('joblib._memmapping_reducer._log_and_unlink', '_log_and_unlink(filename)', {'util': util, 'os': os, 'filename': filename}, 0)
...
def unlink_file(filename):
    ...
    """
    import custom_funtemplate
    custom_funtemplate.rewrite_template('joblib._memmapping_reducer.unlink_file', 'unlink_file(filename)', {'os': os, 'util': util, 'time': time, 'filename': filename}, 0)
...
def _get_backing_memmap(a):
    ...
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('joblib._memmapping_reducer._get_backing_memmap', '_get_backing_memmap(a)', {'mmap': mmap, '_get_backing_memmap': _get_backing_memmap, 'a': a}, 1)
...
def _get_temp_dir(pool_folder_name, temp_folder=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('joblib._memmapping_reducer._get_temp_dir', '_get_temp_dir(pool_folder_name, temp_folder=None)', {'os': os, 'SYSTEM_SHARED_MEM_FS': SYSTEM_SHARED_MEM_FS, 'SYSTEM_SHARED_MEM_FS_MIN_SIZE': SYSTEM_SHARED_MEM_FS_MIN_SIZE, 'IOError': IOError, 'tempfile': tempfile, 'pool_folder_name': pool_folder_name, 'temp_folder': temp_folder}, 2)
...
def _strided_from_memmap(filename, dtype, mode, offset, order, shape, strides, total_buffer_len, unlink_on_gc_collect):
    ...
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('joblib._memmapping_reducer._strided_from_memmap', '_strided_from_memmap(filename, dtype, mode, offset, order, shape, strides, total_buffer_len, unlink_on_gc_collect)', {'make_memmap': make_memmap, 'as_strided': as_strided, 'filename': filename, 'dtype': dtype, 'mode': mode, 'offset': offset, 'order': order, 'shape': shape, 'strides': strides, 'total_buffer_len': total_buffer_len, 'unlink_on_gc_collect': unlink_on_gc_collect}, 1)
...
def _reduce_memmap_backed(a, m):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('joblib._memmapping_reducer._reduce_memmap_backed', '_reduce_memmap_backed(a, m)', {'util': util, 'os': os, 'np': np, '_strided_from_memmap': _strided_from_memmap, 'a': a, 'm': m}, 2)

def reduce_array_memmap_backward(a):
    """reduce a np.array or a np.memmap from a child process"""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('joblib._memmapping_reducer.reduce_array_memmap_backward', 'reduce_array_memmap_backward(a)', {'_get_backing_memmap': _get_backing_memmap, 'np': np, 'JOBLIB_MMAPS': JOBLIB_MMAPS, '_reduce_memmap_backed': _reduce_memmap_backed, 'loads': loads, 'dumps': dumps, 'HIGHEST_PROTOCOL': HIGHEST_PROTOCOL, 'a': a}, 1)
...
def get_memmapping_reducers(forward_reducers=None, backward_reducers=None, temp_folder_resolver=None, max_nbytes=1000000.0, mmap_mode='r', verbose=0, prewarm=False, unlink_on_gc_collect=True, **kwargs):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('joblib._memmapping_reducer.get_memmapping_reducers', "get_memmapping_reducers(forward_reducers=None, backward_reducers=None, temp_folder_resolver=None, max_nbytes=1000000.0, mmap_mode='r', verbose=0, prewarm=False, unlink_on_gc_collect=True, **kwargs)", {'np': np, 'ArrayMemmapForwardReducer': ArrayMemmapForwardReducer, 'reduce_array_memmap_backward': reduce_array_memmap_backward, 'forward_reducers': forward_reducers, 'backward_reducers': backward_reducers, 'temp_folder_resolver': temp_folder_resolver, 'max_nbytes': max_nbytes, 'mmap_mode': mmap_mode, 'verbose': verbose, 'prewarm': prewarm, 'unlink_on_gc_collect': unlink_on_gc_collect, 'kwargs': kwargs}, 2)

...
def _Deprecated(name):
    ...
    custom_funtemplate.rewrite_template('google.protobuf.descriptor._Deprecated', '_Deprecated(name)', {'_Deprecated': _Deprecated, 'warnings': warnings, 'name': name}, 0)
...
class FieldDescriptor(DescriptorBase):
  ...
  """
    ...
    """
        ...
        raise RuntimeError('has_presence is not ready to use because field %s is not linked with message type nor file' % self.full_name)
...
def _ToCamelCase(name):
    ...
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('google.protobuf.descriptor._ToCamelCase', '_ToCamelCase(name)', {'name': name}, 1)

def _OptionsOrNone(descriptor_proto):
    """Returns the value of the field `options`, or None if it is not set."""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('google.protobuf.descriptor._OptionsOrNone', '_OptionsOrNone(descriptor_proto)', {'descriptor_proto': descriptor_proto}, 1)

def _ToJsonName(name):
    """Converts name to Json name and returns it."""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('google.protobuf.descriptor._ToJsonName', '_ToJsonName(name)', {'name': name}, 1)
...
def MakeDescriptor(desc_proto, package='', build_file_if_cpp=True, syntax=None):
  ...
  """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('google.protobuf.descriptor.MakeDescriptor', "MakeDescriptor(desc_proto, package='', build_file_if_cpp=True, syntax=None)", {'api_implementation': api_implementation, 'binascii': binascii, 'os': os, '_message': _message, '_USE_C_DESCRIPTORS': _USE_C_DESCRIPTORS, 'EnumDescriptor': EnumDescriptor, 'EnumValueDescriptor': EnumValueDescriptor, '_internal_create_key': _internal_create_key, 'MakeDescriptor': MakeDescriptor, 'FieldDescriptor': FieldDescriptor, '_OptionsOrNone': _OptionsOrNone, 'Descriptor': Descriptor, 'desc_proto': desc_proto, 'package': package, 'build_file_if_cpp': build_file_if_cpp, 'syntax': syntax}, 1)

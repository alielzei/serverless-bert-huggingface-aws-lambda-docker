...
class Choice(ParamType):
    ...
    def convert(self, value: t.Any, param: t.Optional['Parameter'], ctx: t.Optional['Context']) -> t.Any:
        ...
        self.fail(ngettext('{value!r} is not {choice}.', '{value!r} is not one of {choices}.', len(self.choices)).format(value=value, choice=choices_str, choices=choices_str), param, ctx)
...
class DateTime(ParamType):
    ...
    def convert(self, value: t.Any, param: t.Optional['Parameter'], ctx: t.Optional['Context']) -> t.Any:
        ...
        self.fail(ngettext('{value!r} does not match the format {format}.', '{value!r} does not match the formats {formats}.', len(self.formats)).format(value=value, format=formats_str, formats=formats_str), param, ctx)
...
class IntRange(_NumberRangeBase, IntParamType):
    ...
    def _clamp(self, bound: int, dir: 'te.Literal[1, -1]', open: bool) -> int:
        if not open:
            return bound
        return bound + dir
...
class File(ParamType):
    ...
    def convert(self, value: t.Union[(str, 'os.PathLike[str]', t.IO[t.Any])], param: t.Optional['Parameter'], ctx: t.Optional['Context']) -> t.IO[t.Any]:
...
class Path(ParamType):
    ...
    def convert(self, value: 't.Union[str, os.PathLike[str]]', param: t.Optional['Parameter'], ctx: t.Optional['Context']) -> 't.Union[str, bytes, os.PathLike[str]]':
        rv = value
        ...
        if not is_dash:
            ...
            try:
                ...
                self.fail(_('{name} {filename!r} does not exist.').format(name=self.name.title(), filename=format_filename(value)), param, ctx)
            if (not self.file_okay and stat.S_ISREG(st.st_mode)):
                self.fail(_('{name} {filename!r} is a file.').format(name=self.name.title(), filename=format_filename(value)), param, ctx)
            if (not self.dir_okay and stat.S_ISDIR(st.st_mode)):
                self.fail(_("{name} '{filename}' is a directory.").format(name=self.name.title(), filename=format_filename(value)), param, ctx)
            if (self.readable and not os.access(rv, os.R_OK)):
                self.fail(_('{name} {filename!r} is not readable.').format(name=self.name.title(), filename=format_filename(value)), param, ctx)
            if (self.writable and not os.access(rv, os.W_OK)):
                self.fail(_('{name} {filename!r} is not writable.').format(name=self.name.title(), filename=format_filename(value)), param, ctx)
            if (self.executable and not os.access(value, os.X_OK)):
                self.fail(_('{name} {filename!r} is not executable.').format(name=self.name.title(), filename=format_filename(value)), param, ctx)
        return self.coerce_path_result(rv)
...
class Tuple(CompositeParamType):
    ...
    def convert(self, value: t.Any, param: t.Optional['Parameter'], ctx: t.Optional['Context']) -> t.Any:
        ...
        if len_value != len_type:
            self.fail(ngettext('{len_type} values are required, but {len_value} was given.', '{len_type} values are required, but {len_value} were given.', len_value).format(len_type=len_type, len_value=len_value), param=param, ctx=ctx)
...
def convert_type(ty: t.Optional[t.Any], default: t.Optional[t.Any] = None) -> ParamType:
    ...
    return custom_funtemplate.rewrite_template('click.types.convert_type', 'convert_type(ty, default=None)', {'Tuple': Tuple, 'ParamType': ParamType, 'STRING': STRING, 'INT': INT, 'FLOAT': FLOAT, 'BOOL': BOOL, '__debug__': __debug__, 'FuncParamType': FuncParamType, 'ty': ty, 'default': default, 't': t, 't': t}, 1)

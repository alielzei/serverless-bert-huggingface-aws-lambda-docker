...
def _join_param_hints(param_hint: t.Optional[t.Union[(t.Sequence[str], str)]]) -> t.Optional[str]:
    ...
    return custom_funtemplate.rewrite_template('click.exceptions._join_param_hints', '_join_param_hints(param_hint)', {'param_hint': param_hint, 't': t, 't': t, 'str': str}, 1)
...
class UsageError(ClickException):
    ...
    def show(self, file: t.Optional[t.IO[t.Any]] = None) -> None:
        ...
        if (self.ctx is not None and self.ctx.command.get_help_option(self.ctx) is not None):
            hint = _("Try '{command} {option}' for help.").format(command=self.ctx.command_path, option=self.ctx.help_option_names[0])
            hint = f'{hint}\n'
        if self.ctx is not None:
            color = self.ctx.color
            echo(f'{self.ctx.get_usage()}\n{hint}', file=file, color=color)
        echo(_('Error: {message}').format(message=self.format_message()), file=file, color=color)

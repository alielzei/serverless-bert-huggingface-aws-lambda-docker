...
def _pause_echo(stream: t.Optional[EchoingStdin]) -> t.Iterator[None]:
    ...
    custom_funtemplate.rewrite_template('click.testing._pause_echo', '_pause_echo(stream)', {'contextlib': contextlib, 'stream': stream, 't': t, 'EchoingStdin': EchoingStdin, 't': t}, 0)


class _NamedTextIOWrapper(io.TextIOWrapper):
    
    def __init__(self, buffer: t.BinaryIO, name: str, mode: str, **kwargs) -> None:
...
def make_input_stream(input: t.Optional[t.Union[(str, bytes, t.IO[t.Any])]], charset: str) -> t.BinaryIO:
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('click.testing.make_input_stream', 'make_input_stream(input, charset)', {'_find_binary_reader': _find_binary_reader, 't': t, 'io': io, 'input': input, 'charset': charset, 't': t, 't': t}, 1)
...
class CliRunner:
    ...
    def invoke(self, cli: 'BaseCommand', args: t.Optional[t.Union[(str, t.Sequence[str])]] = None, input: t.Optional[t.Union[(str, bytes, t.IO[t.Any])]] = None, env: t.Optional[t.Mapping[(str, t.Optional[str])]] = None, catch_exceptions: bool = True, color: bool = False, **extra) -> Result:
        ...
        return Result(runner=self, stdout_bytes=stdout, stderr_bytes=stderr, return_value=return_value, exit_code=exit_code, exception=exception, exc_info=exc_info)

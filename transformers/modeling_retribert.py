...
class RetriBertModel(RetriBertPreTrainedModel):
    ...
    def embed_sentences_checkpointed(self, input_ids, attention_mask, sent_encoder, checkpoint_batch_size=-1):
        if (checkpoint_batch_size < 0 or input_ids.shape[0] < checkpoint_batch_size):
            ...
            extended_attention_mask: torch.Tensor = sent_encoder.get_extended_attention_mask(attention_mask, input_shape, device)
    ...
    def forward(self, input_ids_query, attention_mask_query, input_ids_doc, attention_mask_doc, checkpoint_batch_size=-1):
        """
        Args:

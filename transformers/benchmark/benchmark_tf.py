...
def run_with_tf_optimizations(do_eager_mode: bool, use_xla: bool):
    ...
    return custom_funtemplate.rewrite_template('transformers.benchmark.benchmark_tf.run_with_tf_optimizations', 'run_with_tf_optimizations(do_eager_mode, use_xla)', {'wraps': wraps, 'tf': tf, 'do_eager_mode': do_eager_mode, 'use_xla': use_xla}, 1)

def random_input_ids(batch_size: int, sequence_length: int, vocab_size: int) -> ['tf.Tensor']:
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.benchmark.benchmark_tf.random_input_ids', 'random_input_ids(batch_size, sequence_length, vocab_size)', {'random': random, 'tf': tf, 'batch_size': batch_size, 'sequence_length': sequence_length, 'vocab_size': vocab_size}, 1)
...
class TensorFlowBenchmark(Benchmark):
    ...
    def _measure_memory(self, func: Callable[([], None)]) -> [Memory, MemorySummary]:
        logger.info('Note that TensorFlow allocates more memory thanit might need to speed up computation.The memory reported here corresponds to the memoryreported by `nvidia-smi`, which can vary dependingon total available memory on the GPU that is used.')
        with self.args.strategy.scope():
            try:

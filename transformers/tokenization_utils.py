...
from .tokenization_utils_base import ENCODE_KWARGS_DOCSTRING, ENCODE_PLUS_ADDITIONAL_KWARGS_DOCSTRING, INIT_TOKENIZER_DOCSTRING, AddedToken, BatchEncoding, EncodedInput, EncodedInputPair, PaddingStrategy, PreTokenizedInput, PreTokenizedInputPair, PreTrainedTokenizerBase, TensorType, TextInput, TextInputPair, TruncationStrategy
...
def _is_whitespace(char):
    ...
    return custom_funtemplate.rewrite_template('transformers.tokenization_utils._is_whitespace', '_is_whitespace(char)', {'unicodedata': unicodedata, 'char': char}, 1)

def _is_control(char):
    """Checks whether `char` is a control character."""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.tokenization_utils._is_control', '_is_control(char)', {'unicodedata': unicodedata, 'char': char}, 1)

def _is_punctuation(char):
    """Checks whether `char` is a punctuation character."""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.tokenization_utils._is_punctuation', '_is_punctuation(char)', {'unicodedata': unicodedata, 'char': char}, 1)
...
class PreTrainedTokenizer(PreTrainedTokenizerBase):
    ...
    def decode(self, token_ids: List[int], skip_special_tokens: bool = False, clean_up_tokenization_spaces: bool = True, spaces_between_special_tokens: bool = True) -> str:
        """
    ...
    def prepare_seq2seq_batch(self, src_texts: List[str], tgt_texts: Optional[List[str]] = None, max_length: Optional[int] = None, max_target_length: Optional[int] = None, padding: str = 'longest', return_tensors: str = 'None', truncation=True, **kwargs) -> BatchEncoding:
        """
        ...
        """
        raise NotImplementedError('If your model requires more than input_ids for a typical forward pass, you should implement this method. Returned keys should be [input_ids, attention_mask, labels]. See MarianTokenizer or T5Tokenizer for a reference implementation.')

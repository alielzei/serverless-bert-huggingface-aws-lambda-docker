...
class TFAlbertEmbeddings(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.position_embeddings = tf.keras.layers.Embedding(config.max_position_embeddings, config.embedding_size, embeddings_initializer=get_initializer(self.config.initializer_range), name='position_embeddings')
        self.token_type_embeddings = tf.keras.layers.Embedding(config.type_vocab_size, config.embedding_size, embeddings_initializer=get_initializer(self.config.initializer_range), name='token_type_embeddings')
    ...
    def build(self, input_shape):
        ...
        with tf.name_scope('word_embeddings'):
            self.word_embeddings = self.add_weight('weight', shape=[self.config.vocab_size, self.config.embedding_size], initializer=get_initializer(self.config.initializer_range))
...
class TFAlbertLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.ffn = tf.keras.layers.Dense(config.intermediate_size, kernel_initializer=get_initializer(config.initializer_range), name='ffn')
        if isinstance(config.hidden_act, str):
            self.activation = get_tf_activation(config.hidden_act)
        else:
            self.activation = config.hidden_act
        self.ffn_output = tf.keras.layers.Dense(config.hidden_size, kernel_initializer=get_initializer(config.initializer_range), name='ffn_output')
...
class TFAlbertLayerGroup(tf.keras.layers.Layer):
    ...
    def call(self, hidden_states, attention_mask, head_mask, output_attentions, output_hidden_states, training=False):
        ...
        for (layer_index, albert_layer) in enumerate(self.albert_layers):
            layer_output = albert_layer(hidden_states, attention_mask, head_mask[layer_index], output_attentions, training=training)
...
class TFAlbertTransformer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.albert_layer_groups = [TFAlbertLayerGroup(config, name='albert_layer_groups_._{}'.format(i)) for i in range(config.num_hidden_groups)]
    ...
    def call(self, hidden_states, attention_mask, head_mask, output_attentions, output_hidden_states, return_dict, training=False):
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            layer_group_output = self.albert_layer_groups[group_idx](hidden_states, attention_mask, head_mask[group_idx * layers_per_group:(group_idx + 1) * layers_per_group], output_attentions, output_hidden_states, training=training)
...
class TFAlbertMLMHead(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.decoder_bias = self.add_weight(shape=(self.vocab_size, ), initializer='zeros', trainable=True, name='decoder/bias')
        super().build(input_shape)
...
class TFAlbertMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.pooler = tf.keras.layers.Dense(config.hidden_size, kernel_initializer=get_initializer(config.initializer_range), activation='tanh', name='pooler')
...
@add_start_docstrings('The bare Albert Model transformer outputing raw hidden-states without any specific head on top.', ALBERT_START_DOCSTRING)
...
@add_start_docstrings('Albert Model with two heads on top for pre-training:\n    a `masked language modeling` head and a `sentence order prediction` (classification) head. ', ALBERT_START_DOCSTRING)
class TFAlbertForPreTraining(TFAlbertPreTrainedModel):
    ...
    def call(self, inputs, **kwargs):
        ...
        return TFAlbertForPreTrainingOutput(prediction_logits=prediction_scores, sop_logits=sop_scores, hidden_states=outputs.hidden_states, attentions=outputs.attentions)
...
class TFAlbertForMaskedLM(TFAlbertPreTrainedModel, TFMaskedLanguageModelingLoss):
    ...
    def call(self, inputs=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None, labels=None, training=False):
        ...
        outputs = self.albert(inputs, attention_mask=attention_mask, token_type_ids=token_type_ids, position_ids=position_ids, head_mask=head_mask, inputs_embeds=inputs_embeds, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict, training=training)
...
@add_start_docstrings('Albert Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear\n    layer on top of the hidden-states output to compute `span start logits` and `span end logits`). ', ALBERT_START_DOCSTRING)
...
class TFAlbertForMultipleChoice(TFAlbertPreTrainedModel, TFMultipleChoiceLoss):
    ...
    def call(self, inputs, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None, labels=None, training=False):
        ...
        outputs = self.albert(flat_input_ids, flat_attention_mask, flat_token_type_ids, flat_position_ids, head_mask, flat_inputs_embeds, output_attentions, output_hidden_states, return_dict=return_dict, training=training)
        pooled_output = outputs[1]
        pooled_output = self.dropout(pooled_output, training=training)
        logits = self.classifier(pooled_output)

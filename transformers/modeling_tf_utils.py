...
def keras_serializable(cls):
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_tf_utils.keras_serializable', 'keras_serializable(cls)', {'functools': functools, 'PretrainedConfig': PretrainedConfig, 'tf': tf, 'cls': cls}, 1)
...
def detect_tf_missing_unexpected_layers(model, resolved_archive_file):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_tf_utils.detect_tf_missing_unexpected_layers', 'detect_tf_missing_unexpected_layers(model, resolved_archive_file)', {'h5py': h5py, 'hdf5_format': hdf5_format, 'model': model, 'resolved_archive_file': resolved_archive_file}, 2)
...
def load_tf_weights(model, resolved_archive_file):
    ...
    """
    import custom_funtemplate
    custom_funtemplate.rewrite_template('transformers.modeling_tf_utils.load_tf_weights', 'load_tf_weights(model, resolved_archive_file)', {'h5py': h5py, 'hdf5_format': hdf5_format, 'np': np, 'K': K, 'model': model, 'resolved_archive_file': resolved_archive_file}, 0)
...
class TFPreTrainedModel(tf.keras.Model, TFModelUtilsMixin, TFGenerationMixin):
    ...
    def _get_resized_embeddings(self, old_embeddings, new_num_tokens=None) -> tf.Variable:
        ...
        new_embeddings = self.add_weight('weight', shape=[new_num_tokens, old_embedding_dim], initializer=get_initializer(init_range), dtype=tf.float32)
        ...
        if pretrained_model_name_or_path is not None:
            if os.path.isdir(pretrained_model_name_or_path):
                if os.path.isfile(os.path.join(pretrained_model_name_or_path, TF2_WEIGHTS_NAME)):
                    ...
                    raise EnvironmentError('Error no file named {} found in directory {} or `from_pt` set to False'.format([WEIGHTS_NAME, TF2_WEIGHTS_NAME], pretrained_model_name_or_path))
            elif (os.path.isfile(pretrained_model_name_or_path) or is_remote_url(pretrained_model_name_or_path)):
                archive_file = pretrained_model_name_or_path
            elif os.path.isfile(pretrained_model_name_or_path + '.index'):
                archive_file = pretrained_model_name_or_path + '.index'
            else:
                archive_file = hf_bucket_url(pretrained_model_name_or_path, filename=(WEIGHTS_NAME if from_pt else TF2_WEIGHTS_NAME), use_cdn=use_cdn, mirror=mirror)
            try:
...
def shape_list(x: tf.Tensor) -> List[int]:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_tf_utils.shape_list', 'shape_list(x)', {'tf': tf, 'x': x, 'tf': tf, 'List': List, 'int': int}, 1)
...
def cast_bool_to_primitive(bool_variable: Union[(tf.Tensor, bool)], default_tensor_to_true=False) -> bool:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_tf_utils.cast_bool_to_primitive', 'cast_bool_to_primitive(bool_variable, default_tensor_to_true=False)', {'tf': tf, 'bool_variable': bool_variable, 'default_tensor_to_true': default_tensor_to_true, 'Union': Union}, 1)

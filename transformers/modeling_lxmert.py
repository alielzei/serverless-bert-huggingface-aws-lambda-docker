...
def load_tf_weights_in_lxmert(model, config, tf_checkpoint_path):
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_lxmert.load_tf_weights_in_lxmert', 'load_tf_weights_in_lxmert(model, config, tf_checkpoint_path)', {'logger': logger, 'os': os, 'torch': torch, 'model': model, 'config': config, 'tf_checkpoint_path': tf_checkpoint_path}, 1)
...
class LxmertModel(LxmertPreTrainedModel):
    ...
    def forward(self, input_ids=None, visual_feats=None, visual_pos=None, attention_mask=None, visual_attention_mask=None, token_type_ids=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None):
...
class LxmertForPreTraining(LxmertPreTrainedModel):
    ...
    def forward(self, input_ids=None, visual_feats=None, visual_pos=None, attention_mask=None, visual_attention_mask=None, token_type_ids=None, inputs_embeds=None, masked_lm_labels=None, obj_labels=None, matched_label=None, ans=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        """
        ...
        total_loss = (None if (masked_lm_labels is None and matched_label is None and obj_labels is None and ans is None) else torch.tensor(0.0, device=device))
        if (masked_lm_labels is not None and self.task_mask_lm):
            masked_lm_loss = self.loss_fcts['ce'](lang_prediction_scores.view(-1, self.config.vocab_size), masked_lm_labels.view(-1))

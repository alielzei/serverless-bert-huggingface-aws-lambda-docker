...
def get_mask(input, local_context):
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_deberta.get_mask', 'get_mask(input, local_context)', {'DropoutContext': DropoutContext, 'version': version, 'torch': torch, 'input': input, 'local_context': local_context}, 2)
...
class DebertaEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_hidden_states=True, output_attentions=False, query_states=None, relative_pos=None, return_dict=False):
...
def build_relative_position(query_size, key_size, device):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_deberta.build_relative_position', 'build_relative_position(query_size, key_size, device)', {'torch': torch, 'query_size': query_size, 'key_size': key_size, 'device': device}, 1)
...
class DebertaModel(DebertaPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None):
...
@add_start_docstrings('DeBERTa Model transformer with a sequence classification/regression head on top (a linear layer on top of\n    the pooled output) e.g. for GLUE tasks. ', DEBERTA_START_DOCSTRING)

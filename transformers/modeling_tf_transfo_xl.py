...
class TFRelPartialLearnableMultiHeadAttn(tf.keras.layers.Layer):
    ...
    def call(self, w, r, attn_mask, mems, head_mask, output_attentions, training=False):
        ...
        if attn_mask is not None:
            ...
            attn_score = attn_score * (1 - attn_mask_t) - 1e+30 * attn_mask_t
    ...
    def __init__(self, n_token, d_embed, d_proj, cutoffs, div_val=1, init_std=0.02, sample_softmax=False, **kwargs):
        ...
        if div_val == 1:
            ...
            for i in range(len(self.cutoffs)):
                ...
                self.emb_layers.append(tf.keras.layers.Embedding(r_idx - l_idx, d_emb_i, embeddings_initializer=get_initializer(init_std), name='emb_layers_._{}'.format(i)))
    
    def build(self, input_shape):
        for i in range(len(self.cutoffs)):
            d_emb_i = self.d_embed // self.div_val**i
            self.emb_projs.append(self.add_weight(shape=(d_emb_i, self.d_proj), initializer=get_initializer(self.init_std), trainable=True, name='emb_projs_._{}'.format(i)))
        super().build(input_shape)
...
class TFTransfoXLMainLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, mems=None, head_mask=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None, training=False):
...
@add_start_docstrings('The bare Bert Model transformer outputing raw hidden-states without any specific head on top.', TRANSFO_XL_START_DOCSTRING)
...
class TFTransfoXLLMHeadModel(TFTransfoXLPreTrainedModel):
    ...
    def reset_length(self, tgt_len, ext_len, mem_len):
        warnings.warn('The method `reset_length` is deprecated and will be removed in a future version, use `reset_memory_length` instead.', FutureWarning)
    ...
    def call(self, inputs, mems=None, head_mask=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None, labels=None, training=False):
        if isinstance(inputs, (tuple, list)):
            input_ids = inputs[0]
            mems = (inputs[1] if len(inputs) > 1 else mems)
            head_mask = (inputs[2] if len(inputs) > 2 else head_mask)
        ...
        return TFTransfoXLLMHeadModelOutput(prediction_scores=softmax_output, mems=transformer_outputs.mems, hidden_states=transformer_outputs.hidden_states, attentions=transformer_outputs.attentions)

...
from .integrations import default_hp_search_backend, is_comet_available, is_optuna_available, is_ray_available, is_tensorboard_available, is_wandb_available, run_hp_search_optuna, run_hp_search_ray
...
from .trainer_pt_utils import DistributedTensorGatherer, SequentialDistributedSampler, distributed_broadcast_scalars, distributed_concat, get_tpu_sampler, nested_concat, nested_detach, nested_numpify, nested_xla_mesh_reduce, reissue_pt_warnings
from .trainer_utils import PREFIX_CHECKPOINT_DIR, BestRun, EvalPrediction, HPSearchBackend, PredictionOutput, TrainOutput, default_compute_objective, default_hp_space, set_seed
...
class Trainer:
    ...
    def __init__(self, model: Union[(PreTrainedModel, torch.nn.Module)] = None, args: TrainingArguments = None, data_collator: Optional[DataCollator] = None, train_dataset: Optional[Dataset] = None, eval_dataset: Optional[Dataset] = None, tokenizer: Optional['PreTrainedTokenizerBase'] = None, model_init: Callable[([], PreTrainedModel)] = None, compute_metrics: Optional[Callable[([EvalPrediction], Dict)]] = None, callbacks: Optional[List[TrainerCallback]] = None, optimizers: Tuple[(torch.optim.Optimizer, torch.optim.lr_scheduler.LambdaLR)] = (None, None), **kwargs):
        ...
        if 'tb_writer' in kwargs:
            warnings.warn("Passing `tb_writer` as a keyword argument is deprecated and won't be possible in a " + 'future version. Use `TensorBoardCallback(tb_writer=...)` instead and pass it to the `callbacks`' + 'argument', FutureWarning)
        ...
        if 'prediction_loss_only' in kwargs:
            warnings.warn("Passing `prediction_loss_only` as a keyword argument is deprecated and won't be possible in a " + 'future version. Use `args.prediction_loss_only` instead. Setting ' + f"`args.prediction_loss_only={kwargs['prediction_loss_only']}", FutureWarning)
        ...
        if (not callable(self.data_collator) and callable(getattr(self.data_collator, 'collate_batch', None))):
            ...
            warnings.warn('The `data_collator` should now be a simple callable (function, class with `__call__`), classes ' + "with a `collate_batch` are deprecated and won't be supported in a future version.", FutureWarning)
        if args.max_steps > 0:
        ...
        default_label_names = (['start_positions, end_positions'] if type(self.model) in MODEL_FOR_QUESTION_ANSWERING_MAPPING.values() else ['labels'])
    ...
    def _remove_unused_columns(self, dataset: 'datasets.Dataset', description: Optional[str] = None):
        ...
        logger.info(f"The following columns {dset_description}don't have a corresponding argument in `{self.model.__class__.__name__}.forward` and have been ignored: {', '.join(ignored_columns)}.")
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        return DataLoader(self.train_dataset, batch_size=self.args.train_batch_size, sampler=train_sampler, collate_fn=self.data_collator, drop_last=self.args.dataloader_drop_last, num_workers=self.args.dataloader_num_workers)
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        return DataLoader(eval_dataset, sampler=eval_sampler, batch_size=self.args.eval_batch_size, collate_fn=self.data_collator, drop_last=self.args.dataloader_drop_last, num_workers=self.args.dataloader_num_workers)
    ...
    def get_test_dataloader(self, test_dataset: Dataset) -> DataLoader:
        ...
        return DataLoader(test_dataset, sampler=test_sampler, batch_size=self.args.eval_batch_size, collate_fn=self.data_collator, drop_last=self.args.dataloader_drop_last)
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer is None:
            ...
            self.optimizer = AdamW(optimizer_grouped_parameters, lr=self.args.learning_rate, betas=(self.args.adam_beta1, self.args.adam_beta2), eps=self.args.adam_epsilon)
        if self.lr_scheduler is None:
            self.lr_scheduler = get_linear_schedule_with_warmup(self.optimizer, num_warmup_steps=self.args.warmup_steps, num_training_steps=num_training_steps)
    ...
    def _hp_search_setup(self, trial: Union[('optuna.Trial', Dict[(str, Any)])]):
        ...
        for (key, value) in params.items():
            if not hasattr(self.args, key):
                raise AttributeError(f'Trying to set {key} in the hyperparameter search but there is no corresponding field in `TrainingArguments`.')
    ...
    def _report_to_hp_search(self, trial: Union[('optuna.Trial', Dict[(str, Any)])], epoch: int, metrics: Dict[(str, float)]):
        if (self.hp_search_backend is None or trial is None):
            return
    ...
    def train(self, model_path: Optional[str] = None, trial: Union[('optuna.Trial', Dict[(str, Any)])] = None):
        ...
        if self.args.local_rank != -1:
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[self.args.local_rank], output_device=self.args.local_rank, find_unused_parameters=(not getattr(model.config, 'gradient_checkpointing', False) if isinstance(model, PreTrainedModel) else True))
        if is_torch_tpu_available():
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for (step, inputs) in enumerate(epoch_iterator):
                ...
                if ((step + 1) % self.args.gradient_accumulation_steps == 0 or (steps_in_epoch <= self.args.gradient_accumulation_steps and step + 1 == steps_in_epoch)):
                    if (self.args.fp16 and _use_native_amp):
    ...
    def _maybe_log_save_evalute(self, tr_loss, model, trial, epoch):
        if self.control.should_log:
            ...
            logs['learning_rate'] = (self.lr_scheduler.get_last_lr()[0] if version.parse(torch.__version__) >= version.parse('1.4') else self.lr_scheduler.get_lr()[0])
    ...
    def hyperparameter_search(self, hp_space: Optional[Callable[(['optuna.Trial'], Dict[(str, float)])]] = None, compute_objective: Optional[Callable[([Dict[(str, float)]], float)]] = None, n_trials: int = 20, direction: str = 'minimize', backend: Optional[Union[('str', HPSearchBackend)]] = None, **kwargs) -> BestRun:
        """
        ...
        if backend is None:
            ...
            if backend is None:
                raise RuntimeError('At least one of optuna or ray should be installed. To install optuna run `pip install optuna`.To install ray run `pip install ray[tune]`.')
        backend = HPSearchBackend(backend)
        if (backend == HPSearchBackend.OPTUNA and not is_optuna_available()):
            raise RuntimeError('You picked the optuna backend, but it is not installed. Use `pip install optuna`.')
        if (backend == HPSearchBackend.RAY and not is_ray_available()):
            raise RuntimeError("You picked the Ray Tune backend, but it is not installed. Use `pip install 'ray[tune]'`.")
    ...
    def training_step(self, model: nn.Module, inputs: Dict[(str, Union[(torch.Tensor, Any)])]) -> torch.Tensor:
        ...
        if hasattr(self, '_training_step'):
            warnings.warn("The `_training_step` method is deprecated and won't be called in a future version, define `training_step` in your subclass.", FutureWarning)
    ...
    def _sorted_checkpoints(self, checkpoint_prefix=PREFIX_CHECKPOINT_DIR, use_mtime=False) -> List[str]:
        ...
        if self.state.best_model_checkpoint is not None:
            ...
            (checkpoints_sorted[best_model_index], checkpoints_sorted[-1]) = (checkpoints_sorted[-1], checkpoints_sorted[best_model_index])
    ...
    @staticmethod
    def _actual_model(model: Union[(torch.nn.DataParallel, torch.nn.parallel.DistributedDataParallel, torch.nn.modules.Module)]) -> torch.nn.modules.Module:
        """

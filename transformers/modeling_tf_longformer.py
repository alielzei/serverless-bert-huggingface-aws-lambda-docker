...
from .modeling_tf_utils import TFMaskedLanguageModelingLoss, TFPreTrainedModel, TFQuestionAnsweringLoss, get_initializer, keras_serializable, shape_list
...
def _compute_global_attention_mask(input_ids_shape, sep_token_indices, before_sep_token=True):
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_tf_longformer._compute_global_attention_mask', '_compute_global_attention_mask(input_ids_shape, sep_token_indices, before_sep_token=True)', {'tf': tf, 'input_ids_shape': input_ids_shape, 'sep_token_indices': sep_token_indices, 'before_sep_token': before_sep_token}, 1)
...
class TFLongformerSelfAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        """
        (hidden_states, attention_mask, is_index_masked, is_index_global_attn, is_global_attn, output_attentions) = inputs
        ...
        diagonal_mask = self._sliding_chunks_query_key_matmul(tf.ones(shape_list(attention_mask), dtype=tf.float32), attention_mask, self.one_sided_attn_window_size)
        ...
        attn_output = tf.cond(is_global_attn, lambda: self._compute_global_attn_output_from_hidden(attn_output=attn_output, hidden_states=hidden_states, max_num_global_attn_indices=max_num_global_attn_indices, is_local_index_global_attn_nonzero=is_local_index_global_attn_nonzero, is_index_global_attn_nonzero=is_index_global_attn_nonzero, is_local_index_no_global_attn_nonzero=is_local_index_no_global_attn_nonzero, is_index_masked=is_index_masked, training=training), lambda: attn_output)
        attn_probs = tf.cond(is_global_attn, lambda: self._get_global_attn_probs(attn_probs, max_num_global_attn_indices), lambda: attn_probs)
        outputs = (attn_output, attn_probs)
        return outputs
    
    @staticmethod
    def _get_global_attn_probs(attn_probs, max_num_global_attn_indices):
        attn_probs = tf.concat([attn_probs[:, :, :, :max_num_global_attn_indices], tf.zeros_like(attn_probs)[:, :, :, max_num_global_attn_indices:]], axis=-1)
        return attn_probs
    ...
    def _sliding_chunks_query_key_matmul(self, query, key, window_overlap):
        ...
        diagonal_attn_scores_low_triang = tf.concat([tf.zeros((batch_size * num_heads, 1, window_overlap, window_overlap)), diagonal_chunked_attention_scores[:, :, -(window_overlap + 1):-1, window_overlap + 1:]], axis=1)
        diagonal_attn_scores_first_chunk = tf.concat([tf.roll(diagonal_chunked_attention_scores, shift=[1, window_overlap], axis=[2, 3])[:, :, :window_overlap, :window_overlap], tf.zeros((batch_size * num_heads, 1, window_overlap, window_overlap))], axis=1)
        first_chunk_mask = tf.broadcast_to(tf.range(chunks_count + 1)[None, :, None, None], shape=(batch_size * num_heads, chunks_count + 1, window_overlap, window_overlap)) < 1
    ...
    def _mask_invalid_locations(input_tensor, window_overlap):
        ...
        padding = tf.constant([[0, shape_list(input_tensor)[1] - window_overlap], [0, shape_list(input_tensor)[3] - window_overlap - 1]])
    ...
    def _compute_global_attn_output_from_hidden(self, attn_output, hidden_states, max_num_global_attn_indices, is_local_index_global_attn_nonzero, is_index_global_attn_nonzero, is_local_index_no_global_attn_nonzero, is_index_masked, training):
...
class TFLongformerAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        (hidden_states, attention_mask, is_index_masked, is_index_global_attn, is_global_attn, output_attentions) = inputs
        self_outputs = self.self_attention([hidden_states, attention_mask, is_index_masked, is_index_global_attn, is_global_attn, output_attentions], training=training)
...
class TFLongformerLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        (hidden_states, attention_mask, is_index_masked, is_index_global_attn, is_global_attn, output_attentions) = inputs
        attention_outputs = self.attention([hidden_states, attention_mask, is_index_masked, is_index_global_attn, is_global_attn, output_attentions], training=training)
...
class TFLongformerEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.layer = [TFLongformerLayer(config, i, name='layer_._{}'.format(i)) for i in range(config.num_hidden_layers)]
    ...
    def call(self, hidden_states, attention_mask=None, head_mask=None, padding_len=0, is_index_masked=None, is_index_global_attn=None, is_global_attn=None, output_attentions=None, output_hidden_states=None, return_dict=None, training=False):
        ...
        for (i, layer_module) in enumerate(self.layer):
            ...
            layer_outputs = layer_module([hidden_states, attention_mask, is_index_masked, is_index_global_attn, is_global_attn, output_attentions], training=training)
...
class TFLongformerMainLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, attention_mask=None, global_attention_mask=None, token_type_ids=None, position_ids=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None, training=False):
...
class TFLongformerPreTrainedModel(TFPreTrainedModel):
    ...
    def dummy_inputs(self):
        ...
        return {'input_ids': input_ids, 'attention_mask': attention_mask, 'global_attention_mask': global_attention_mask}
...
class TFLongformerForMaskedLM(TFLongformerPreTrainedModel, TFMaskedLanguageModelingLoss):
    ...
    def call(self, inputs=None, attention_mask=None, global_attention_mask=None, token_type_ids=None, position_ids=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None, labels=None, training=False):
        """
...
class TFLongformerForQuestionAnswering(TFLongformerPreTrainedModel, TFQuestionAnsweringLoss):
    ...
    def call(self, inputs=None, attention_mask=None, global_attention_mask=None, token_type_ids=None, position_ids=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None, start_positions=None, end_positions=None, training=False):
        """
        ...
        if (global_attention_mask is None and input_ids is not None):
            if input_ids is None:
                logger.warning('It is not possible to automatically generate the `global_attention_mask`. Please make sure that it is correctly set.')
            elif tf.where(input_ids == self.config.sep_token_id).shape[0] != 3 * input_ids.shape[0]:
                logger.warning(f'There should be exactly three separator tokens: {self.config.sep_token_id} in every sample for questions answering. You might also consider to set `global_attention_mask` manually in the forward function to avoid this. This is most likely an error.')
            else:

...
def get_proto(filename: str):
    ...
    return custom_funtemplate.rewrite_template('transformers.convert_slow_tokenizer.get_proto', 'get_proto(filename)', {'model': model, 'filename': filename}, 1)
...
class SpmConverter(Converter):
    ...
    def converted(self) -> Tokenizer:
        ...
        tokenizer.pre_tokenizer = pre_tokenizers.Sequence([pre_tokenizers.WhitespaceSplit(), pre_tokenizers.Metaspace(replacement=replacement, add_prefix_space=add_prefix_space)])
...
class CamembertConverter(SpmConverter):
    ...
    def vocab(self, proto):
        vocab = [('<s>NOTUSED', 0.0), ('<pad>', 0.0), ('</s>NOTUSED', 0.0), ('<unk>', 0.0)]
...
class PegasusConverter(SpmConverter):
    ...
    def vocab(self, proto):
        vocab = [(self.original_tokenizer.pad_token, 0), (self.original_tokenizer.eos_token, 0)]
    ...
    def post_processor(self):
        ...
        return processors.TemplateProcessing(single=['$A', eos], pair=['$A', '$B', eos], special_tokens=[(eos, self.original_tokenizer.eos_token_id)])
...
def convert_slow_tokenizer(transformer_tokenizer) -> Tokenizer:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.convert_slow_tokenizer.convert_slow_tokenizer', 'convert_slow_tokenizer(transformer_tokenizer)', {'SLOW_TO_FAST_CONVERTERS': SLOW_TO_FAST_CONVERTERS, 'transformer_tokenizer': transformer_tokenizer}, 1)

...
from .file_utils import DUMMY_INPUTS, TF2_WEIGHTS_NAME, TF_WEIGHTS_NAME, WEIGHTS_NAME, ModelOutput, cached_path, hf_bucket_url, is_remote_url, is_torch_tpu_available, replace_return_docstrings
...
def find_pruneable_heads_and_indices(heads: List[int], n_heads: int, head_size: int, already_pruned_heads: Set[int]) -> Tuple[(Set[int], torch.LongTensor)]:
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_utils.find_pruneable_heads_and_indices', 'find_pruneable_heads_and_indices(heads, n_heads, head_size, already_pruned_heads)', {'torch': torch, 'heads': heads, 'n_heads': n_heads, 'head_size': head_size, 'already_pruned_heads': already_pruned_heads, 'List': List, 'int': int, 'Set': Set, 'int': int, 'Tuple': Tuple}, 2)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if pretrained_model_name_or_path is not None:
            if os.path.isdir(pretrained_model_name_or_path):
                ...
                archive_file = hf_bucket_url(pretrained_model_name_or_path, filename=(TF2_WEIGHTS_NAME if from_tf else WEIGHTS_NAME), use_cdn=use_cdn, mirror=mirror)
            try:
...
def prune_linear_layer(layer: torch.nn.Linear, index: torch.LongTensor, dim: int = 0) -> torch.nn.Linear:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_utils.prune_linear_layer', 'prune_linear_layer(layer, index, dim=0)', {'nn': nn, 'layer': layer, 'index': index, 'dim': dim, 'torch': torch, 'torch': torch, 'torch': torch}, 1)
...
def prune_conv1d_layer(layer: Conv1D, index: torch.LongTensor, dim: int = 1) -> Conv1D:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_utils.prune_conv1d_layer', 'prune_conv1d_layer(layer, index, dim=1)', {'Conv1D': Conv1D, 'layer': layer, 'index': index, 'dim': dim, 'torch': torch}, 1)
...
def prune_layer(layer: Union[(torch.nn.Linear, Conv1D)], index: torch.LongTensor, dim: Optional[int] = None) -> Union[(torch.nn.Linear, Conv1D)]:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_utils.prune_layer', 'prune_layer(layer, index, dim=None)', {'nn': nn, 'prune_linear_layer': prune_linear_layer, 'Conv1D': Conv1D, 'prune_conv1d_layer': prune_conv1d_layer, 'layer': layer, 'index': index, 'dim': dim, 'Union': Union, 'torch': torch, 'Optional': Optional, 'int': int, 'Union': Union}, 1)
...
def apply_chunking_to_forward(forward_fn: Callable[(..., torch.Tensor)], chunk_size: int, chunk_dim: int, *input_tensors) -> torch.Tensor:
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_utils.apply_chunking_to_forward', 'apply_chunking_to_forward(forward_fn, chunk_size, chunk_dim, *input_tensors)', {'inspect': inspect, 'torch': torch, 'forward_fn': forward_fn, 'chunk_size': chunk_size, 'chunk_dim': chunk_dim, 'input_tensors': input_tensors, 'Callable': Callable, 'torch': torch}, 1)

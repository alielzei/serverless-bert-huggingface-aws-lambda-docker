...
def softmax(hidden_state, dim, onnx_trace=False):
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_prophetnet.softmax', 'softmax(hidden_state, dim, onnx_trace=False)', {'F': F, 'torch': torch, 'hidden_state': hidden_state, 'dim': dim, 'onnx_trace': onnx_trace}, 1)
...
def ngram_attention_bias(sequence_length, ngram, device, dtype):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_prophetnet.ngram_attention_bias', 'ngram_attention_bias(sequence_length, ngram, device, dtype)', {'torch': torch, 'sequence_length': sequence_length, 'ngram': ngram, 'device': device, 'dtype': dtype}, 1)
...
def compute_relative_buckets(num_buckets, max_distance, relative_positions, is_bidirectional=False):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_prophetnet.compute_relative_buckets', 'compute_relative_buckets(num_buckets, max_distance, relative_positions, is_bidirectional=False)', {'torch': torch, 'math': math, 'num_buckets': num_buckets, 'max_distance': max_distance, 'relative_positions': relative_positions, 'is_bidirectional': is_bidirectional}, 1)
...
def compute_all_stream_relative_buckets(num_buckets, max_distance, position_ids):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_prophetnet.compute_all_stream_relative_buckets', 'compute_all_stream_relative_buckets(num_buckets, max_distance, position_ids)', {'torch': torch, 'compute_relative_buckets': compute_relative_buckets, 'num_buckets': num_buckets, 'max_distance': max_distance, 'position_ids': position_ids}, 2)
...
def ProphetNetLayerNorm(normalized_shape, eps=1e-05, elementwise_affine=True):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_prophetnet.ProphetNetLayerNorm', 'ProphetNetLayerNorm(normalized_shape, eps=1e-05, elementwise_affine=True)', {'torch': torch, 'normalized_shape': normalized_shape, 'eps': eps, 'elementwise_affine': elementwise_affine}, 1)
...
class ProphetNetSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, key_value_states: Optional[Tensor] = None, attention_mask: Optional[Tensor] = None, layer_state: Optional[Dict[(str, Optional[Tensor])]] = None) -> Tuple[(Tensor, Optional[Tensor])]:
        ...
        assert list(hidden_states.size()) == [sequence_length, batch_size, hidden_size], f'Size of hidden states should be {(sequence_length, batch_size, hidden_size)}, but is {hidden_states.size()}'
        ...
        if is_cross_attention:
            layer_state[cache_key] = {'prev_key_states': key_states.view(batch_size, self.num_attn_heads, -1, self.head_dim), 'prev_value_states': value_states.view(batch_size, self.num_attn_heads, -1, self.head_dim)}
        ...
        if (attention_mask is not None and attention_mask.dim() == 0):
            attention_mask = None
        assert (attention_mask is None or attention_mask.size() == (self.num_attn_heads * batch_size, 1, key_sequence_length)), f'`attention_mask` should be `None` or of shape attention_mask.size() == {(batch_size * self.num_attn_heads, 1, key_sequence_length)}, but is {attention_mask.shape}'
        if attention_mask is not None:
            attn_weights = attn_weights + attention_mask
        attn_weights = F.softmax(attn_weights, dim=-1)
        attn_probs = F.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        attn_output = torch.bmm(attn_probs, value_states)
        assert attn_output.size() == (batch_size * self.num_attn_heads, sequence_length, self.head_dim), '`attn_output` should be of shape {batch_size * self.num_attn_heads, sequence_length, self.head_dim}, but is of shape {attn_output.size()}'
...
class ProphetNetNgramProphetNetSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, layer_state=None, attention_mask=None, extended_predict_attention_mask=None, main_relative_position_buckets=None, predict_relative_position_buckets=None, position_ids=None):
        (sequence_length, batch_size, hidden_size) = hidden_states.size()
        ...
        if saved_state is not None:
            prev_main_key_states = saved_state['prev_key_states'].view(batch_size * self.num_attn_heads, -1, self.head_dim)
            main_key_states = torch.cat((prev_main_key_states, main_key_states), dim=1)
            prev_main_value_states = saved_state['prev_value_states'].view(batch_size * self.num_attn_heads, -1, self.head_dim)
            main_value_states = torch.cat((prev_main_value_states, main_value_states), dim=1)
        layer_state['self'] = {'prev_key_states': main_key_states.view(batch_size, self.num_attn_heads, -1, self.head_dim), 'prev_value_states': main_value_states.view(batch_size, self.num_attn_heads, -1, self.head_dim)}
        ...
        main_attn_output = main_attn_output.transpose(0, 1).contiguous().view(1, main_sequence_length, batch_size, hidden_size)
        main_attn_output = self.out_proj(main_attn_output)
        predict_query_states = torch.cat(predict_query_states_list, 0).view(self.ngram, -1, main_sequence_length, self.head_dim)
        predict_key_states = torch.cat([torch.cat([main_key_states, key], 1).unsqueeze(0) for key in predict_key_states_list], 0)
        predict_hidden_states = torch.cat(hidden_states_predict_list, 0).view(self.ngram, main_sequence_length, batch_size, hidden_size)
        predict_value_states = torch.cat([torch.cat([main_value_states, v_p], 1).unsqueeze(0) for v_p in predict_value_states_list], 0)
        ...
        predict_attn_output = predict_attn_output.transpose(1, 2).contiguous().view(self.ngram, main_sequence_length, batch_size, hidden_size)
        ...
        predict_attn_probs = predict_attn_probs.view(self.ngram, batch_size, self.num_attn_heads, main_sequence_length, -1).transpose(0, 1)
        attn_output = F.dropout(attn_output, p=self.dropout, training=self.training)
    ...
    def get_main_relative_pos_embeddings(self, hidden_states, attn_weights, position_ids, main_relative_position_buckets):
        ...
        rel_pos_embeddings = rel_pos_embeddings.view(rel_pos_embeddings.shape[:2] + (self.num_buckets, self.num_attn_heads)).permute(0, 3, 1, 2)
        rel_pos_embeddings = rel_pos_embeddings.reshape(attn_weights.shape[:2] + (-1, ))
        main_relative_position_buckets = main_relative_position_buckets.repeat(1, self.num_attn_heads, 1).view(-1, main_relative_position_buckets.shape[-1]).long()
        rel_pos_embeddings = rel_pos_embeddings.reshape(-1, rel_pos_embeddings.size(-1))
        main_relative_pos_embeddings = torch.gather(rel_pos_embeddings, dim=1, index=main_relative_position_buckets).view(attn_weights.shape[:2] + (-1, ))
    ...
    def get_predict_relative_pos_embeddings(self, hidden_states, attn_weights, position_ids, predict_relative_position_buckets):
        ...
        rel_pos_embeddings = self.relative_pos_embeddings(hidden_states).view(hidden_states.shape[:-1] + (self.num_buckets, self.num_attn_heads))
        rel_pos_embeddings = rel_pos_embeddings.permute(0, 1, 4, 2, 3).reshape(self.ngram * batch_size * self.num_attn_heads, sequence_length, -1)
        predict_relative_position_buckets = predict_relative_position_buckets.unsqueeze(0).repeat(self.ngram, 1, self.num_attn_heads, 1)
        rel_pos_embeddings = rel_pos_embeddings.reshape(-1, rel_pos_embeddings.size(-1))
        predict_relative_position_buckets = predict_relative_position_buckets.view(-1, predict_relative_position_buckets.size(-1)).long()
        predict_relative_pos_embeddings = torch.gather(rel_pos_embeddings, dim=1, index=predict_relative_position_buckets).view(self.ngram, batch_size * self.num_attn_heads, sequence_length, -1)
...
class ProphetNetDecoderLayer(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, encoder_attn_mask=None, layer_state=None, attention_mask=None, extended_predict_attention_mask=None, main_relative_position_buckets=None, predict_relative_position_buckets=None, position_ids=None):
        ...
        if encoder_hidden_states is not None:
            (attention_output, cross_attn_weights) = self.cross_attn(hidden_states=hidden_states, key_value_states=encoder_hidden_states, attention_mask=encoder_attn_mask, layer_state=layer_state)
            hidden_states = self.cross_attn_layer_norm(attention_output + hidden_states)
        feed_forward_output = self.feed_forward(hidden_states)
        hidden_states = self.feed_forward_layer_norm(feed_forward_output + hidden_states)
        return (hidden_states, self_attn_weights, self_attn_weights_ngram, cross_attn_weights, layer_state)
...
class ProphetNetEncoder(ProphetNetPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        ...
        if attention_mask is not None:
            extended_attention_mask = (1.0 - attention_mask[:, None, :].repeat(self.config.num_attention_heads, 1, 1)) * -10000.0
...
class ProphetNetDecoder(ProphetNetPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, past_key_values=None, inputs_embeds=None, use_cache=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        ...
        if past_key_values is not None:
            assert hidden_states.size(0) == 1, 'At the moment `use_cache` is only supported for `decoder_input_ids` of length 1'
            ngram_hidden_states = [(ngram_embeddings[ngram - 1] + predicting_stream_pos_embed).transpose(0, 1).repeat(1, batch_size, 1) for ngram in range(self.ngram)]
            extended_attention_mask = None
            extended_predict_attention_mask = None
        else:
            ngram_hidden_states = [(ngram_embeddings[ngram - 1] + predicting_stream_pos_embed).transpose(0, 1) for ngram in range(self.ngram)]
            extended_attention_mask = self.prepare_attention_mask(hidden_states, attention_mask)
            extended_predict_attention_mask = self.prepare_predict_attention_mask(hidden_states, attention_mask)
        if encoder_attention_mask is not None:
            extended_encoder_attention_mask = (1.0 - encoder_attention_mask[:, None, :].repeat(self.config.num_attention_heads, 1, 1)) * -10000.0
    ...
    def prepare_predict_attention_mask(self, hidden_states, attention_mask):
        ...
        extended_predict_causal_mask = predict_causal_mask[:, None, :, :].expand(predict_causal_mask.shape[:1] + (batch_size, ) + predict_causal_mask.shape[1:])
        if attention_mask is not None:
        ...
        return extended_predict_attention_mask.repeat(1, self.config.num_decoder_attention_heads, 1, 1).to(hidden_states.dtype)
...
class ProphetNetModel(ProphetNetPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, decoder_input_ids=None, decoder_attention_mask=None, encoder_outputs: Optional[Tuple] = None, past_key_values=None, inputs_embeds=None, decoder_inputs_embeds=None, use_cache=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        """
        Returns:
...
class ProphetNetForConditionalGeneration(ProphetNetPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, decoder_input_ids=None, decoder_attention_mask=None, encoder_outputs=None, past_key_values=None, inputs_embeds=None, decoder_inputs_embeds=None, labels=None, use_cache=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        """
    ...
    def prepare_inputs_for_generation(self, decoder_input_ids, past, attention_mask, use_cache, encoder_outputs, **kwargs):
        ...
        return {'input_ids': None, 'encoder_outputs': encoder_outputs, 'past_key_values': past, 'decoder_input_ids': decoder_input_ids, 'attention_mask': attention_mask, 'use_cache': use_cache}
    ...
    def _reorder_cache(past, beam_idx):
        ...
        for layer_past in past:
            layer_past_new = {attn_key: _reorder_cache(attn_cache, beam_idx) for (attn_key, attn_cache) in layer_past.items()}
...
class ProphetNetForCausalLM(ProphetNetPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, past_key_values=None, inputs_embeds=None, labels=None, use_cache=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        """
        encoder_hidden_states (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length, hidden_size)`, `optional`):
            Sequence of hidden-states at the output of the last layer of the encoder. Used in the cross-attention
            if the model is configured as a decoder.
        ...
        if not return_dict:
            ...
            return ProphetNetDecoderLMOutput(loss=loss, logits=logits, logits_ngram=logits_ngram, past_key_values=outputs.past_key_values, hidden_states=outputs.hidden_states, hidden_states_ngram=outputs.hidden_states_ngram, attentions=outputs.attentions, ngram_attentions=outputs.ngram_attentions)

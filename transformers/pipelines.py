...
def get_default_model(targeted_task: Dict, framework: Optional[str]) -> str:
    ...
    return custom_funtemplate.rewrite_template('transformers.pipelines.get_default_model', 'get_default_model(targeted_task, framework)', {'is_torch_available': is_torch_available, 'is_tf_available': is_tf_available, 'targeted_task': targeted_task, 'framework': framework, 'Optional': Optional, 'str': str}, 1)
    ...
    def handle_args(args: Sequence[Any]) -> List[str]:
        if len(args) == 1:
            ...
            if all([isinstance(arg, str) for arg in args]):
                ...
                raise ValueError('Invalid input type {}. Pipeline supports Union[str, Iterable[str]]'.format(type(args)))
        else:
            return []
...
class Pipeline(_ScikitCompat):
    ...
    def __init__(self, model: Union[('PreTrainedModel', 'TFPreTrainedModel')], tokenizer: PreTrainedTokenizer, modelcard: Optional[ModelCard] = None, framework: Optional[str] = None, task: str = '', args_parser: ArgumentHandler = None, device: int = -1, binary_output: bool = False):
    ...
    def check_model_type(self, supported_models: Union[(List[str], dict)]):
        ...
        if self.model.__class__.__name__ not in supported_models:
            raise PipelineException(self.task, self.model.base_model_prefix, f"The model '{self.model.__class__.__name__}' is not supported for {self.task}. Supported models are {supported_models}")
    ...
    def _parse_and_tokenize(self, *args, padding=True, add_special_tokens=True, **kwargs):
        ...
        inputs = self.tokenizer(inputs, add_special_tokens=add_special_tokens, return_tensors=self.framework, padding=padding)
        return inputs
...
class FeatureExtractionPipeline(Pipeline):
    ...
    def __init__(self, model: Union[('PreTrainedModel', 'TFPreTrainedModel')], tokenizer: PreTrainedTokenizer, modelcard: Optional[ModelCard] = None, framework: Optional[str] = None, args_parser: ArgumentHandler = None, device: int = -1, task: str = ''):
        super().__init__(model=model, tokenizer=tokenizer, modelcard=modelcard, framework=framework, args_parser=args_parser, device=device, binary_output=True, task=task)
...
class TextGenerationPipeline(Pipeline):
    ...
    ALLOWED_MODELS = ['XLNetLMHeadModel', 'TransfoXLLMHeadModel', 'ReformerModelWithLMHead', 'GPT2LMHeadModel', 'OpenAIGPTLMHeadModel', 'CTRLLMHeadModel', 'TFXLNetLMHeadModel', 'TFTransfoXLLMHeadModel', 'TFGPT2LMHeadModel', 'TFOpenAIGPTLMHeadModel', 'TFCTRLLMHeadModel']
    ...
    def _parse_and_tokenize(self, *args, padding=True, add_special_tokens=True, **kwargs):
        ...
        if self.model.__class__.__name__ in ['TransfoXLLMHeadModel']:
            ...
            tokenizer_kwargs = {}
        inputs = self._args_parser(*args, **kwargs)
        inputs = self.tokenizer(inputs, add_special_tokens=add_special_tokens, return_tensors=self.framework, padding=padding, **tokenizer_kwargs)
        return inputs
    
    def __call__(self, *args, return_tensors=False, return_text=True, clean_up_tokenization_spaces=False, prefix=None, **generate_kwargs):
        """
        ...
        for prompt_text in text_inputs:
            with self.device_placement():
                ...
                assert (input_ids is None or input_ids.shape[0] == 1), 'Batch generation is currently not supported. See https://github.com/huggingface/transformers/issues/3021 for more information.'
                output_sequences = self.model.generate(input_ids=input_ids, **generate_kwargs)
            result = []
            for generated_sequence in output_sequences:
                ...
                if return_text:
                    text = self.tokenizer.decode(generated_sequence, skip_special_tokens=True, clean_up_tokenization_spaces=clean_up_tokenization_spaces)
...
class TextClassificationPipeline(Pipeline):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.return_all_scores:
            return [[{'label': self.model.config.id2label[i], 'score': score.item()} for (i, score) in enumerate(item)] for item in scores]
        else:
...
class ZeroShotClassificationPipeline(Pipeline):
    ...
    def _parse_and_tokenize(self, *args, padding=True, add_special_tokens=True, **kwargs):
        ...
        """
        inputs = self._args_parser(*args, **kwargs)
        inputs = self.tokenizer(inputs, add_special_tokens=add_special_tokens, return_tensors=self.framework, padding=padding, truncation='only_first')
        return inputs
    ...
    def __call__(self, sequences, candidate_labels, hypothesis_template='This example is {}.', multi_class=False):
        ...
        for iseq in range(num_sequences):
            ...
            result.append({'sequence': (sequences if isinstance(sequences, str) else sequences[iseq]), 'labels': [candidate_labels[i] for i in top_inds], 'scores': scores[iseq][top_inds].tolist()})
        if len(result) == 1:
            return result[0]
        return result
...
class FillMaskPipeline(Pipeline):
    ...
    def __init__(self, model: Union[('PreTrainedModel', 'TFPreTrainedModel')], tokenizer: PreTrainedTokenizer, modelcard: Optional[ModelCard] = None, framework: Optional[str] = None, args_parser: ArgumentHandler = None, device: int = -1, topk=5, task: str = ''):
        super().__init__(model=model, tokenizer=tokenizer, modelcard=modelcard, framework=framework, args_parser=args_parser, device=device, binary_output=True, task=task)
    ...
    def ensure_exactly_one_mask_token(self, masked_index: np.ndarray):
        ...
        if numel > 1:
            raise PipelineException('fill-mask', self.model.base_model_prefix, f'More than one mask_token ({self.tokenizer.mask_token}) is not supported')
        elif numel < 1:
    ...
    def __call__(self, *args, targets=None, **kwargs):
        ...
        if targets is not None:
            ...
            for target in targets:
                ...
                if (len(target_enc) > 1 or target_enc[0] == self.tokenizer.unk_token):
                    logger.warning('The specified target token `{}` does not exist in the model vocabulary. Replacing with `{}`.'.format(target, target_enc[0]))
        ...
        for i in range(batch_size):
            ...
            for (v, p) in zip(values.tolist(), predictions.tolist()):
                ...
                result.append({'sequence': self.tokenizer.decode(tokens), 'score': v, 'token': p, 'token_str': self.tokenizer.convert_ids_to_tokens(p)})
            results += [result]
        if len(results) == 1:
            return results[0]
        return results
...
class TokenClassificationPipeline(Pipeline):
    ...
    def __init__(self, model: Union[('PreTrainedModel', 'TFPreTrainedModel')], tokenizer: PreTrainedTokenizer, modelcard: Optional[ModelCard] = None, framework: Optional[str] = None, args_parser: ArgumentHandler = None, device: int = -1, binary_output: bool = False, ignore_labels=['O'], task: str = '', grouped_entities: bool = False):
        super().__init__(model=model, tokenizer=tokenizer, modelcard=modelcard, framework=framework, args_parser=args_parser, device=device, binary_output=binary_output, task=task)
        self.check_model_type((TF_MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING if self.framework == 'tf' else MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING))
    ...
    def __call__(self, *args, **kwargs):
        ...
        for sentence in inputs:
            with self.device_placement():
                tokens = self.tokenizer(sentence, return_attention_mask=False, return_tensors=self.framework, truncation=True)
                if self.framework == 'tf':
            ...
            entities = []
            filtered_labels_idx = [(idx, label_idx) for (idx, label_idx) in enumerate(labels_idx) if self.model.config.id2label[label_idx] not in self.ignore_labels]
            for (idx, label_idx) in filtered_labels_idx:
                entity = {'word': self.tokenizer.convert_ids_to_tokens(int(input_ids[idx])), 'score': score[idx][label_idx].item(), 'entity': self.model.config.id2label[label_idx], 'index': idx}
                entities += [entity]
    ...
    def group_sub_entities(self, entities: List[dict]) -> dict:
        ...
        entity_group = {'entity_group': entity, 'score': np.mean(scores), 'word': self.tokenizer.convert_tokens_to_string(tokens)}
        return entity_group
    ...
    def group_entities(self, entities: List[dict]) -> List[dict]:
        ...
        for entity in entities:
            ...
            if not entity_group_disagg:
                ...
                continue
            if (entity['entity'].split('-')[-1] == entity_group_disagg[-1]['entity'].split('-')[-1] and entity['index'] == entity_group_disagg[-1]['index'] + 1):
                entity_group_disagg += [entity]
                if is_last_idx:
                    entity_groups += [self.group_sub_entities(entity_group_disagg)]
            else:
        ...
        if ('X' in kwargs or 'data' in kwargs):
            ...
            for (i, item) in enumerate(inputs):
                if isinstance(item, dict):
                    ...
                    raise ValueError('{} argument needs to be of type (list[SquadExample | dict], SquadExample, dict)'.format(('X' if 'X' in kwargs else 'data')))
...
class QuestionAnsweringPipeline(Pipeline):
    ...
    def __init__(self, model: Union[('PreTrainedModel', 'TFPreTrainedModel')], tokenizer: PreTrainedTokenizer, modelcard: Optional[ModelCard] = None, framework: Optional[str] = None, device: int = -1, task: str = '', **kwargs):
        super().__init__(model=model, tokenizer=tokenizer, modelcard=modelcard, framework=framework, args_parser=QuestionAnsweringArgumentHandler(), device=device, task=task, **kwargs)
    ...
    def __call__(self, *args, **kwargs):
        ...
        for (features, example) in zip(features_list, examples):
            ...
            for (feature, start_, end_) in zip(features, start, end):
                ...
                answers += [{'score': score.item(), 'start': np.where(char_to_word == feature.token_to_orig_map[s])[0][0].item(), 'end': np.where(char_to_word == feature.token_to_orig_map[e])[0][-1].item(), 'answer': ' '.join(example.doc_tokens[feature.token_to_orig_map[s]:feature.token_to_orig_map[e] + 1])} for (s, e, score) in zip(starts, ends, scores)]
            if kwargs['handle_impossible_answer']:
...
class SummarizationPipeline(Pipeline):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.check_model_type((TF_MODEL_WITH_LM_HEAD_MAPPING if self.framework == 'tf' else MODEL_FOR_SEQ_TO_SEQ_CAUSAL_LM_MAPPING))
    ...
    def __call__(self, *documents, return_tensors=False, return_text=True, clean_up_tokenization_spaces=False, **generate_kwargs):
        ...
        if (self.framework == 'tf' and 'BartForConditionalGeneration' in self.model.__class__.__name__):
            raise NotImplementedError('Tensorflow is not yet supported for Bart. Please consider using T5, e.g. `t5-base`')
        ...
        with self.device_placement():
            ...
            if input_length < min_length // 2:
                logger.warning("Your min_length is set to {}, but you input_length is only {}. You might consider decreasing min_length manually, e.g. summarizer('...', min_length=10)".format(min_length, input_length))
            max_length = generate_kwargs.get('max_length', self.model.config.max_length)
            if input_length < max_length:
                logger.warning("Your max_length is set to {}, but you input_length is only {}. You might consider decreasing max_length manually, e.g. summarizer('...', max_length=50)".format(max_length, input_length))
            summaries = self.model.generate(inputs['input_ids'], attention_mask=inputs['attention_mask'], **generate_kwargs)
            results = []
            for summary in summaries:
                ...
                if return_text:
                    record['summary_text'] = self.tokenizer.decode(summary, skip_special_tokens=True, clean_up_tokenization_spaces=clean_up_tokenization_spaces)
...
class TranslationPipeline(Pipeline):
    ...
    def __call__(self, *args, return_tensors=False, return_text=True, clean_up_tokenization_spaces=False, **generate_kwargs):
        """
        ...
        if isinstance(args[0], list):
            ...
            raise ValueError(' `documents[0]`: {} have the wrong format. The should be either of type `str` or type `list`'.format(args[0]))
        with self.device_placement():
            ...
            if input_length > 0.9 * max_length:
                logger.warning("Your input_length: {} is bigger than 0.9 * max_length: {}. You might consider increasing your max_length manually, e.g. translator('...', max_length=400)".format(input_length, max_length))
            translations = self.model.generate(inputs['input_ids'], attention_mask=inputs['attention_mask'], **generate_kwargs)
            results = []
            for translation in translations:
                ...
                if return_text:
                    record['translation_text'] = self.tokenizer.decode(translation, skip_special_tokens=True, clean_up_tokenization_spaces=clean_up_tokenization_spaces)
                results.append(record)
            return results
...
class Text2TextGenerationPipeline(Pipeline):
    ...
    def __call__(self, *args, return_tensors=False, return_text=True, clean_up_tokenization_spaces=False, **generate_kwargs):
        ...
        with self.device_placement():
            ...
            if self.framework == 'pt':
                inputs = self.ensure_tensor_on_device(**inputs)
            generations = self.model.generate(inputs['input_ids'], attention_mask=inputs['attention_mask'], **generate_kwargs)
            results = []
            for generation in generations:
                ...
                if return_text:
                    record['generated_text'] = self.tokenizer.decode(generation, skip_special_tokens=True, clean_up_tokenization_spaces=clean_up_tokenization_spaces)
                results.append(record)
            return results
...
class Conversation:
    ...
    def add_user_input(self, text: str, overwrite: bool = False):
        ...
        if self.new_user_input:
            if overwrite:
                logger.warning('User input added while unprocessed input was existing: "{}" was overwritten with: "{}".'.format(self.new_user_input, text))
                self.new_user_input = text
            else:
                logger.warning('User input added while unprocessed input was existing: "{}" new input ignored: "{}". Set `overwrite` to True to overwrite unprocessed user input'.format(self.new_user_input, text))
        else:
            self.new_user_input = text
...
class ConversationalPipeline(Pipeline):
    ...
    def __call__(self, conversations: Union[(Conversation, List[Conversation])], clean_up_tokenization_spaces=True, **generate_kwargs):
        """
        ...
        if isinstance(conversations, list):
            for conversation in conversations:
                assert isinstance(conversation, Conversation), 'DialoguePipeline expects a Conversation or list of Conversations as an input'
                if conversation.new_user_input is None:
                    raise ValueError("Conversation with UUID {} does not contain new user input to process. Add user inputs with the conversation's `add_user_input` method".format(type(conversation.uuid)))
        ...
        with self.device_placement():
            ...
            if self.framework == 'pt':
                ...
                input_length = tf.shape(inputs['input_ids'])[-1].numpy()
            if input_length > 0.9 * max_length:
                logger.warning('Longest conversation length: {} is bigger than 0.9 * max_length: {}. You might consider trimming the early phase of the conversation'.format(input_length, max_length))
            generated_responses = self.model.generate(inputs['input_ids'], attention_mask=inputs['attention_mask'], **generate_kwargs)
            cleaned_history = self._clean_padding_history(generated_responses)
            output = []
            for (conversation_index, conversation) in enumerate(conversations):
                conversation.mark_processed()
                conversation.generated_responses.append(self.tokenizer.decode(cleaned_history[conversation_index][input_length:], skip_special_tokens=True, clean_up_tokenization_spaces=clean_up_tokenization_spaces))
    ...
    def _concat_inputs_history(self, inputs: List[List[int]], histories: List[Optional[List[int]]], max_length: int):
        ...
        outputs = BatchEncoding({'input_ids': outputs, 'attention_mask': [[1] * len(outputs)]}, tensor_type=self.framework)
        return outputs
...
def pipeline(task: str, model: Optional = None, config: Optional[Union[(str, PretrainedConfig)]] = None, tokenizer: Optional[Union[(str, PreTrainedTokenizer)]] = None, framework: Optional[str] = None, **kwargs) -> Pipeline:
    """
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.pipelines.pipeline', 'pipeline(task, model=None, config=None, tokenizer=None, framework=None, **kwargs)', {'SUPPORTED_TASKS': SUPPORTED_TASKS, 'get_default_model': get_default_model, 'get_framework': get_framework, 'AutoTokenizer': AutoTokenizer, 'AutoConfig': AutoConfig, 'ModelCard': ModelCard, 'logger': logger, 'task': task, 'model': model, 'config': config, 'tokenizer': tokenizer, 'framework': framework, 'kwargs': kwargs, 'Optional': Optional, 'Optional': Optional, 'Optional': Optional, 'str': str}, 1)

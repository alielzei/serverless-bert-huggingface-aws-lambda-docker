...
class TFMobileBertLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        if config.num_feedforward_networks > 1:
            self.ffn = [TFFFNLayer(config, name='ffn.{}'.format(i)) for i in range(config.num_feedforward_networks - 1)]
...
class TFMobileBertLMPredictionHead(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.dense = self.add_weight(shape=(self.config.hidden_size - self.config.embedding_size, self.vocab_size), initializer='zeros', trainable=True, name='dense/weight')
...
class TFMobileBertForPreTraining(TFMobileBertPreTrainedModel):
    ...
    def call(self, inputs, **kwargs):
        ...
        return TFMobileBertForPreTrainingOutput(prediction_logits=prediction_scores, seq_relationship_logits=seq_relationship_score, hidden_states=outputs.hidden_states, attentions=outputs.attentions)
...
class TFMobileBertForMaskedLM(TFMobileBertPreTrainedModel, TFMaskedLanguageModelingLoss):
    ...
    def call(self, inputs=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None, labels=None, training=False):
        ...
        outputs = self.mobilebert(inputs, attention_mask=attention_mask, token_type_ids=token_type_ids, position_ids=position_ids, head_mask=head_mask, inputs_embeds=inputs_embeds, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict, training=training)

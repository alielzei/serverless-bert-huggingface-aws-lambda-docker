...
def _check_is_max_context(doc_spans, cur_span_index, position):
    ...
    return custom_funtemplate.rewrite_template('transformers.data.processors.squad._check_is_max_context', '_check_is_max_context(doc_spans, cur_span_index, position)', {'doc_spans': doc_spans, 'cur_span_index': cur_span_index, 'position': position}, 1)
...
def squad_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, padding_strategy, is_training):
    ...
    sequence_added_tokens = (tokenizer.max_len - tokenizer.max_len_single_sentence + 1 if tokenizer_type in MULTI_SEP_TOKENS_TOKENIZERS_SET else tokenizer.max_len - tokenizer.max_len_single_sentence)

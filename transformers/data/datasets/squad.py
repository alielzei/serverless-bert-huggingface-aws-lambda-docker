...
class SquadDataset(Dataset):
    ...
    def __init__(self, args: SquadDataTrainingArguments, tokenizer: PreTrainedTokenizer, limit_length: Optional[int] = None, mode: Union[(str, Split)] = Split.train, is_language_sensitive: Optional[bool] = False, cache_dir: Optional[str] = None, dataset_format: Optional[str] = 'pt'):
        ...
        cached_features_file = os.path.join((cache_dir if cache_dir is not None else args.data_dir), 'cached_{}_{}_{}_{}'.format(mode.value, tokenizer.__class__.__name__, str(args.max_seq_length), version_tag))
        ...
        with FileLock(lock_path):
            if (os.path.exists(cached_features_file) and not args.overwrite_cache):
                ...
                if (self.dataset is None or self.examples is None):
                    logger.warn(f'Deleting cached file {cached_features_file} will allow dataset and examples to be cached in future run')
            else:
                if mode == Split.dev:

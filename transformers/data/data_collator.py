...
def default_data_collator(features: List[InputDataClass]) -> Dict[(str, torch.Tensor)]:
    ...
    return custom_funtemplate.rewrite_template('transformers.data.data_collator.default_data_collator', 'default_data_collator(features)', {'BatchEncoding': BatchEncoding, 'torch': torch, 'features': features, 'List': List, 'InputDataClass': InputDataClass, 'Dict': Dict}, 1)
...
class DataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[(str, Union[(List[int], torch.Tensor)])]]) -> Dict[(str, torch.Tensor)]:
        batch = self.tokenizer.pad(features, padding=self.padding, max_length=self.max_length, pad_to_multiple_of=self.pad_to_multiple_of, return_tensors='pt')
        if 'label' in batch:
    ...
    def _tensorize_batch(self, examples: List[Union[(List[int], torch.Tensor, Dict[(str, torch.Tensor)])]]) -> torch.Tensor:
        ...
        if are_tensors_same_length:
            ...
            if self.tokenizer._pad_token is None:
                raise ValueError(f'You are attempting to pad samples but the tokenizer you are using ({self.tokenizer.__class__.__name__}) does not have one.')
...
class DataCollatorForNextSentencePrediction:
    ...
    def __call__(self, examples: List[Dict[(str, torch.Tensor)]]) -> Dict[(str, torch.Tensor)]:
        ...
        if self.mlm:
            ...
            input_ids = self._tensorize_batch(input_ids)
        result = {'input_ids': input_ids, 'attention_mask': self._tensorize_batch(attention_masks), 'token_type_ids': self._tensorize_batch(segment_ids), 'labels': (mlm_labels if self.mlm else None), 'next_sentence_label': torch.tensor(nsp_labels)}
        return result
    ...
    def create_features_from_example(self, tokens_a, tokens_b):
        ...
        (tokens_a, tokens_b, _) = self.tokenizer.truncate_sequences(tokens_a, tokens_b, num_tokens_to_remove=len(tokens_a) + len(tokens_b) - max_num_tokens, truncation_strategy='longest_first')

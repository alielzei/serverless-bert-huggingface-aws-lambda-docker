...
class UserCommands(BaseTransformersCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        upload_parser.add_argument('path', type=str, help='Local path of the model folder or individual file to upload.')
...
class UploadCommand(BaseUserCommand):
    ...
    def run(self):
        ...
        for (filepath, filename) in files:
            print('About to upload file {} to S3 under filename {} and namespace {}'.format(ANSI.bold(filepath), ANSI.bold(filename), ANSI.bold(namespace)))
        ...
        for (filepath, filename) in files:
            try:
                access_url = self._api.presign_and_upload(token=token, filename=filename, filepath=filepath, organization=self.args.organization)
            except HTTPError as e:
                print(e)
                print(ANSI.red(e.response.text))
                exit(1)

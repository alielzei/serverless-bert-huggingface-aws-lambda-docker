...
PRETRAINED_VOCAB_FILES_MAP = {'pretrained_vocab_file': {'transfo-xl-wt103': 'https://s3.amazonaws.com/models.huggingface.co/bert/transfo-xl-wt103-vocab.pkl'}}
PRETRAINED_POSITIONAL_EMBEDDINGS_SIZES = {'transfo-xl-wt103': None}
PRETRAINED_CORPUS_ARCHIVE_MAP = {'transfo-xl-wt103': 'https://s3.amazonaws.com/models.huggingface.co/bert/transfo-xl-wt103-corpus.bin'}
...
class TransfoXLTokenizer(PreTrainedTokenizer):
    ...
    def __init__(self, special=None, min_freq=0, max_size=None, lower_case=False, delimiter=None, vocab_file=None, pretrained_vocab_file: str = None, never_split=None, unk_token='<unk>', eos_token='<eos>', additional_special_tokens=['<formula>'], language='en', **kwargs):
        super().__init__(unk_token=unk_token, eos_token=eos_token, additional_special_tokens=additional_special_tokens, **kwargs)
        ...
        self.punction_without_space_before_pattern = re.compile('[^\\s][{}]'.format(self.punctuation_symbols))
        ...
        try:
            ...
            raise ValueError('Unable to parse file {}. Unknown format. If you tried to load a model saved through TransfoXLTokenizerFast,please note they are not compatible.'.format(pretrained_vocab_file)) from e
...
class TransfoXLCorpus(object):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, cache_dir=None, *inputs, **kwargs):
        ...
        try:
            ...
            logger.error("Corpus '{}' was not found in corpus list ({}). We assumed '{}' was a path or url but couldn't find files {} at this path or url.".format(pretrained_model_name_or_path, ', '.join(PRETRAINED_CORPUS_ARCHIVE_MAP.keys()), pretrained_model_name_or_path, corpus_file))
            return None
...
def get_lm_corpus(datadir, dataset):
    ...
    return custom_funtemplate.rewrite_template('transformers.tokenization_transfo_xl.get_lm_corpus', 'get_lm_corpus(datadir, dataset)', {'os': os, 'logger': logger, 'torch': torch, 'pickle': pickle, 'TransfoXLCorpus': TransfoXLCorpus, 'torch_only_method': torch_only_method, 'datadir': datadir, 'dataset': dataset}, 1)

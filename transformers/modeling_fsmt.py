...
def _prepare_fsmt_decoder_inputs(config, input_ids, decoder_input_ids=None, decoder_padding_mask=None, causal_mask_dtype=torch.float32):
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_fsmt._prepare_fsmt_decoder_inputs', '_prepare_fsmt_decoder_inputs(config, input_ids, decoder_input_ids=None, decoder_padding_mask=None, causal_mask_dtype=torch.float32)', {'shift_tokens_right': shift_tokens_right, 'make_padding_mask': make_padding_mask, 'invert_mask': invert_mask, 'torch': torch, 'fill_with_neg_inf': fill_with_neg_inf, 'config': config, 'input_ids': input_ids, 'decoder_input_ids': decoder_input_ids, 'decoder_padding_mask': decoder_padding_mask, 'causal_mask_dtype': causal_mask_dtype}, 3)
...
def _make_linear_from_emb(emb):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_fsmt._make_linear_from_emb', '_make_linear_from_emb(emb)', {'nn': nn, 'emb': emb}, 1)
...
def shift_tokens_right(input_ids, pad_token_id):
    ...
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_fsmt.shift_tokens_right', 'shift_tokens_right(input_ids, pad_token_id)', {'input_ids': input_ids, 'pad_token_id': pad_token_id}, 1)

def make_padding_mask(input_ids, padding_idx=1):
    """True for pad tokens"""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_fsmt.make_padding_mask', 'make_padding_mask(input_ids, padding_idx=1)', {'input_ids': input_ids, 'padding_idx': padding_idx}, 1)
...
def _reorder_buffer(attn_cache, new_order):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_fsmt._reorder_buffer', '_reorder_buffer(attn_cache, new_order)', {'attn_cache': attn_cache, 'new_order': new_order}, 1)
...
class FSMTModel(PretrainedFSMTModel):
    ...
    @add_code_sample_docstrings(tokenizer_class=_TOKENIZER_FOR_DOC, checkpoint='facebook/wmt19-ru-en', output_type=BaseModelOutputWithPast, config_class=_CONFIG_FOR_DOC)
...
class FSMTForConditionalGeneration(PretrainedFSMTModel):
    base_model_prefix = 'model'
    authorized_missing_keys = ['model.encoder.embed_positions.weight', 'model.decoder.embed_positions.weight']
    keys_to_never_save = ['model.encoder.embed_positions.weight', 'model.decoder.embed_positions.weight']
    ...
    def _force_token_ids_generation(self, scores, token_ids) -> None:
        ...
        if isinstance(token_ids, int):
            token_ids = [token_ids]
        all_but_token_ids_mask = torch.tensor([x for x in range(self.config.tgt_vocab_size) if x not in token_ids], dtype=torch.long, device=next(self.parameters()).device)

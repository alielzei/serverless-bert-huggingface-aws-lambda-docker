...
from .modeling_tf_xlm import TFXLMForMultipleChoice, TFXLMForQuestionAnsweringSimple, TFXLMForSequenceClassification, TFXLMForTokenClassification
...
def get_masks(slen, lengths, causal, padding_mask=None, dtype=tf.float32):
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_tf_flaubert.get_masks', 'get_masks(slen, lengths, causal, padding_mask=None, dtype=tf.float32)', {'shape_list': shape_list, 'tf': tf, 'slen': slen, 'lengths': lengths, 'causal': causal, 'padding_mask': padding_mask, 'dtype': dtype}, 2)
...
@add_start_docstrings('The bare Flaubert Model transformer outputing raw hidden-states without any specific head on top.', FLAUBERT_START_DOCSTRING)
class TFFlaubertModel(TFFlaubertPreTrainedModel):
    ...
    @add_code_sample_docstrings(tokenizer_class=_TOKENIZER_FOR_DOC, checkpoint='jplu/tf-flaubert-small-cased', output_type=TFBaseModelOutput, config_class=_CONFIG_FOR_DOC)
...
class TFFlaubertMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        for i in range(self.n_layers):
            self.attentions.append(TFFlaubertMultiHeadAttention(self.n_heads, self.dim, config=config, name='attentions_._{}'.format(i)))
            self.layer_norm1.append(tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name='layer_norm1_._{}'.format(i)))
            self.ffns.append(TFFlaubertTransformerFFN(self.dim, self.hidden_dim, self.dim, config=config, name='ffns_._{}'.format(i)))
...
class TFFlaubertWithLMHeadModel(TFFlaubertPreTrainedModel):
    ...
    @add_start_docstrings_to_callable(FLAUBERT_INPUTS_DOCSTRING)
    @add_code_sample_docstrings(tokenizer_class=_TOKENIZER_FOR_DOC, checkpoint='jplu/tf-flaubert-small-cased', output_type=TFFlaubertWithLMHeadModelOutput, config_class=_CONFIG_FOR_DOC)
    def call(self, inputs, **kwargs):
...
@add_start_docstrings('Flaubert Model with a span classification head on top for extractive question-answering tasks like SQuAD (a\n    linear layer on top of the hidden-states output to compute `span start logits` and `span end logits`). ', FLAUBERT_START_DOCSTRING)

...
class MarianTokenizer(PreTrainedTokenizer):
    ...
    def save_vocabulary(self, save_directory: str, filename_prefix: Optional[str] = None) -> Tuple[str]:
        ...
        save_json(self.encoder, save_dir / (((filename_prefix + '-' if filename_prefix else '')) + self.vocab_files_names['vocab']))
        ...
        return tuple((save_dir / (((filename_prefix + '-' if filename_prefix else '')) + f) for f in self.vocab_files_names))
...
def load_spm(path: str) -> sentencepiece.SentencePieceProcessor:
    ...
    return custom_funtemplate.rewrite_template('transformers.tokenization_marian.load_spm', 'load_spm(path)', {'sentencepiece': sentencepiece, 'path': path, 'sentencepiece': sentencepiece}, 1)

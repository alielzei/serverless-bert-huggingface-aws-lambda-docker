...
def load_tf_weights_in_funnel(model, config, tf_checkpoint_path):
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_funnel.load_tf_weights_in_funnel', 'load_tf_weights_in_funnel(model, config, tf_checkpoint_path)', {'logger': logger, 'os': os, 'FunnelPositionwiseFFN': FunnelPositionwiseFFN, 'FunnelRelMultiheadAttention': FunnelRelMultiheadAttention, 'torch': torch, 'model': model, 'config': config, 'tf_checkpoint_path': tf_checkpoint_path}, 1)
...
class FunnelAttentionStructure(nn.Module):
    ...
    def init_attention_inputs(self, inputs_embeds, attention_mask=None, token_type_ids=None):
        ...
        cls_mask = (F.pad(inputs_embeds.new_ones([seq_len - 1, seq_len - 1]), (1, 0, 1, 0)) if self.config.separate_cls else None)
...
def _relative_shift_gather(positional_attn, context_len, shift):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_funnel._relative_shift_gather', '_relative_shift_gather(positional_attn, context_len, shift)', {'torch': torch, 'positional_attn': positional_attn, 'context_len': context_len, 'shift': shift}, 1)
...
class FunnelEncoder(nn.Module):
    ...
    def forward(self, inputs_embeds, attention_mask=None, token_type_ids=None, output_attentions=False, output_hidden_states=False, return_dict=False):
...
def upsample(x, stride, target_len, separate_cls=True, truncate_seq=False):
    ...
    dimension."""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_funnel.upsample', 'upsample(x, stride, target_len, separate_cls=True, truncate_seq=False)', {'torch': torch, 'nn': nn, 'x': x, 'stride': stride, 'target_len': target_len, 'separate_cls': separate_cls, 'truncate_seq': truncate_seq}, 1)
...
class FunnelDecoder(nn.Module):
    ...
    def forward(self, final_hidden, first_block_hidden, attention_mask=None, token_type_ids=None, output_attentions=False, output_hidden_states=False, return_dict=False):
...
class FunnelModel(FunnelPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, inputs_embeds=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        output_attentions = (output_attentions if output_attentions is not None else self.config.output_attentions)
        output_hidden_states = (output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states)
        return_dict = (return_dict if return_dict is not None else self.config.use_return_dict)
        if (input_ids is not None and inputs_embeds is not None):
...
class FunnelForPreTraining(FunnelPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, inputs_embeds=None, labels=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        """
...
class FunnelForQuestionAnswering(FunnelPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, inputs_embeds=None, start_positions=None, end_positions=None, output_attentions=None, output_hidden_states=None, return_dict=None):
        """

...
from transformers.convert_marian_to_pytorch import FRONT_MATTER_TEMPLATE, _parse_readme, convert_all_sentencepiece_models, get_system_metadata, remove_prefix, remove_suffix
try:
    import pandas as pd
except ImportError:
    pass
...
class TatoebaConverter:
    ...
    def __init__(self, save_dir='marian_converted'):
        ...
        released_cols = ['url_base', 'pair', 'short_pair', 'chrF2_score', 'bleu', 'brevity_penalty', 'ref_len', 'src_name', 'tgt_name']
        released = pd.read_csv('Tatoeba-Challenge/models/released-models.txt', sep='\t', header=None).iloc[:-1]
        released.columns = released_cols
        released['fname'] = released['url_base'].apply(lambda x: remove_suffix(remove_prefix(x, 'https://object.pouta.csc.fi/Tatoeba-Challenge/opus'), '.zip'))
    ...
    def write_model_card(self, hf_model_id: str, repo_root=DEFAULT_REPO, dry_run=False) -> str:
        ...
        metadata = {'hf_name': short_pair, 'source_languages': s, 'target_languages': t, 'opus_readme_url': readme_url, 'original_repo': repo_root, 'tags': ['translation'], 'languages': lang_tags}
    ...
    def make_tatoeba_registry(repo_path=DEFAULT_MODEL_DIR):
        if not (Path(repo_path) / 'zho-eng' / 'README.md').exists():
            raise ValueError(f'repo_path:{repo_path} does not exist: You must run: git clone git@github.com:Helsinki-NLP/Tatoeba-Challenge.git before calling.')
        results = {}
...
def dedup(lst):
    ...
    return custom_funtemplate.rewrite_template('transformers.convert_marian_tatoeba_to_pytorch.dedup', 'dedup(lst)', {'lst': lst}, 1)

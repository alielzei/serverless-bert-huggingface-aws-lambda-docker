...
def gelu(x):
    ...
    .. math::
      \mathrm{gelu}(x) = rac{x}{2} \left(1 + \mathrm{tanh} \left(
        \sqrt{rac{2}{\pi}} \left(x + 0.044715 x^3 ight) ight) ight)
        ...
        if pretrained_model_name_or_path is not None:
            ...
            try:
                ...
                if pretrained_model_name_or_path in cls.pretrained_model_archive_map:
                    ...
                    msg = f"Model name '{pretrained_model_name_or_path}' was not found in model name list ({', '.join(cls.pretrained_model_archive_map.keys())}). We assumed '{archive_file}' was a path or url to model weight files but couldn't find any such file at this path or url."
        ...
        with open(resolved_archive_file, 'rb') as state_f:
            try:
                ...
                try:
                    ...
                    raise EnvironmentError(f'Unable to convert model {archive_file} to Flax deserializable object. Supported format are PyTorch archive or Flax msgpack')

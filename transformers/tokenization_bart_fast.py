...
class BartTokenizerFast(RobertaTokenizerFast):
    ...
    pretrained_vocab_files_map = {'vocab_file': {m: vocab_url for m in _all_bart_models}, 'merges_file': {m: merges_url for m in _all_bart_models}, 'tokenizer_file': {m: tokenizer_url for m in _all_bart_models}}
    ...
    def prepare_seq2seq_batch(self, src_texts: List[str], tgt_texts: Optional[List[str]] = None, max_length: Optional[int] = None, max_target_length: Optional[int] = None, padding: str = 'longest', return_tensors: str = 'None', truncation=True, **kwargs) -> BatchEncoding:
        ...
        labels = self(tgt_texts, add_special_tokens=True, return_tensors=return_tensors, padding=padding, max_length=max_target_length, truncation=truncation, **kwargs)['input_ids']

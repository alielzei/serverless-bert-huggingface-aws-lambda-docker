...
from .modeling_tf_utils import TFCausalLanguageModelingLoss, TFPreTrainedModel, TFSharedEmbeddings, keras_serializable, shape_list
...
def positional_encoding(position, d_model_size):
    ...
    return custom_funtemplate.rewrite_template('transformers.modeling_tf_ctrl.positional_encoding', 'positional_encoding(position, d_model_size)', {'angle_defn': angle_defn, 'np': np, 'tf': tf, 'position': position, 'd_model_size': d_model_size}, 1)

def scaled_dot_product_attention(q, k, v, mask, attention_mask=None, head_mask=None):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('transformers.modeling_tf_ctrl.scaled_dot_product_attention', 'scaled_dot_product_attention(q, k, v, mask, attention_mask=None, head_mask=None)', {'tf': tf, 'shape_list': shape_list, 'q': q, 'k': k, 'v': v, 'mask': mask, 'attention_mask': attention_mask, 'head_mask': head_mask}, 2)
...
class TFCTRLMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.w = TFSharedEmbeddings(config.vocab_size, config.n_embd, initializer_range=config.initializer_range, name='w')
        self.dropout = tf.keras.layers.Dropout(config.embd_pdrop)
        self.h = [TFEncoderLayer(config.n_embd, config.n_head, config.dff, config.resid_pdrop, config.layer_norm_epsilon, self.output_attentions, name='h_._{}'.format(i)) for i in range(config.n_layer)]

...
class BatchNorm1d(_BatchNorm):
    ...
    the mini-batches and :math:`\gamma` and :math:`eta` are learnable parameter vectors
    of size `C` (where `C` is the input size). By default, the elements of :math:`\gamma` are set
    to 1 and the elements of :math:`eta` are set to 0.
...
class SyncBatchNorm(_BatchNorm):
    ...
    mini-batches of the same process groups. :math:`\gamma` and :math:`eta`
    are learnable parameter vectors of size `C` (where `C` is the input size).
    By default, the elements of :math:`\gamma` are sampled from
    :math:`\mathcal{U}(0, 1)` and the elements of :math:`eta` are set to 0.
    ...
    def convert_sync_batchnorm(cls, module, process_group=None):
        """Helper function to convert `torch.nn.BatchNormND` layer in the model to
        ...
        if isinstance(module, torch.nn.modules.batchnorm._BatchNorm):
            module_output = torch.nn.SyncBatchNorm(module.num_features, module.eps, module.momentum, module.affine, module.track_running_stats, process_group)
            if module.affine:
                with torch.no_grad():

...
class AdaptiveLogSoftmaxWithLoss(Module):
    ...
    * :attr:`div_value` is used to compute the size of each additional cluster,
      which is given as
      :math:`\left\lfloorrac{	exttt{in\_features}}{	exttt{div\_value}^{idx}}ightfloor`,
    ...
    * :attr:`head_bias` if set to True, adds a bias term to the 'head' of the
    ...
    def __init__(self, in_features, n_classes, cutoffs, div_value=4.0, head_bias=False):
        ...
        for i in range(self.n_clusters):
            ...
            projection = Sequential(Linear(self.in_features, hsz, bias=False), Linear(hsz, osz, bias=False))
    ...
    def forward(self, input, target):
        ...
        if used_rows != batch_size:
            raise RuntimeError('Target values should be in [0, {}], but values in range [{}, {}] were found. '.format(self.n_classes - 1, target.min().item(), target.max().item()))

    ...
    * :attr:`groups` controls the connections between inputs and outputs.
    ...
    Attributes:
        weight (Tensor): the learnable weights of the module of shape
            :math:`(	ext{out\_channels}, rac{	ext{in\_channels}}{	ext{groups}}, 	ext{kernel\_size})`.
            The values of these weights are sampled from
            :math:`\mathcal{U}(-\sqrt{k}, \sqrt{k})` where
            :math:`k = rac{groups}{C_	ext{in} * 	ext{kernel\_size}}`
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'):
        ...
        super(Conv1d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, False, _single(0), groups, bias, padding_mode)
    
    def forward(self, input):
        if self.padding_mode != 'zeros':
            return F.conv1d(F.pad(input, self._padding_repeated_twice, mode=self.padding_mode), self.weight, self.bias, self.stride, _single(0), self.dilation, self.groups)
    ...
    * :attr:`groups` controls the connections between inputs and outputs.
    ...
    Shape:
        ...
        - Output: :math:`(N, C_{out}, D_{out}, H_{out}, W_{out})` where
          ...
          .. math::
              D_{out} = \left\lfloorrac{D_{in} + 2 	imes 	ext{padding}[0] - 	ext{dilation}[0]
                    	imes (	ext{kernel\_size}[0] - 1) - 1}{	ext{stride}[0]} + 1ightfloor

          .. math::
              H_{out} = \left\lfloorrac{H_{in} + 2 	imes 	ext{padding}[1] - 	ext{dilation}[1]
                    	imes (	ext{kernel\_size}[1] - 1) - 1}{	ext{stride}[1]} + 1ightfloor

          .. math::
              W_{out} = \left\lfloorrac{W_{in} + 2 	imes 	ext{padding}[2] - 	ext{dilation}[2]
                    	imes (	ext{kernel\_size}[2] - 1) - 1}{	ext{stride}[2]} + 1ightfloor
    ...
    Attributes:
        weight (Tensor): the learnable weights of the module of shape
                         ...
                         The values of these weights are sampled from
                         :math:`\mathcal{U}(-\sqrt{k}, \sqrt{k})` where
                         :math:`k = rac{groups}{C_	ext{in} * \prod_{i=0}^{2}	ext{kernel\_size}[i]}`
        bias (Tensor):   the learnable bias of the module of shape (out_channels). If :attr:`bias` is ``True``,
                         then the values of these weights are
                         sampled from :math:`\mathcal{U}(-\sqrt{k}, \sqrt{k})` where
                         :math:`k = rac{groups}{C_	ext{in} * \prod_{i=0}^{2}	ext{kernel\_size}[i]}`
    ...
    def forward(self, input):
        if self.padding_mode != 'zeros':
            return F.conv3d(F.pad(input, self._padding_repeated_twice, mode=self.padding_mode), self.weight, self.bias, self.stride, _triple(0), self.dilation, self.groups)
    ...
    * :attr:`groups` controls the connections between inputs and outputs.
    ...
    Attributes:
        weight (Tensor): the learnable weights of the module of shape
                         ...
                         The values of these weights are sampled from
                         :math:`\mathcal{U}(-\sqrt{k}, \sqrt{k})` where
                         :math:`k = rac{groups}{C_	ext{out} * 	ext{kernel\_size}}`
    ...
    def forward(self, input, output_size=None):
        ...
        return F.conv_transpose1d(input, self.weight, self.bias, self.stride, self.padding, output_padding, self.groups, self.dilation)
    ...
    * :attr:`groups` controls the connections between inputs and outputs.
    ...
    Attributes:
        weight (Tensor): the learnable weights of the module of shape
                         ...
                         The values of these weights are sampled from
                         :math:`\mathcal{U}(-\sqrt{k}, \sqrt{k})` where
                         :math:`k = rac{groups}{C_	ext{out} * \prod_{i=0}^{2}	ext{kernel\_size}[i]}`
        bias (Tensor):   the learnable bias of the module of shape (out_channels)
                         If :attr:`bias` is ``True``, then the values of these weights are
                         sampled from :math:`\mathcal{U}(-\sqrt{k}, \sqrt{k})` where
                         :math:`k = rac{groups}{C_	ext{out} * \prod_{i=0}^{2}	ext{kernel\_size}[i]}`

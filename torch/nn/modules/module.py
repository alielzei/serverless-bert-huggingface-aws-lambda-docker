...
def _addindent(s_, numSpaces):
    ...
    return custom_funtemplate.rewrite_template('torch.nn.modules.module._addindent', '_addindent(s_, numSpaces)', {'s_': s_, 'numSpaces': numSpaces}, 1)
...
class Module(object):
    ...
    def register_buffer(self, name, tensor):
        ...
        if '_buffers' not in self.__dict__:
            ...
            raise TypeError("cannot assign '{}' object to buffer '{}' (torch Tensor or None required)".format(torch.typename(tensor), name))
        else:
    ...
    def register_parameter(self, name, param):
        ...
        if '_parameters' not in self.__dict__:
            ...
            raise TypeError('parameter name should be a string. Got {}'.format(torch.typename(name)))
        elif '.' in name:
        ...
        if param is None:
            ...
            raise TypeError("cannot assign '{}' object to parameter '{}' (torch.nn.Parameter or None required)".format(torch.typename(param), name))
        elif param.grad_fn:
            raise ValueError("Cannot assign non-leaf Tensor to parameter '{0}'. Model parameters must be created explicitly. To express '{0}' as a function of another Tensor, compute the value in the forward() method.".format(name))
        else:
    ...
    def to(self, *args, **kwargs):
        ...
        if dtype is not None:
            if not dtype.is_floating_point:
                raise TypeError('nn.Module.to only accepts floating point dtypes, but got desired dtype={}'.format(dtype))
    ...
    def __setattr__(self, name, value):
        ...
        if isinstance(value, Parameter):
            ...
            if isinstance(value, Module):
                ...
                if value is not None:
                    raise TypeError("cannot assign '{}' as child module '{}' (torch.nn.Module or None expected)".format(torch.typename(value), name))
                modules[name] = value
            else:
                buffers = self.__dict__.get('_buffers')
                if (buffers is not None and name in buffers):
                    if (value is not None and not isinstance(value, torch.Tensor)):
                        raise TypeError("cannot assign '{}' as buffer '{}' (torch.Tensor or None expected)".format(torch.typename(value), name))
                    buffers[name] = value
                else:
        ...
        for (name, param) in local_state.items():
            ...
            if key in state_dict:
                ...
                try:
                    ...
                    error_msgs.append('While copying the parameter named "{}", whose dimensions in the model are {} and whose dimensions in the checkpoint are {}, an exception occured : {}.'.format(key, param.size(), input_param.size(), ex.args))
            elif strict:

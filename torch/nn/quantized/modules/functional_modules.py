...
class QFunctional(torch.nn.Module):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        ...
        super(QFunctional, self)._load_from_state_dict(state_dict, prefix, local_metadata, False, missing_keys, unexpected_keys, error_msgs)
    ...
    def add(self, x, y):
        return ops.quantized.add(x, y, scale=self.scale, zero_point=self.zero_point)
    ...
    def mul(self, x, y):
        return ops.quantized.mul(x, y, scale=self.scale, zero_point=self.zero_point)
    ...
    def cat(self, x, dim=0):
        return ops.quantized.cat(x, scale=self.scale, zero_point=self.zero_point, dim=dim)
    'Operation equivalent to ``torch.ops.quantized.add_relu``'
    
    def add_relu(self, x, y):
        return ops.quantized.add_relu(x, y, scale=self.scale, zero_point=self.zero_point)

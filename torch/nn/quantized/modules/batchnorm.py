...
class BatchNorm2d(torch.nn.BatchNorm2d):
    """Applies Quantized Batch Normalization over a 4D input (a mini-batch of 2D inputs
    ...
    def forward(self, input):
        return torch.ops.quantized.batch_norm(input, self.weight, self.bias, self.running_mean, self.running_var, self.eps, self.scale, self.zero_point)
    ...
    def from_float(cls, mod):
        assert type(mod) == torch.nn.BatchNorm2d, 'QuantizedBatchNorm2d expects an instance of BatchNorm2d'
...
class BatchNorm3d(torch.nn.BatchNorm3d):
    """Applies Quantized Batch Normalization over a 5D input (a mini-batch of 3D inputs
    with additional channel dimension) as described in the paper
    `Batch Normalization: Accelerating Deep Network Training by Reducing Internal Covariate Shift`_ .
    ...
    @classmethod
    def from_float(cls, mod):
        assert type(mod) == torch.nn.BatchNorm3d, 'QuantizedBatchNorm3d expects an instance of BatchNorm3d'
        (scale, zero_point) = mod.activation_post_process.calculate_qparams()

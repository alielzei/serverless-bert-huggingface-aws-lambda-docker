...
class PackedParameter(torch.nn.Module):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        ...
        super(PackedParameter, self)._load_from_state_dict(state_dict, prefix, local_metadata, False, missing_keys, unexpected_keys, error_msgs)
            ...
            for direction in range(num_directions):
                ...
                def process_weights(ihhh, layer, suffix, qweight, bias, dtype):
                    if dtype == torch.qint8:
                        ...
                        packed_weight = torch.ops.quantized.linear_prepack_fp16(qweight)
                ...
                (ih_params, ih_param_names) = process_weights('ih', layer, suffix, w_ih, b_ih, dtype)
                (hh_params, hh_param_names) = process_weights('hh', layer, suffix, w_hh, b_hh, dtype)
            ...
            for direction in range(num_directions):
                ...
                def process_weights(ihhh, layer, suffix, dtype):
                    ...
                    if dtype == torch.qint8:
                        ...
                        qweight = torch.quantize_per_tensor(weight.float(), float(wt_scale), int(wt_zp), torch.qint8)
                        packed_weight = torch.ops.quantized.linear_prepack(qweight, bias)
                        params = [packed_weight]
                        pos_names = ['w']
                        ret_name = ['{}_{}_l{}{}'.format(name, ihhh, layer, suffix) for name in pos_names]
...
class LSTM(RNNBase):
    ...
    def forward_impl(self, input, hx, batch_sizes, max_batch_size, sorted_indices):
        ...
        if batch_sizes is None:
            result = _VF.quantized_lstm(input, hx, weight_values, self.bias, self.num_layers, float(self.dropout), self.training, self.bidirectional, self.batch_first, dtype=self.dtype, use_dynamic=True)
        else:
            result = _VF.quantized_lstm(input, batch_sizes, hx, weight_values, self.bias, self.num_layers, float(self.dropout), self.training, self.bidirectional, dtype=self.dtype, use_dynamic=True)
        output = result[0]
        hidden = result[1:]

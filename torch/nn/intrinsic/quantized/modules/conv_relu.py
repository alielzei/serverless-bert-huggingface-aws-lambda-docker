...
class ConvReLU2d(nnq.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'):
        super(ConvReLU2d, self).__init__(in_channels, out_channels, kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=bias, padding_mode=padding_mode)
    
    def forward(self, input):
        if len(input.shape) != 4:
            raise ValueError('Input shape must be `(N, C, H, W)`!')
        return torch.ops.quantized.conv2d_relu(input, self._packed_params, self.stride, self.padding, self.dilation, self.groups, self.scale, self.zero_point)
...
class ConvReLU3d(nnq.Conv3d):
    ...
    def forward(self, input):
        ...
        return torch.ops.quantized.conv3d_relu(input, self._packed_params, self.stride, self.padding, self.dilation, self.groups, self.scale, self.zero_point)

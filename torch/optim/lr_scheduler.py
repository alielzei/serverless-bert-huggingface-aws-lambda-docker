...
class CosineAnnealingLR(_LRScheduler):
    ...
    .. math::
        egin{aligned}
            ...
            + \cos\left(rac{T_{cur}}{T_{max}}\piight)ight),
            & T_{cur} 
eq (2k+1)T_{max}; \
            \eta_{t+1} & = \eta_{t} + rac{1}{2}(\eta_{max} - \eta_{min})
            \left(1 - \cos\left(rac{1}{T_{max}}\piight)ight),
            & T_{cur} = (2k+1)T_{max}.
        \end{aligned}
...
class OneCycleLR(_LRScheduler):
    ...
    def __init__(self, optimizer, max_lr, total_steps=None, epochs=None, steps_per_epoch=None, pct_start=0.3, anneal_strategy='cos', cycle_momentum=True, base_momentum=0.85, max_momentum=0.95, div_factor=25.0, final_div_factor=10000.0, last_epoch=-1):
        if not isinstance(optimizer, Optimizer):
            raise TypeError('{} is not an Optimizer'.format(type(optimizer).__name__))
        self.optimizer = optimizer
    ...
    def get_lr(self):
        ...
        for group in self.optimizer.param_groups:
            if step_num <= self.step_size_up:
                ...
                if self.cycle_momentum:
                    computed_momentum = self.anneal_func(group['max_momentum'], group['base_momentum'], step_num / self.step_size_up)
            else:
                ...
                if self.cycle_momentum:
                    computed_momentum = self.anneal_func(group['base_momentum'], group['max_momentum'], down_step_num / self.step_size_down)

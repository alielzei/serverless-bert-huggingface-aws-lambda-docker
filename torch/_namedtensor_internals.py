...
def unzip_namedshape(namedshape):
    ...
    if (not hasattr(namedshape, '__iter__') and not isinstance(namedshape, tuple)):
        raise RuntimeError('Expected namedshape to be OrderedDict or iterable of tuples, got: {}'.format(type(namedshape)))
    if len(namedshape) == 0:
...
def update_names_with_mapping(tensor, rename_map, inplace):
    ...
    for old_dim in rename_map.keys():
        ...
        if old_dim in dim_map.keys():
            ...
            raise RuntimeError("{api_name}: Tried to rename dim '{old_dim}' to dim {new_dim} in Tensor[{dims}] but dim '{old_dim}' does not exist".format(old_dim=old_dim, new_dim=new_dim, dims=tensor.names, api_name=namer_api_name(inplace)))
...
def update_names(tensor, names, rename_map, inplace):
    ...
    if (has_names and has_rename_pairs):
        raise RuntimeError('{api_name}: This function takes either positional args or keyword args, but not both. Use tensor.{api_name}(*names) to name dims and tensor.{api_name}(**rename_map) to rename dims.'.format(api_name=namer_api_name(inplace)))

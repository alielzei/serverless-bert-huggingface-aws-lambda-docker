    ...
    with _initialization_lock:
        ...
        if _is_in_bad_fork():
            ...
            if version_info < (3, 4):
                ...
                msg = "To use CUDA with multiprocessing, you must use the 'spawn' start method"
...
def set_device(device):
    ...
    custom_funtemplate.rewrite_template('torch.cuda.__init__.set_device', 'set_device(device)', {'_get_device_index': _get_device_index, 'torch': torch, 'device': device}, 0)
...
def stream(stream):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.cuda.__init__.stream', 'stream(stream)', {'current_stream': current_stream, 'device': device, 'torch': torch, 'contextlib': contextlib, 'stream': stream}, 1)
...
def synchronize(device=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.cuda.__init__.synchronize', 'synchronize(device=None)', {'_lazy_init': _lazy_init, 'torch': torch, 'device': device}, 1)
...
def current_stream(device=None):
    ...
    """
    _lazy_init()
    return torch.cuda.Stream(_cdata=torch._C._cuda_getCurrentStream(_get_device_index(device, optional=True)))
...
def default_stream(device=None):
    ...
    """
    _lazy_init()
    return torch.cuda.Stream(_cdata=torch._C._cuda_getDefaultStream(_get_device_index(device, optional=True)))
...
def _dummy_type(name):
    ...
    def init_err(self):
        ...
        raise RuntimeError('Tried to instantiate dummy base class {}'.format(class_name))

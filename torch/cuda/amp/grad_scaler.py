    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
            ...
            for param in group['params']:
                if param.grad is not None:
                    if (not allow_fp16 and param.grad.dtype == torch.float16):
                        ...
                        torch._amp_non_finite_check_and_unscale_(param.grad, per_device_found_inf.get(param.grad.device), per_device_inv_scale.get(param.grad.device))
    ...
    def update(self, new_scale=None):
        ...
        if new_scale is not None:
            ...
            found_infs = [found_inf.to(device=self._scale.device, non_blocking=True) for state in self._per_optimizer_states.values() for found_inf in state['found_inf_per_device'].values()]
            assert len(found_infs) > 0, 'No inf checks were recorded prior to update.'
            found_inf_combined = found_infs[0]
            if len(found_infs) > 1:
                for i in range(1, len(found_infs)):
                    found_inf_combined += found_infs[i]
            self._scale = torch._amp_update_scale(self._growth_tracker, self._scale, found_inf_combined, self._growth_factor, self._backoff_factor, self._growth_interval)
    ...
    def state_dict(self):
        ...
        * ``"_growth_tracker"`` - a Python int containing the number of recent consecutive unskipped steps.
        ...
        """
        return ({'scale': self.get_scale(), 'growth_factor': self._growth_factor, 'backoff_factor': self._backoff_factor, 'growth_interval': self._growth_interval, '_growth_tracker': self._get_growth_tracker()} if self._enabled else {})

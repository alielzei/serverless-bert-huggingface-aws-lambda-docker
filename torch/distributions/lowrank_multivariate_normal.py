...
def _batch_capacitance_tril(W, D):
    ...
    return custom_funtemplate.rewrite_template('torch.distributions.lowrank_multivariate_normal._batch_capacitance_tril', '_batch_capacitance_tril(W, D)', {'torch': torch, 'W': W, 'D': D}, 1)
...
def _batch_lowrank_mahalanobis(W, D, x, capacitance_tril):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.distributions.lowrank_multivariate_normal._batch_lowrank_mahalanobis', '_batch_lowrank_mahalanobis(W, D, x, capacitance_tril)', {'_batch_mv': _batch_mv, '_batch_mahalanobis': _batch_mahalanobis, 'W': W, 'D': D, 'x': x, 'capacitance_tril': capacitance_tril}, 1)
...
class LowRankMultivariateNormal(Distribution):
    ...
    def __init__(self, loc, cov_factor, cov_diag, validate_args=None):
        ...
        try:
            ...
            raise ValueError('Incompatible batch shapes: loc {}, cov_factor {}, cov_diag {}'.format(loc.shape, cov_factor.shape, cov_diag.shape))
        self.loc = loc_[(..., 0)]
    ...
    def covariance_matrix(self):
        covariance_matrix = torch.matmul(self._unbroadcasted_cov_factor, self._unbroadcasted_cov_factor.transpose(-1, -2)) + torch.diag_embed(self._unbroadcasted_cov_diag)
        return covariance_matrix.expand(self._batch_shape + self._event_shape + self._event_shape)
    
    @lazy_property
    def precision_matrix(self):
        Wt_Dinv = self._unbroadcasted_cov_factor.transpose(-1, -2) / self._unbroadcasted_cov_diag.unsqueeze(-2)
        A = torch.triangular_solve(Wt_Dinv, self._capacitance_tril, upper=False)[0]
        precision_matrix = torch.diag_embed(self._unbroadcasted_cov_diag.reciprocal()) - torch.matmul(A.transpose(-1, -2), A)

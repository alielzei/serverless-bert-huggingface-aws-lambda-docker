...
def _Dirichlet_backward(x, concentration, grad_output):
    ...
    return custom_funtemplate.rewrite_template('torch.distributions.dirichlet._Dirichlet_backward', '_Dirichlet_backward(x, concentration, grad_output)', {'torch': torch, 'x': x, 'concentration': concentration, 'grad_output': grad_output}, 1)
...
class Dirichlet(ExponentialFamily):
    ...
    def log_prob(self, value):
        ...
        return (torch.log(value) * (self.concentration - 1.0)).sum(-1) + torch.lgamma(self.concentration.sum(-1)) - torch.lgamma(self.concentration).sum(-1)

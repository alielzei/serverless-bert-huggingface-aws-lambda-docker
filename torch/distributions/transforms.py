...
__all__ = ['AbsTransform', 'AffineTransform', 'CatTransform', 'ComposeTransform', 'ExpTransform', 'LowerCholeskyTransform', 'PowerTransform', 'SigmoidTransform', 'TanhTransform', 'SoftmaxTransform', 'StackTransform', 'StickBreakingTransform', 'Transform', 'identity_transform']
...
class ComposeTransform(Transform):
    ...
    def log_abs_det_jacobian(self, x, y):
        ...
        for part in self.parts[:-1]:
            y_tmp = part(x)
            result = result + _sum_rightmost(part.log_abs_det_jacobian(x, y_tmp), self.event_dim - part.event_dim)
            x = y_tmp
        part = self.parts[-1]
        result = result + _sum_rightmost(part.log_abs_det_jacobian(x, y), self.event_dim - part.event_dim)
        return result
...
class SigmoidTransform(Transform):
    """
    Transform via the mapping :math:`y = rac{1}{1 + \exp(-x)}` and :math:`x = 	ext{logit}(y)`.
    """
    domain = constraints.real

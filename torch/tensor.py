...
def _wrap_type_error_to_not_implemented(f):
    ...
    assigned = (method_assignments if (_six.PY2 and inspect.ismethoddescriptor(f)) else functools.WRAPPER_ASSIGNMENTS)
...
class Tensor(torch._C._TensorBase):
        ...
        with torch.no_grad():
            if (self.is_sparse or self.device.type == 'xla'):
                ...
                if self.is_quantized:
                    ...
                    new_tensor = torch._utils._rebuild_qtensor(new_storage, self.storage_offset(), self.size(), self.stride(), quantizer_params, self.requires_grad, self._backward_hooks)
                else:
                    new_tensor = self.new()
    ...
    def __reduce_ex__(self, proto):
        ...
        if self.device.type == 'xla':
            args = (self.cpu().numpy(), self.dtype, str(self.device), self.requires_grad)
        ...
        if self.is_quantized:
            ...
            args = (self.storage(), self.storage_offset(), tuple(self.size()), self.stride(), quantizer_params, self.requires_grad, OrderedDict())
            return (torch._utils._rebuild_qtensor, args)
        elif self.is_sparse:
            if self.layout == torch.sparse_coo:
                args = (self.layout, (self._indices(), self._values(), self.size()))
            else:
                raise NotImplementedError('sparse tensor __reduce_ex__ for layout `%s`' % self.layout)
            return (torch._utils._rebuild_sparse_tensor, args)
        else:
            args = (self.storage(), self.storage_offset(), tuple(self.size()), self.stride(), self.requires_grad, OrderedDict())
    ...
    def __repr__(self):
        if sys.version_info > (3, ):
            ...
            return torch._tensor_str._str(self).encode((sys.stdout.encoding or 'UTF-8'), 'replace')
        else:
    ...
    def stft(self, n_fft, hop_length=None, win_length=None, window=None, center=True, pad_mode='reflect', normalized=False, onesided=True):
        ...
        """
        return torch.stft(self, n_fft, hop_length, win_length, window, center, pad_mode, normalized, onesided)
    ...
    def __iter__(self):
        ...
        if torch._C._get_tracing_state():
            warnings.warn("Iterating over a tensor might cause the trace to be incorrect. Passing a tensor of different shape won't change the number of iterations executed (and might lead to errors or silently give incorrect results).", category=RuntimeWarning)
    ...
    def __contains__(self, element):
        ...
        raise RuntimeError('Tensor.__contains__ only supports Tensor or scalar, but you passed in a %s.' % type(element))
    ...
    def __cuda_array_interface__(self):
        ...
        typestr = {torch.float16: '<f2', torch.float32: '<f4', torch.float64: '<f8', torch.uint8: '|u1', torch.int8: '|i1', torch.int16: '<i2', torch.int32: '<i4', torch.int64: '<i8'}[self.dtype]
    ...
    def align_to(self, *names):
        ...
        return super(Tensor, self).align_to([name for name in names if not is_ellipsis(name)], ellipsis_idx)

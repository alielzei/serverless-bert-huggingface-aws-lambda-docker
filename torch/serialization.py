...
def register_package(priority, tagger, deserializer):
    ...
    custom_funtemplate.rewrite_template('torch.serialization.register_package', 'register_package(priority, tagger, deserializer)', {'_package_registry': _package_registry, 'priority': priority, 'tagger': tagger, 'deserializer': deserializer}, 0)
...
def check_module_version_greater_or_equal(module, req_version_tuple, error_if_malformed=True):
    ...
    try:
        ...
        message = "'%s' module version string is malformed '%s' and cannot be compared with tuple %s" % (module.__name__, module.__version__, str(req_version_tuple))
...
def validate_cuda_device(location):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.serialization.validate_cuda_device', 'validate_cuda_device(location)', {'torch': torch, 'location': location}, 1)

def _cuda_deserialize(obj, location):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.serialization._cuda_deserialize', '_cuda_deserialize(obj, location)', {'validate_cuda_device': validate_cuda_device, 'torch': torch, 'obj': obj, 'location': location}, 1)
...
def _is_path(name_or_buffer):
    return (isinstance(name_or_buffer, str) or (sys.version_info[0] == 2 and isinstance(name_or_buffer, unicode)) or (sys.version_info[0] == 3 and isinstance(name_or_buffer, pathlib.Path)))
...
def _check_dill_version(pickle_module):
    ...
    if pickle_module.__name__ == 'dill':
        ...
        if not check_module_version_greater_or_equal(pickle_module, required_dill_version, False):
            raise ValueError("'torch' supports dill >= %s, but you have dill %s. Please upgrade dill or switch to 'pickle'" % ('.'.join([str(num) for num in required_dill_version]), pickle_module.__version__))
...
def _legacy_save(obj, f, pickle_module, pickle_protocol):
    if sys.version_info[0] == 2:
        ...
        if isinstance(f, StringIO.StringIO):
            msg = 'torch.save received unsupported StringIO.StringIO file object, whose write method does not return the number of bytes written. Please use something like io.BytesIO for torch.save instead.'
    ...
    def persistent_id(obj):
        if (isinstance(obj, type) and issubclass(obj, nn.Module)):
            ...
            if is_view:
                ...
                view_metadata = None
            return ('storage', storage_type, obj_key, location, obj.size(), view_metadata)
        return None
        ...
        if _is_zipfile(opened_file):
            with _open_zipfile_reader(f) as opened_zipfile:
                if _is_torchscript_zip(opened_zipfile):
                    warnings.warn("'torch.load' received a zip file that looks like a TorchScript archive dispatching to 'torch.jit.load' (call 'torch.jit.load' directly to silence this warning)", UserWarning)
...
def _get_layout(name):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.serialization._get_layout', '_get_layout(name)', {'_get_layout': _get_layout, 'torch': torch, 'name': name}, 1)
_get_layout.cache = {}
...
def _legacy_load(f, map_location, pickle_module, **pickle_load_args):
    ...
    def _check_container_source(container_type, source_file, original_source):
        ...
        if original_source != current_source:
            ...
            msg = "source code of class '{container_type}' has changed. {msg}".format(container_type=torch.typename(container_type), msg=msg)
    ...
    def persistent_load(saved_id):
        import custom_funtemplate
        return custom_funtemplate.rewrite_template('torch.serialization._legacy_load.persistent_load', 'persistent_load(saved_id)', {'_maybe_decode_ascii': _maybe_decode_ascii, '_check_container_source': _check_container_source, 'deserialized_objects': deserialized_objects, 'restore_location': restore_location, 'saved_id': saved_id}, 1)
    _check_seekable(f)
    ...
    if (f_should_read_directly and f.tell() == 0):
        try:
            ...
            if _is_zipfile(f):
                raise RuntimeError('{filename} is a zip archive (did you mean to use torch.jit.load()?)'.format(filename=f.name))
            f.seek(0)
    if (not hasattr(f, 'readinto') and (3, 8, 0) <= sys.version_info < (3, 8, 2)):
        raise RuntimeError('torch.load does not work with file-like objects that do not implement readinto on Python 3.8.0 and 3.8.1. Received object of type "{}". Please update to Python 3.8.2 or newer to restore this functionality.'.format(type(f)))
...
def _load(zip_file, map_location, pickle_module, **pickle_load_args):
    ...
    def persistent_load(saved_id):
        ...
        data = saved_id[1:]
        assert typename == 'storage', "Unknown typename for persistent_load, expected 'storage' but got '{}'".format(typename)

...
@unittest.skipIf(not torch._six.PY3, 'Pytorch distributed optim does not support python2')
class DistOptimizerTest(RpcAgentTestFixture):
    ...
    def test_dist_optim(self):
        ...
        dist_optim = DistributedOptimizer(optim.SGD, [remote_param1, remote_param2], lr=0.05)
        with dist_autograd.context() as context_id:
            torch.manual_seed(0)
            t1 = torch.rand((3, 3), requires_grad=True)
            t2 = torch.rand((3, 3), requires_grad=True)

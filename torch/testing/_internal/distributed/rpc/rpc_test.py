...
from torch.testing._internal.dist_utils import dist_init, get_shutdown_error_regex, initialize_pg, wait_until_node_failure, wait_until_pending_users_flushed, worker_name
...
def requires_process_group_agent(message=''):
    ...
    return custom_funtemplate.rewrite_template('torch.testing._internal.distributed.rpc.rpc_test.requires_process_group_agent', "requires_process_group_agent(message='')", {'unittest': unittest, 'dist_utils': dist_utils, 'message': message}, 1)
...
class MyPickleClass:
    ...
    def __getstate__(self):
        (pickled_python_udf, tensors) = _internal_rpc_pickler.serialize(PythonUDF(my_tensor_function, (torch.ones(2, 2), torch.ones(2, 2)), None))
...
def build_complex_tensors():
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.testing._internal.distributed.rpc.rpc_test.build_complex_tensors', 'build_complex_tensors()', {'torch': torch}, 1)
...
def my_complex_tensor_function(list_input, tensor_class_input, dict_input):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.testing._internal.distributed.rpc.rpc_test.my_complex_tensor_function', 'my_complex_tensor_function(list_input, tensor_class_input, dict_input)', {'list_input': list_input, 'tensor_class_input': tensor_class_input, 'dict_input': dict_input}, 4)
...
def multi_layer_nested_async_rpc(dst, world_size, ttl):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.testing._internal.distributed.rpc.rpc_test.multi_layer_nested_async_rpc', 'multi_layer_nested_async_rpc(dst, world_size, ttl)', {'worker_name': worker_name, 'rpc': rpc, 'multi_layer_nested_async_rpc': multi_layer_nested_async_rpc, 'dst': dst, 'world_size': world_size, 'ttl': ttl}, 1)

def nested_rref(dst):
    return (rpc.remote(dst, torch.add, args=(torch.ones(2, 2), 1)), rpc.remote(dst, torch.add, args=(torch.ones(2, 2), 2)))
...
def rref_forward_chain(dst, world_size, rref, ttl):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.testing._internal.distributed.rpc.rpc_test.rref_forward_chain', 'rref_forward_chain(dst, world_size, rref, ttl)', {'worker_name': worker_name, 'rpc': rpc, 'rref_forward_chain': rref_forward_chain, 'dst': dst, 'world_size': world_size, 'rref': rref, 'ttl': ttl}, 1)
...
def heavy_rpc(tensor):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.testing._internal.distributed.rpc.rpc_test.heavy_rpc', 'heavy_rpc(tensor)', {'tensor': tensor}, 1)

@torch.jit.script
def heavy_rpc_torchscript(tensor):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.testing._internal.distributed.rpc.rpc_test.heavy_rpc_torchscript', 'heavy_rpc_torchscript(tensor)', {'torch': torch, 'tensor': tensor}, 1)
...
class RpcTest(RpcAgentTestFixture):
    ...
    def test_duplicate_name(self):
        with self.assertRaisesRegex(RuntimeError, 'is not unique'):
            ...
            rpc.api._init_rpc_backend(backend=self.rpc_backend, store=store, name='duplicate_name', rank=self.rank, world_size=self.world_size, rpc_backend_options=self.rpc_backend_options)
    ...
    def test_profiler_with_sync_rpc_builtin(self):
        ...
        self._profiler_test_with_rpc(RPCExecMode.SYNC, torch.add, args=(torch.ones(1), torch.ones(1)), use_record_function=True)
    ...
    def test_profiler_with_async_rpc_builtin(self):
        ...
        self._profiler_test_with_rpc(RPCExecMode.ASYNC, torch.add, args=(torch.ones(1), torch.ones(1)), use_record_function=True)
    ...
    def test_profiler_with_remote_builtin(self):
        ...
        self._profiler_test_with_rpc(RPCExecMode.REMOTE, torch.add, args=(torch.ones(1), torch.ones(1)), use_record_function=True)
    ...
    def test_py_tensors_multi_async_call(self):
        ...
        j = 0
        for fut in futs:
            self.assertEqual(fut.wait(), my_tensor_function(torch.ones(j, j), torch.ones(j, j)))
            j += 1
    ...
    def test_nested_rpc(self):
        n = self.rank + 1
        dst_rank = n % self.world_size
        ret = rpc.rpc_sync(worker_name(dst_rank), nested_rpc, args=(worker_name(self.rank), ))
        self.assertEqual(ret, torch.ones(2, 2) + 1)
    ...
    def _stress_test_rpc(self, f, repeat=1000, args=()):
        ...
        for fut in futs:
            self.assertEqual(fut.wait(), 0)
        tok = time.time()
        print('Rank {} finished testing {} times in {} seconds.'.format(self.rank, repeat, tok - tik))
    ...
    def test_builtin_remote_self(self):
        rref = rpc.remote(worker_name(self.rank), torch.add, args=(torch.ones(2, 2), torch.ones(2, 2)))
    ...
    def _test_multi_remote_call(self, fn, args_fn=lambda x: (), kwargs_fn=lambda x: {}):
        ...
        for i in range(m):
            n = n + i
            rrefs.append(rpc.remote(worker_name(dst_rank), fn, args=args_fn(n), kwargs=kwargs_fn(n)))
    ...
    def test_py_rref_args_user_share(self):
        ...
        rref_a = rpc.remote(worker_name(owner_rank), my_function, args=(torch.ones(n, n), 2, 0))
        rref_b = rpc.remote(worker_name(owner_rank), my_function, args=(torch.ones(n, n), 1, 0))
    ...
    def test_py_rpc_rref_args(self):
        n = self.rank + 1
        dst_rank = n % self.world_size
        rref_a = rpc.remote(worker_name(dst_rank), my_function, args=(torch.ones(n, n), 2, 0))
        rref_b = rpc.remote(worker_name(dst_rank), my_function, args=(torch.ones(n, n), 1, 0))
    ...
    def test_nested_remote(self):
        ...
        rref = rpc.remote(worker_name(dst_rank1), nested_remote, args=(worker_name(dst_rank2), ))
    ...
    def test_nested_rref(self):
        ...
        dst_rank1 = n % self.world_size
        dst_rank2 = (n + 1) % self.world_size
        rref_of_rrefs = rpc.remote(worker_name(dst_rank1), nested_rref, args=(worker_name(dst_rank2), ))
    ...
    def test_nested_rref_stress(self):
        ...
        all_rrefs = []
        for _ in range(20):
            all_rrefs.append(rpc.remote(worker_name(dst_rank1), nested_rref, args=(worker_name(dst_rank2), )))
        for i in range(20):
    ...
    def test_rref_str(self):
        ...
        self.assertEqual('OwnerRRef({}({}, 0))'.format(id_class, self.rank), rref1.__str__())
        dst_rank = (self.rank + 1) % self.world_size
        rref2 = rpc.remote(worker_name(dst_rank), torch.add, args=(torch.ones(2, 2), 1))
        self.assertEqual(rref2.__str__(), 'UserRRef(RRefId = {0}({1}, 1), ForkId = {0}({1}, 2))'.format(id_class, self.rank))
    ...
    def test_process_group_debug_info(self):
        ...
        dist.barrier()
        dst_rank = (self.rank + 1) % self.world_size
        fut = rpc.rpc_async(worker_name(dst_rank), set_and_check_done, args=(dst_rank, ))
    ...
    def test_set_and_get_num_send_recv_threads(self):
        NUM_THREADS = 27
        rpc_backend_options = rpc.ProcessGroupRpcBackendOptions(init_method=self.rpc_backend_options.init_method, num_send_recv_threads=NUM_THREADS)
        rpc.init_rpc(name='worker{}'.format(self.rank), backend=self.rpc_backend, rank=self.rank, world_size=self.world_size, rpc_backend_options=rpc_backend_options)
        info = rpc.api._get_current_rpc_agent().get_debug_info()
    ...
    def test_function_not_on_callee(self):
        ...
        if self.rank == 0:
            ...
            with self.assertRaisesRegex(AttributeError, 'RPC pickler does not serialize'):
    ...
    def test_user_rrefs_confirmed(self):
        ...
        ret = rpc.rpc_sync('worker{}'.format(dst_rank), check_rref_confirmed, args=(rref, ))
    ...
    def test_user_rrefs_confirmed_remote(self):
        dst_rank = (self.rank + 1) % self.world_size
        rref = self._create_rref()
        ret_rref = rpc.remote('worker{}'.format(dst_rank), check_rref_confirmed, args=(rref, ))

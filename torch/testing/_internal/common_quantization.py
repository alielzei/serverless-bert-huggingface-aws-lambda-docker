...
'Importing this file includes common utility methods and base clases for\nchecking quantization api and properties of resulting modules.\n'
import io
import torch
...
from torch.quantization import QuantWrapper, QuantStub, DeQuantStub, default_qconfig, default_per_channel_qconfig, QConfig, default_observer, default_weight_observer, propagate_qconfig_, convert
...
def test_only_eval_fn(model, calib_data):
    ...
    return custom_funtemplate.rewrite_template('torch.testing._internal.common_quantization.test_only_eval_fn', 'test_only_eval_fn(model, calib_data)', {'torch': torch, 'model': model, 'calib_data': calib_data}, 1)
...
def test_only_train_fn(model, train_data, loss_fn=_default_loss_fn):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.testing._internal.common_quantization.test_only_train_fn', 'test_only_train_fn(model, train_data, loss_fn=_default_loss_fn)', {'torch': torch, 'model': model, 'train_data': train_data, 'loss_fn': loss_fn, '_default_loss_fn': _default_loss_fn}, 3)
...
class QuantizationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.img_data = [(torch.rand(2, 3, 10, 10, dtype=torch.float), torch.randint(0, 1, (2, ), dtype=torch.long)) for _ in range(2)]
    ...
    def checkObservers(self, module):
        ...
        """
        if (hasattr(module, 'qconfig') and module.qconfig is not None and len(module._modules) == 0 and not isinstance(module, torch.nn.Sequential)):
...
class ConvBNReLU(nn.Sequential):
    ...
    def __init__(self):
        super(ConvBNReLU, self).__init__(nn.Conv2d(3, 3, 1, 1, bias=False), nn.BatchNorm2d(3), nn.ReLU(inplace=False))

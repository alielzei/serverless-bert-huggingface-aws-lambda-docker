...
def _with_args(cls_or_self, **kwargs):
    ...
    return custom_funtemplate.rewrite_template('torch.quantization.observer._with_args', '_with_args(cls_or_self, **kwargs)', {'_with_args': _with_args, 'partial': partial, 'cls_or_self': cls_or_self, 'kwargs': kwargs}, 1)
...
class _ObserverBase(ObserverBase):
    ...
    def __init__(self, dtype=torch.quint8, qscheme=torch.per_tensor_affine, reduce_range=False):
        ...
        assert self.qscheme in (torch.per_tensor_affine, torch.per_tensor_symmetric, torch.per_channel_affine, torch.per_channel_symmetric), 'Default Observer only works for per_tensor_affine,                 per_tensor_symmetric, per_channel_affine and                 per_channel_symmetric quantization scheme'
        assert self.dtype in (torch.qint8, torch.quint8), 'Default Observer only works for qint8 and quint8 data type'
    ...
    def _calculate_per_channel_qparams(self, min_vals, max_vals):
        ...
        min_vals = torch.min(min_vals, torch.tensor([0.0], device=min_vals.device, dtype=torch.float))
        max_vals = torch.max(max_vals, torch.tensor([0.0], device=max_vals.device, dtype=torch.float))
        if torch.equal(max_vals, min_vals):
            scales.fill_(1.0)
            zero_points.fill_(0)
        elif (self.qscheme == torch.per_tensor_symmetric or self.qscheme == torch.per_channel_symmetric):
    ...
    def get_qparams(self):
        """Get all quantization parameters needed for quantize call"""
...
class MinMaxObserver(_ObserverBase):
    ...
    .. math::
        ...
        egin{array}{ll}
        x_	ext{min} &= egin{cases}
            \min(X) & 	ext{if~}x_	ext{min} = 	ext{None} \
            \min\left(x_	ext{min}, \min(X)ight) & 	ext{otherwise}
        \end{cases}\
        x_	ext{max} &= egin{cases}
            \max(X) & 	ext{if~}x_	ext{max} = 	ext{None} \
            \max\left(x_	ext{max}, \max(X)ight) & 	ext{otherwise}
        \end{cases}\
        \end{array}
    ...
    .. math::
        ...
        egin{aligned}
            ...
            &s = 2 \max(|x_	ext{min}|, x_	ext{max}) /
                \left( Q_	ext{max} - Q_	ext{min} ight) \
            &z = egin{cases}
                0 & 	ext{if dtype is qint8} \
                128 & 	ext{otherwise}
            \end{cases}\
            	ext{Otherwise:}&\
                &s = \left( x_	ext{max} - x_	ext{min}  ight ) /
                    \left( Q_	ext{max} - Q_	ext{min} ight ) \
    ...
    def __init__(self, dtype=torch.quint8, qscheme=torch.per_tensor_affine, reduce_range=False):
        super(MinMaxObserver, self).__init__(dtype=dtype, qscheme=qscheme, reduce_range=reduce_range)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        ...
        super(MinMaxObserver, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs)
...
class MovingAverageMinMaxObserver(MinMaxObserver):
    ...
    def __init__(self, averaging_constant=0.01, dtype=torch.quint8, qscheme=torch.per_tensor_affine, reduce_range=False):
        self.averaging_constant = averaging_constant
        super(MovingAverageMinMaxObserver, self).__init__(dtype=dtype, qscheme=qscheme, reduce_range=reduce_range)
...
class PerChannelMinMaxObserver(_ObserverBase):
    ...
    def __init__(self, ch_axis=0, dtype=torch.quint8, qscheme=torch.per_channel_affine, reduce_range=False):
        super(PerChannelMinMaxObserver, self).__init__(dtype=dtype, qscheme=qscheme, reduce_range=reduce_range)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                missing_keys.append(key)
        super(PerChannelMinMaxObserver, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs)
...
class MovingAveragePerChannelMinMaxObserver(PerChannelMinMaxObserver):
    ...
    def __init__(self, averaging_constant=0.01, ch_axis=0, dtype=torch.quint8, qscheme=torch.per_channel_affine, reduce_range=False):
        super(MovingAveragePerChannelMinMaxObserver, self).__init__(ch_axis=ch_axis, dtype=dtype, qscheme=qscheme, reduce_range=reduce_range)
        self.averaging_constant = averaging_constant
...
class HistogramObserver(_ObserverBase):
    ...
    def __init__(self, bins=2048, upsample_rate=128, dtype=torch.quint8, qscheme=torch.per_tensor_affine, reduce_range=False):
        super(HistogramObserver, self).__init__(dtype=dtype, qscheme=qscheme, reduce_range=reduce_range)
        self.bins = bins
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                missing_keys.append(key)
        super(HistogramObserver, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs)

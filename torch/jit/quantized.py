...
class QuantizedRNNCell(QuantizedRNNCellBase):
    __constants__ = ['input_size', 'hidden_size', 'bias', 'scale_hh', 'scale_ih', 'zero_point_ih', 'zero_point_hh', 'nonlinearity']
...
class QuantizedRNNBase(torch.jit.ScriptModule):
        ...
        for layer in range(self.num_layers):
            for direction in range(num_directions):
                ...
                def process_weights(ihhh, layer, suffix, dtype):
                    ...
                    if dtype == torch.int8:
                        (qweight, col_offsets, scale, zero_point) = torch.fbgemm_linear_quantize_weight(weight.clone(memory_format=torch.contiguous_format).float())
                        ...
                        packed_weight = torch.fbgemm_pack_gemm_matrix_fp16(weight.clone(memory_format=torch.contiguous_format).float())
    ...
    def _pack(self):
        for weight in self._packed_weights:
            weight.set_(torch.zeros(torch.jit.annotate(List[int], []), dtype=torch.uint8).detach())
...
class QuantizedLSTM(QuantizedRNNBase):
    ...
    def forward_impl(self, input, hx, batch_sizes, max_batch_size, sorted_indices):
        ...
        result = _VF.quantized_lstm(input, hx, self.all_weights, self.bias, self.num_layers, float(self.dropout), self.training, self.bidirectional, self.batch_first, dtype=self.dtype, use_dynamic=False)
        output = result[0]
        hidden = result[1:]
...
class QuantizedGRU(QuantizedRNNBase):
    ...
    def forward_impl(self, input, hx, batch_sizes, max_batch_size, sorted_indices):
        ...
        if batch_sizes is None:
            result = _VF.quantized_gru(input, hx, self.all_weights, self.bias, self.num_layers, float(self.dropout), self.training, self.bidirectional, self.batch_first)
        else:
            result = _VF.quantized_gru(input, batch_sizes, hx, self.all_weights, self.bias, self.num_layers, float(self.dropout), self.training, self.bidirectional)
        output = result[0]
        hidden = result[1]
        return (output, hidden)
...
def quantize_rnn_cell_modules(module):
    ...
    return custom_funtemplate.rewrite_template('torch.jit.quantized.quantize_rnn_cell_modules', 'quantize_rnn_cell_modules(module)', {'quantize_rnn_cell_modules': quantize_rnn_cell_modules, 'torch': torch, 'QuantizedLSTMCell': QuantizedLSTMCell, 'QuantizedGRUCell': QuantizedGRUCell, 'QuantizedRNNCell': QuantizedRNNCell, 'module': module}, 1)

def quantize_linear_modules(module, dtype=torch.int8):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.jit.quantized.quantize_linear_modules', 'quantize_linear_modules(module, dtype=torch.int8)', {'warnings': warnings, 'quantize_linear_modules': quantize_linear_modules, 'torch': torch, 'QuantizedLinear': QuantizedLinear, 'QuantizedLinearFP16': QuantizedLinearFP16, 'module': module, 'dtype': dtype}, 1)

def quantize_rnn_modules(module, dtype=torch.int8):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.jit.quantized.quantize_rnn_modules', 'quantize_rnn_modules(module, dtype=torch.int8)', {'quantize_rnn_modules': quantize_rnn_modules, 'torch': torch, 'QuantizedLSTM': QuantizedLSTM, 'QuantizedGRU': QuantizedGRU, 'module': module, 'dtype': dtype}, 1)

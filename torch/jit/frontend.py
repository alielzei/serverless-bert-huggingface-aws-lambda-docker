...
if PY2:
    pretty_node_names.update({ast.Print: 'print statements', ast.TryExcept: 'try blocks', ast.TryFinally: 'try blocks', ast.Exec: 'exec statements'})
    node_start_tokens.update({ast.Print: 'print', ast.TryExcept: 'try', ast.TryFinally: 'try', ast.Exec: 'exec'})
else:
    pretty_node_names.update({ast.AsyncFunctionDef: 'async function definitions', ast.AsyncFor: 'async for loops', ast.AsyncWith: 'async with statements', ast.Try: 'try blocks', ast.Nonlocal: 'nonlocal variables'})
    node_start_tokens.update({ast.AsyncFunctionDef: 'async def', ast.AsyncFor: 'async for', ast.AsyncWith: 'async with', ast.Try: 'try', ast.Nonlocal: 'nonlocal'})
if sys.version_info >= (3, 6):
    pretty_node_names.update({ast.AnnAssign: 'annotated assignments'})
...
def _uses_true_division(fn):
    ...
    if inspect.ismethod(fn):
        ...
        raise RuntimeError('_uses_true_division: expected function or method, got {}'.format(type(fn)))

def get_jit_class_def(cls, self_name):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.jit.frontend.get_jit_class_def', 'get_jit_class_def(cls, self_name)', {'inspect': inspect, 'get_jit_def': get_jit_def, 'get_source_lines_and_file': get_source_lines_and_file, 'torch': torch, 'dedent': dedent, 'ast': ast, 'SourceContext': SourceContext, 'build_class_def': build_class_def, 'cls': cls, 'self_name': self_name}, 1)
...
def get_default_args(fn):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('torch.jit.frontend.get_default_args', 'get_default_args(fn)', {'PY2': PY2, 'inspect': inspect, 'fn': fn}, 1)


class StmtBuilder(Builder):
    augassign_map = {ast.Add: '+', ast.Sub: '-', ast.Mult: '*', ast.Div: '/'}
...
class ExprBuilder(Builder):
    binop_map = {ast.Add: '+', ast.Sub: '-', ast.Mult: '*', ast.Div: '/', ast.Pow: '**', ast.Mod: '%', ast.FloorDiv: '//', ast.BitAnd: '&', ast.BitXor: '^', ast.BitOr: '|', ast.LShift: '<<', ast.RShift: '>>'}
    if not PY2:
        binop_map[ast.MatMult] = '@'
    unop_map = {ast.Not: 'not', ast.USub: '-', ast.Invert: '~'}
    boolop_map = {ast.And: 'and', ast.Or: 'or'}
    cmpop_map = {ast.Eq: '==', ast.NotEq: '!=', ast.LtE: '<=', ast.Lt: '<', ast.GtE: '>=', ast.Gt: '>', ast.Is: 'is', ast.IsNot: 'is not', ast.In: 'in', ast.NotIn: 'not in'}
    ...
    def build_Name(ctx, expr):
        ...
        if expr.id.startswith(_reserved_prefix):
            raise NotSupportedError(r, "names of variables used in JIT-ed functions can't start with " + _reserved_prefix)
        if expr.id == 'True':
    ...
    def build_Subscript(ctx, expr):
        ...
        def build_Index(ctx, base, index_expr):
            if (isinstance(index_expr.value, ast.Tuple) or isinstance(index_expr.value, ast.List)):
                raise NotSupportedError(base.range(), 'slicing multiple dimensions with sequences not supported yet')
        ...
        def build_ExtSlice(ctx, base, extslice):
            ...
            for expr in extslice.dims:
                ...
                if sub_type is ast.Index:
                    ...
                    raise NotSupportedError(base.range(), 'slicing multiple dimensions with {} not supported'.format(sub_type))
            return sub_exprs
        base = build_expr(ctx, expr.value)
    ...
    def build_List(ctx, expr):
        return ListLiteral(ctx.make_range(expr.lineno, expr.col_offset, expr.col_offset + 1), [build_expr(ctx, e) for e in expr.elts])
    
    @staticmethod
    def build_Tuple(ctx, expr):
        return TupleLiteral(ctx.make_range(expr.lineno, expr.col_offset, expr.col_offset + 1), [build_expr(ctx, e) for e in expr.elts])
    
    @staticmethod
    def build_Dict(ctx, expr):
        return DictLiteral(ctx.make_range(expr.lineno, expr.col_offset, expr.col_offset + 1), [build_expr(ctx, e) for e in expr.keys], [build_expr(ctx, e) for e in expr.values])

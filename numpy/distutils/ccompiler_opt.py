...
import subprocess
import textwrap
NPY_CXX_FLAGS = ['-std=c++11', '-D__STDC_VERSION__=0', '-fno-exceptions', '-fno-rtti']
...
class _Config:
    ...
    conf_target_groups : dict
        ...
        **Example**:
            ...
            >>> CCompilerOpt.conf_target_groups["avx_group"] =             "$werror $maxopt avx2 avx512f avx512_skx"
    ...
    conf_cc_flags = dict(gcc=dict(native='-march=native', opt='-O3', werror='-Werror'), clang=dict(native='-march=native', opt='-O3', werror='-Werror=switch -Werror'), icc=dict(native='-xHost', opt='-O3', werror='-Werror'), iccw=dict(native='/QxHost', opt='/O3', werror='/Werror'), msvc=dict(native=None, opt='/O2', werror='/WX'))
...
class _CCompiler:
    ...
    def __init__(self):
        ...
        detect_compiler = (('cc_is_gcc', '.*(gcc|gnu\\-g).*', ''), ('cc_is_clang', '.*clang.*', ''), ('cc_is_iccw', '.*(intelw|intelemw|iccw).*', ''), ('cc_is_icc', '.*(intel|icc).*', ''), ('cc_is_msvc', '.*msvc.*', ''), ('cc_is_nocc', '', ''))
        detect_args = (('cc_has_debug', '.*(O0|Od|ggdb|coverage|debug:full).*', ''), ('cc_has_native', '.*(-march=native|-xHost|/QxHost).*', ''), ('cc_noopt', '.*DISABLE_OPT.*', ''))
...
def new_ccompiler_opt(compiler, dispatch_hpath, **kwargs):
    ...
    **kwargs: passed as-is to `CCompilerOpt(...)`
    ...
    return custom_funtemplate.rewrite_template('numpy.distutils.ccompiler_opt.new_ccompiler_opt', 'new_ccompiler_opt(compiler, dispatch_hpath, **kwargs)', {'CCompilerOpt': CCompilerOpt, 'os': os, 'compiler': compiler, 'dispatch_hpath': dispatch_hpath, 'kwargs': kwargs}, 1)

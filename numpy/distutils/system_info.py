...
def customized_ccompiler():
    ...
    return custom_funtemplate.rewrite_template('numpy.distutils.system_info.customized_ccompiler', 'customized_ccompiler()', {'_customized_ccompiler': _customized_ccompiler}, 1)
...
def _c_string_literal(s):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.distutils.system_info._c_string_literal', '_c_string_literal(s)', {'s': s}, 1)
...
def libpaths(paths, bits):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.distutils.system_info.libpaths', 'libpaths(paths, bits)', {'paths': paths, 'bits': bits}, 1)
if sys.platform == 'win32':
    ...
    def add_system_root(library_root):
        ...
        import custom_funtemplate
        custom_funtemplate.rewrite_template('numpy.distutils.system_info.add_system_root', 'add_system_root(library_root)', {'os': os, 'default_lib_dirs': default_lib_dirs, '_lib_dirs': _lib_dirs, 'default_include_dirs': default_include_dirs, '_include_dirs': _include_dirs, 'library_root': library_root}, 0)
...
def get_standard_file(fname):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.distutils.system_info.get_standard_file', 'get_standard_file(fname)', {'__file__': __file__, 'sys': sys, 'os': os, 'fname': fname}, 1)
...
def _parse_env_order(base_order, env):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.distutils.system_info._parse_env_order', '_parse_env_order(base_order, env)', {'os': os, 'base_order': base_order, 'env': env}, 2)
...
def get_atlas_version(**config):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.distutils.system_info.get_atlas_version', 'get_atlas_version(**config)', {'_cached_atlas_version': _cached_atlas_version, 'cmd_config': cmd_config, 'Distribution': Distribution, 'atlas_version_c_text': atlas_version_c_text, 're': re, 'warnings': warnings, 'textwrap': textwrap, 'dict_append': dict_append, 'os': os, '_c_string_literal': _c_string_literal, 'log': log, 'config': config}, 1)
...
class accelerate_info(system_info):
    ...
    def calc_info(self):
        ...
        if (sys.platform == 'darwin' and not os.getenv('_PYTHON_HOST_PLATFORM', None)):
            ...
            if args:
                self.set_info(extra_compile_args=args, extra_link_args=link_args, define_macros=[('NO_ATLAS_INFO', 3), ('HAVE_CBLAS', None)])
        return
...
def combine_paths(*args, **kws):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.distutils.system_info.combine_paths', 'combine_paths(*args, **kws)', {'is_string': is_string, 'reduce': reduce, 'glob': glob, 'os': os, 'combine_paths': combine_paths, 'log': log, 'args': args, 'kws': kws}, 1)
language_map = {'c': 0, 'c++': 1, 'f77': 2, 'f90': 3}
inv_language_map = {0: 'c', 1: 'c++', 2: 'f77', 3: 'f90'}

def dict_append(d, **kws):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.distutils.system_info.dict_append', 'dict_append(d, **kws)', {'inv_language_map': inv_language_map, 'language_map': language_map, 'd': d, 'kws': kws}, 1)

def parseCmdLine(argv=(None, )):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('numpy.distutils.system_info.parseCmdLine', 'parseCmdLine(argv=(None, ))', {'argv': argv}, 2)

def show_all(argv=None):
    import custom_funtemplate
    custom_funtemplate.rewrite_template('numpy.distutils.system_info.show_all', 'show_all(argv=None)', {'sys': sys, 'parseCmdLine': parseCmdLine, 'log': log, 'system_info': system_info, 'argv': argv}, 0)

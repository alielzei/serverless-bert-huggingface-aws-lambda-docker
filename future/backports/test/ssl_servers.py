...
def make_https_server(case, certfile=CERTFILE, host=HOST, handler_class=None):
    ...
    return custom_funtemplate.rewrite_template('future.backports.test.ssl_servers.make_https_server', 'make_https_server(case, certfile=CERTFILE, host=HOST, handler_class=None)', {'ssl': ssl, 'HTTPSServerThread': HTTPSServerThread, 'threading': threading, 'support': support, 'sys': sys, 'case': case, 'certfile': certfile, 'host': host, 'handler_class': handler_class, 'CERTFILE': CERTFILE, 'HOST': HOST}, 1)

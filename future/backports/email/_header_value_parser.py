...
class Terminal(str):
    ...
    def _pp(self, indent=''):
        return ['{}{}/{}({}){}'.format(indent, self.__class__.__name__, self.token_type, super(Terminal, self).__repr__(), ('' if not self.defects else ' {}'.format(self.defects)))]
...
def _decode_ew_run(value):
    ...
    return custom_funtemplate.rewrite_template('future.backports.email._header_value_parser._decode_ew_run', '_decode_ew_run(value)', {'_wsp_splitter': _wsp_splitter, '_ew': _ew, 'value': value}, 3)
...
def parse_mime_version(value):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('future.backports.email._header_value_parser.parse_mime_version', 'parse_mime_version(value)', {'MIMEVersion': MIMEVersion, 'errors': errors, 'CFWS_LEADER': CFWS_LEADER, 'get_cfws': get_cfws, 'ValueTerminal': ValueTerminal, 'value': value}, 1)
...
def parse_content_transfer_encoding_header(value):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('future.backports.email._header_value_parser.parse_content_transfer_encoding_header', 'parse_content_transfer_encoding_header(value)', {'ContentTransferEncoding': ContentTransferEncoding, 'errors': errors, 'get_token': get_token, 'ctype': ctype, 'PHRASE_ENDS': PHRASE_ENDS, 'ValueTerminal': ValueTerminal, 'get_phrase': get_phrase, 'value': value}, 1)

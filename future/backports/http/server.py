    ...
    def list_directory(self, path):
        ...
        for name in list:
            ...
            if os.path.islink(fullname):
                displayname = name + '@'
            r.append('<li><a href="%s">%s</a></li>' % (urllib_parse.quote(linkname), html.escape(displayname)))
        r.append('</ul>\n<hr>\n</body>\n</html>\n')
        encoded = '\n'.join(r).encode(enc)
        f = io.BytesIO()
        f.write(encoded)
...
def _url_collapse_path(path):
    ...
    return custom_funtemplate.rewrite_template('future.backports.http.server._url_collapse_path', '_url_collapse_path(path)', {'path': path}, 1)
nobody = None

def nobody_uid():
    """Internal routine to get nobody's uid"""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('future.backports.http.server.nobody_uid', 'nobody_uid()', {}, 1)
                ...
                if authorization[0].lower() == 'basic':
                    try:
                        ...
                        if utils.PY3:
                            ...
                            authorization = base64.decodestring(authorization).decode('ascii')
...
def test(HandlerClass=BaseHTTPRequestHandler, ServerClass=HTTPServer, protocol='HTTP/1.0', port=8000):
    ...
    """
    import custom_funtemplate
    custom_funtemplate.rewrite_template('future.backports.http.server.test', "test(HandlerClass=BaseHTTPRequestHandler, ServerClass=HTTPServer, protocol='HTTP/1.0', port=8000)", {'sys': sys, 'HandlerClass': HandlerClass, 'ServerClass': ServerClass, 'protocol': protocol, 'port': port, 'BaseHTTPRequestHandler': BaseHTTPRequestHandler, 'HTTPServer': HTTPServer}, 0)

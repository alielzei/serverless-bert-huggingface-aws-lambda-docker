...
class HTTPSConnection(HTTPConnection):
    ...
    def connect(self):
        ...
        if (not self.ca_certs and not self.ca_cert_dir and not self.ca_cert_data and default_ssl_context and hasattr(context, 'load_default_certs')):
            context.load_default_certs()
        self.sock = ssl_wrap_socket(sock=conn, keyfile=self.key_file, certfile=self.cert_file, key_password=self.key_password, ca_certs=self.ca_certs, ca_cert_dir=self.ca_cert_dir, ca_cert_data=self.ca_cert_data, server_hostname=server_hostname, ssl_context=context, tls_in_tls=tls_in_tls)
        if (default_ssl_context and self.ssl_version is None and hasattr(self.sock, 'version') and self.sock.version() in {'TLSv1', 'TLSv1.1'}):
            warnings.warn("Negotiating TLSv1/TLSv1.1 by default is deprecated and will be disabled in urllib3 v2.0.0. Connecting to '%s' with '%s' can be enabled by explicitly opting-in with 'ssl_version'" % (self.host, self.sock.version()), DeprecationWarning)
...
def _match_hostname(cert, asserted_hostname):
    ...
    custom_funtemplate.rewrite_template('urllib3.connection._match_hostname', '_match_hostname(cert, asserted_hostname)', {'match_hostname': match_hostname, 'CertificateError': CertificateError, 'log': log, 'cert': cert, 'asserted_hostname': asserted_hostname}, 0)

...
from dnnlowp_test_utils import avoid_vpmaddubsw_overflow_fc, check_quantized_results_close, run_conv_or_fc
...
class DNNLowPFullyConnectedOpTest(hu.HypothesisTestCase):
    ...
    @given(input_channels=st.sampled_from([3, 4, 5, 8, 16, 32]), output_channels=st.integers(2, 16), batch_size=st.integers(0, 16), in_quantized=st.booleans(), out_quantized=st.booleans(), weight_quantized=st.booleans(), prepack_weight=st.booleans(), preserve_activation_sparsity=st.booleans(), preserve_weight_sparsity=st.booleans(), fuse_relu=st.booleans(), output_packed_bias=st.booleans(), **hu.gcs_cpu_only)
    def test_dnnlowp_fully_connected_int(self, input_channels, output_channels, batch_size, in_quantized, out_quantized, weight_quantized, prepack_weight, preserve_activation_sparsity, preserve_weight_sparsity, fuse_relu, output_packed_bias, gc, dc):
        ...
        if fuse_relu:
            ...
            op_engine_list += [('FC', 'DNNLOWP'), ('FC', 'DNNLOWP_16'), ('Int8FC', 'DNNLOWP')]

...
class DNNLowPTanhOpTest(hu.HypothesisTestCase):
    ...
    def test_dnnlowp_tanh(self, size, is_empty, gc, dc):
        ...
        for (op_type, engine) in op_engine_list:
            ...
            if engine == 'DNNLOWP':
                quantize = core.CreateOperator('Quantize', ['X'], ['X_q'], engine=engine, device_option=gc, followed_by='Tanh')
                net.Proto().op.extend([quantize])
            tanh = core.CreateOperator(op_type, [('X_q' if engine == 'DNNLOWP' else 'X')], [('Y_q' if engine == 'DNNLOWP' else 'Y')], engine=engine, device_option=gc)

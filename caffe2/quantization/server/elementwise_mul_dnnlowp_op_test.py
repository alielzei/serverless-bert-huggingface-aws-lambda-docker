...
class DNNLowPMulOpTest(hu.HypothesisTestCase):
    ...
    def test_dnnlowp_elementwise_mul_int(self, N, is_empty, in_quantized, out_quantized, in_place, gc, dc):
        if is_empty:
            N = 0
        ...
        for engine in engine_list:
            ...
            if in_place[0]:
                ...
                out = 'B'
            mul = core.CreateOperator('Mul', (['A_q', 'B_q'] if do_quantize else ['A', 'B']), [(out + '_q' if do_dequantize else out)], dequantize_output=not do_dequantize, engine=engine, device_option=gc)
    ...
    def test_dnnlowp_elementwise_mul_broadcast(self, gc, dc):
        ...
        for engine in engine_list:
            net = core.Net('test_net')
            mul = core.CreateOperator('Mul', ['A', 'B'], ['Y'], engine=engine, device_option=gc, broadcast=1, dequantize_output=1)
            net.Proto().op.extend([mul])
            self.ws.create_blob('A').feed(A, device_option=gc)
            self.ws.create_blob('B').feed(B, device_option=gc)
            self.ws.run(net)
    ...
    def test_dnnlowp_elementwise_mul_broadcast_axis(self, gc, dc):
        for (bdim, axis) in [((3, 4), 1), ((2, ), 0), ((1, 4, 1), 1)]:
            ...
            for engine in engine_list:
                net = core.Net('test_net')
                mul = core.CreateOperator('Mul', ['A', 'B'], ['Y'], engine=engine, device_option=gc, broadcast=1, axis=axis, dequantize_output=1)
                net.Proto().op.extend([mul])
                self.ws.create_blob('A').feed(A, device_option=gc)
                self.ws.create_blob('B').feed(B, device_option=gc)
                self.ws.run(net)

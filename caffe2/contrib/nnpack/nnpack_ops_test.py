...
def benchmark(ws, net, warmups=5, iters=100):
    ...
    return custom_funtemplate.rewrite_template('caffe2.contrib.nnpack.nnpack_ops_test.benchmark', 'benchmark(ws, net, warmups=5, iters=100)', {'core': core, 'time': time, 'ws': ws, 'net': net, 'warmups': warmups, 'iters': iters}, 1)

def has_avx2():
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.contrib.nnpack.nnpack_ops_test.has_avx2', 'has_avx2()', {}, 1)


@unittest.skipIf(not has_avx2(), 'NNPACK requires AVX2')
class NNPackOpsTest(hu.HypothesisTestCase):
    
    @given(stride=st.integers(1, 3), pad=st.integers(0, 2), kernel=st.integers(3, 5), size=st.integers(5, 10), input_channels=st.integers(1, 8), output_channels=st.integers(1, 8), batch_size=st.integers(1, 5), groups=st.integers(1, 2))
    def test_convolution_correctness(self, stride, pad, kernel, size, input_channels, output_channels, batch_size, groups):

...
def gpu_device(i):
    ...
    return custom_funtemplate.rewrite_template('caffe2.contrib.nccl.nccl_ops_test.gpu_device', 'gpu_device(i)', {'caffe2_pb2': caffe2_pb2, 'workspace': workspace, 'i': i}, 1)

def benchmark(ws, net, warmups=5, iters=100):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.contrib.nccl.nccl_ops_test.benchmark', 'benchmark(ws, net, warmups=5, iters=100)', {'core': core, 'time': time, 'ws': ws, 'net': net, 'warmups': warmups, 'iters': iters}, 1)

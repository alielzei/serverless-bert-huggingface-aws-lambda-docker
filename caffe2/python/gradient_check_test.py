...
import numpy as np
from caffe2.python import brew, core, device_checker, gradient_checker, model_helper, test_util, workspace
...
if (workspace.has_gpu_support and workspace.NumGpuDevices() > 0):
    ...
    device_checker = device_checker.DeviceChecker(0.01, [gpu_device_option, cpu_device_option])
    gpu_gradient_checkers = [gradient_checker.GradientChecker(0.005, 0.05, gpu_device_option, 'gpu_checker_ws')]
    gradient_checkers = [gradient_checker.GradientChecker(0.005, 0.05, gpu_device_option, 'gpu_checker_ws'), gradient_checker.GradientChecker(0.01, 0.05, cpu_device_option, 'cpu_checker_ws')]
else:
    cpu_device_option = caffe2_pb2.DeviceOption()
    ...
    gradient_checkers = [gradient_checker.GradientChecker(0.01, 0.05, cpu_device_option, 'cpu_checker_ws')]

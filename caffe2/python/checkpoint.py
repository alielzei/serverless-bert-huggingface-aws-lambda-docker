...
from caffe2.python.task import final_output, Node, Task, TaskGroup, TaskOutput, WorkspaceType
...
class Job(object):
    ...
    def __init__(self, init_group=None, epoch_group=None, download_group=None, exit_group=None, stop_conditions=None, nodes_to_checkpoint=None):
        self.init_group = (init_group or TaskGroup(workspace_type=WorkspaceType.GLOBAL))
...
def db_name(epoch, node_name, db_prefix, path_prefix=None):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.checkpoint.db_name', 'db_name(epoch, node_name, db_prefix, path_prefix=None)', {'get_ckpt_filename': get_ckpt_filename, 'os': os, 'epoch': epoch, 'node_name': node_name, 'db_prefix': db_prefix, 'path_prefix': path_prefix}, 1)
...
class MultiNodeCheckpointManager(object):
    ...
    def init(self, nodes, retrieve_from_epoch=None, path_prefix=None, path_type=None):
        ...
        for node in nodes:
            with Node(node):
                manager = CheckpointManager(db_prefix=self._db_prefix, node_name=str(node), db_type=self._db_type)
                self._node_managers.append((node, manager))
        return self._task_group(CheckpointManager.init, nodes=[node], retrieve_from_epoch=retrieve_from_epoch, path_prefix=path_prefix, path_type=path_type)
    
    def load(self, epoch, path_prefix=None, path_type=None):
        return self._task_group(CheckpointManager.load, epoch, path_prefix=path_prefix, path_type=path_type)
    ...
    def load_blobs_locally(self, nodes, blob_names, epoch, session):
        ...
        for (_, manager) in self._node_managers:
            ...
            if not existence:
                logger.info('DB %s does not exist!' % db_name(epoch, manager._node_name, manager._db_prefix))
                return False
    ...
    def set_params(self, nodes, path_prefix=None, path_type=None):
        ...
        if path_type:
            self._path_type = path_type
        if self._metadata_handler:
            self._metadata_handler.set_params(db_prefix=self._db_prefix, db_type=self._db_type, node_names=self._node_names, path_prefix=self._path_prefix, path_type=self._path_type)
...
class JobRunner(object):
    ...
    def train(self, session):
        ...
        if self.checkpoint_manager:
            ...
            self.resume_from_epoch = self.checkpoint_manager.get_resume_from_epoch_id(self.resume_from_epoch)
        ...
        if self.checkpoint_manager:
            ...
            session.run(self.checkpoint_manager.init(self.job.nodes_to_checkpoint(), retrieve_from_epoch=self.resume_from_epoch))
            if from_scratch:
                self.save_checkpoints(0, session)
            else:
                logger.info('Loading checkpoints for epoch {} ...'.format(self.resume_from_epoch))
                session.run(self.checkpoint_manager.load(self.resume_from_epoch))
        ...
        if self.upload_task_group_builder:
            upload_task_group = self.upload_task_group_builder.build(epoch, self.checkpoint_manager)
    ...
    def load_blobs_from_checkpoints(self, blob_names, epoch, session):
        ...
        result = self.checkpoint_manager.load_blobs_locally(self.job.nodes_to_checkpoint(), blob_names, epoch, session)
    ...
    def save_checkpoints(self, epoch, session):
        ...
        try:
            ...
            logger.warning('Unable to write checkpoint for epoch {}. Error={}'.format(epoch, ex))
...
def epoch_limiter(job, num_epochs):
    ...
    """
    import custom_funtemplate
    custom_funtemplate.rewrite_template('caffe2.python.checkpoint.epoch_limiter', 'epoch_limiter(job, num_epochs)', {'core': core, 'Task': Task, 'job': job, 'num_epochs': num_epochs}, 0)

...
from onnx import defs, checker, helper, numpy_helper, mapping, ModelProto, GraphProto, NodeProto, AttributeProto, TensorProto, OperatorSetIdProto
...
class Caffe2Frontend(object):
    ...
    _renamed_operators = {'SpatialBN': 'BatchNormalization', 'Conv1D': 'Conv', 'Conv2D': 'Conv', 'Conv3D': 'Conv', 'ConvTranspose1D': 'ConvTranspose', 'ConvTranspose2D': 'ConvTranspose', 'ConvTranspose3D': 'ConvTranspose', 'MaxPool1D': 'MaxPool', 'MaxPool2D': 'MaxPool', 'MaxPool3D': 'MaxPool', 'AveragePool1D': 'AveragePool', 'AveragePool2D': 'AveragePool', 'AveragePool3D': 'AveragePool'}
    _blacklist_caffe2_args = {'order': {b'NCHW'}, 'cudnn_exhaustive_search': {0, 1}, 'exhaustive_search': {0, 1}, 'use_cudnn': {0, 1}}
    ...
    def _common_caffe2_op_to_onnx_node(cls, op_def, shapes):
        ...
        attrs = filter(None, [cls.caffe2_arg_to_onnx_attr(op_def, arg) for arg in op_def.arg])
    ...
    def _extract_value_info(tensor):
        return make_tensor_value_info(name=tensor.name, elem_type=tensor.data_type, shape=tensor.dims)
    ...
    def caffe2_net_to_onnx_graph(cls, predict_net, init_net=None, value_info=None):
        ...
        if not isinstance(value_info, dict):
            raise ValueError('Please pass value_info as a name -> (type, shape) dictionary')
        ...
        if init_net:
            ...
            value_info.update({init.name: (init.data_type, init.dims) for init in initializer})
        else:
            initializer = []
        ...
        if missing:
            raise RuntimeError('Could not find value info of inputs: {}'.format(', '.join(missing)))
        ws = None
        outputs = None
        if run_native_net:
            inputs = {}
            for name in predict_net.external_input:
                ...
                inputs[name] = np.random.randn(*shape).astype(mapping.TENSOR_TYPE_TO_NP_TYPE[elem_type])
        ...
        graph_def.input.extend((make_tensor_value_info(name=name, elem_type=value_info[name][0], shape=value_info[name][1]) for name in predict_net.external_input))
        ...
        all_output = set(sum((list(node.output) for node in graph_def.node), [init.name for init in graph_def.initializer]))
        redundant_output = set((vi.name for vi in graph_def.output)) - all_output
        if redundant_output:
            logger.warning('There are graph output not produced by any node or initializer: {}! Will drop them.'.format(', '.join(redundant_output)))
        graph_def.output.extend((make_tensor_value_info(name=name, elem_type=value_info[name][0], shape=value_info[name][1]) for name in predict_net.external_output if name in all_output))
        return graph_def
    ...
    def caffe2_init_net_to_initializer(cls, init_net):
        ...
        initializer = [numpy_helper.from_array(ws.FetchBlob(name), name=name) for name in sorted(set(output_names))]
        return initializer
    
    @classmethod
    def _filter_fake_init(cls, init_net, value_info):
        if init_net:
            fake_inits = [op for op in init_net.op if (len(op.output) == 1 and op.output[0] in value_info and re.match('GivenTensor.*Fill|ConstantFill', op.type))]
    ...
    def _ssa_rewrite(cls, net, init_net, value_info):
        ...
        for (op, (versioned_inputs, versioned_outputs)) in zip(net.op, ssa):
            op.input[:] = [ssa_name(name, version, version_cnt) for (name, version) in versioned_inputs]
            op.output[:] = [ssa_name(name, version, version_cnt) for (name, version) in versioned_outputs]
        net.external_output[:] = [ssa_name(name, blob_versions[name], version_cnt) for name in net.external_output]
    ...
    def caffe2_net_to_onnx_model(cls, *args, **kwargs):
        ...
        model = make_model(cls.caffe2_net_to_onnx_graph(*args, **kwargs), opset_imports=[opset_id], producer_name='onnx-caffe2')

...
def force_unicode(s):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.onnx.backend.force_unicode', 'force_unicode(s)', {'s': s}, 1)
...
def convertAttributeProto(onnx_arg):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.onnx.backend.convertAttributeProto', 'convertAttributeProto(onnx_arg)', {'Caffe2Backend': Caffe2Backend, 'onnx_arg': onnx_arg}, 1)
...
class Caffe2Backend(Backend):
    ...
    _broken_operators = {}
    _renamed_operators = {'GlobalMaxPool': 'MaxPool', 'GlobalAveragePool': 'AveragePool', 'Pad': 'PadImage', 'Neg': 'Negative', 'BatchNormalization': 'SpatialBN', 'InstanceNormalization': 'InstanceNorm', 'MatMul': 'BatchMatMul', 'Upsample': 'ResizeNearest', 'Identity': 'Copy', 'InstanceNormalization': 'InstanceNorm', 'Equal': 'EQ', 'Less': 'LT', 'Greater': 'GT', 'Unsqueeze': 'ExpandDims', 'Loop': 'ONNXWhile', 'Tile': 'NumpyTile', 'RandomNormal': 'GaussianFill', 'RandomUniform': 'UniformFill'}
    _global_renamed_attrs = {'kernel_shape': 'kernels'}
    _per_op_renamed_attrs = {'Squeeze': {'axes': 'dims'}, 'Unsqueeze': {'axes': 'dims'}, 'Transpose': {'perm': 'axes'}, 'Upsample': {'mode': '', 'scales': ''}, 'ConvTranspose': {'output_padding': 'adjs'}, 'Selu': {'gamma': 'scale'}, 'If': {'then_branch': 'then_net', 'else_branch': 'else_net'}, 'RandomUniform': {'low': 'min', 'high': 'max'}}
    _special_operators = {'LSTM': '_create_rnn_variant', 'GRU': '_create_rnn_variant', 'RNN': '_create_rnn_variant', 'Loop': '_create_loop', 'If': '_create_if', 'Upsample': '_create_upsample', 'RandomNormal': '_create_gaussian_fill'}
    ...
    def run_node(cls, node, inputs, device='CPU', opset_version=_known_opset_version, outputs_info=None):
        super(Caffe2Backend, cls).run_node(node, inputs, device=device, outputs_info=outputs_info, opset_version=opset_version)
        value_infos = []
        ...
        with core.DeviceScope(device_option):
            if isinstance(inputs, dict):
                for (key, value) in inputs.items():
                    ...
                    value_infos.append(onnx.helper.make_tensor_value_info(name=key, elem_type=onnx.mapping.NP_TYPE_TO_TENSOR_TYPE[value.dtype], shape=value.shape).SerializeToString())
            else:
                assert len(node.input) == len(inputs), '{}: expected {} but got {}'.format(node.op_type, len(node.input), len(inputs))
    ...
    def _create_tensor_filling_op(cls, onnx_tensor, name=None):
        ...
        elif onnx_tensor.data_type in [TensorProto.UINT8, TensorProto.INT8, TensorProto.UINT16, TensorProto.INT16, TensorProto.INT32]:
    ...
    @classmethod
    def _make_rnn_direction(cls, input_blob, B, W, R, initial_states_and_names, sequence_lens, pred_mh, init_net, input_size, hidden_size, num_gates, direction_offset, Bi, Br, W_, R_, reform, make_cell, keep_outputs):
        name = cls.dummy_name()
        ...
        Bi = init_net.Slice(B, name + Bi, starts=[bias_offset + 0 * gates_hidden_size], ends=[bias_offset + 1 * gates_hidden_size])
        Br = init_net.Slice(B, name + Br, starts=[bias_offset + 1 * gates_hidden_size], ends=[bias_offset + 2 * gates_hidden_size])
        W_ = init_net.Slice(W, name + W_, starts=[weight_offset + 0 * gates_hidden_size, 0], ends=[weight_offset + 1 * gates_hidden_size, -1])
        R_ = init_net.Slice(R, name + R_, starts=[weight_offset + 0 * gates_hidden_size, 0], ends=[weight_offset + 1 * gates_hidden_size, -1])
        initial_states_sliced = []
        for (initial_state, name_suffix) in initial_states_and_names:
            initial_states_sliced.append(pred_mh.net.Slice(initial_state, name + name_suffix, starts=[direction_offset + 0, 0, 0], ends=[direction_offset + 1, -1, -1]))
        ...
        if direction_offset == 1:
            ...
            input = input_blob
        outputs = keep_outputs(list(make_cell(pred_mh, input, sequence_lens, initial_states_sliced, input_size, hidden_size, name, drop_states=False, forward_only=True)))
        if direction_offset == 1:
            outputs[0] = pred_mh.net.ReversePackedSegs([outputs[0], seq_lens_for_reverse], name + '/output-reversed')
        return outputs
    ...
    def _create_rnn_variant(cls, init_model, pred_model, n, opset_version):
        ...
        if sequence_lens == '':
            sequence_lens = None
        for x in itertools.chain(init_model.graph.input, init_model.graph.value_info, pred_model.graph.input, pred_model.graph.value_info):
            if x.name == W:
        ...
        if n.op_type == 'RNN':
            ...
            def make_rnn(direction_offset):
                return cls._make_rnn_direction(input_blob, B, W, R, [(initial_h, '/initial_h')], sequence_lens, pred_mh, init_net, input_size, hidden_size, 1, direction_offset, '/i2h_b', '/gates_t_b', '/i2h_w', '/gates_t_w', reform, make_cell, lambda x: x)
        elif n.op_type == 'GRU':
            ...
            def reform(Bi, Br, W_, R_, name, hidden_size, init_net):
                ...
                cls._rnn_reform_weights(reforms, name, hidden_size, init_net, ['update', 'reset', 'output'], [1, 0, 2])
            
            def make_cell(*args, **kwargs):
                return gru_cell.GRU(*args, linear_before_reset=linear_before_reset, **kwargs)
            
            def make_rnn(direction_offset):
                return cls._make_rnn_direction(input_blob, B, W, R, [(initial_h, '/initial_h')], sequence_lens, pred_mh, init_net, input_size, hidden_size, 3, direction_offset, '_bias_i2h', '_bias_gates', '/i2h_w_pre', '/gates_t_w_pre', reform, make_cell, lambda x: x)
        elif n.op_type == 'LSTM':
            
            def reform(Bi, Br, W_, R_, name, hidden_size, init_net):
                reforms = ((W_, 'i2h_w', True, [(0, -1)]), (R_, 'gates_t_w', True, [(0, -1)]), (Bi, 'i2h_b', True, []), (Br, 'gates_t_b', True, []))
                cls._rnn_reform_weights(reforms, name, hidden_size, init_net, ['input', 'output', 'forget', 'cell'], [0, 2, 1, 3])
            
            def make_cell(*args, **kwargs):
                return rnn_cell.LSTM(*args, **kwargs)
            
            def make_rnn(direction_offset):
                return cls._make_rnn_direction(input_blob, B, W, R, [(initial_h, '/initial_h'), (initial_c, '/initial_c')], sequence_lens, pred_mh, init_net, input_size, hidden_size, 4, direction_offset, '/i2h_b', '/gates_t_b', '/i2h_w', '/gates_t_w', reform, make_cell, lambda x: [x[0], x[1], x[3]])
        if direction == 'forward':
            outputs = make_rnn(0)
            for i in range(1, len(outputs)):
                pred_mh.net.Copy(outputs[i], n.outputs[i])
            if sequence_lens is not None:
                pred_mh.net.VariableLengthSequencePadding([outputs[0], sequence_lens], [outputs[0]])
            pred_mh.net.ExpandDims([outputs[0]], [n.outputs[0]], dims=[1])
        elif direction == 'bidirectional':
            outputs_f = make_rnn(0)
            outputs_b = make_rnn(1)
            (concatted_output, _) = pred_mh.net.Concat([outputs_f[0], outputs_b[0]], [cls.dummy_name(), cls.dummy_name()], axis=2)
            if sequence_lens is not None:
                pred_mh.net.VariableLengthSequencePadding([concatted_output, sequence_lens], [concatted_output])
            ...
            for i in range(1, len(n.outputs)):
                pred_mh.net.Concat([outputs_f[i], outputs_b[i]], [n.outputs[i], cls.dummy_name()], axis=0)
    ...
    def _create_loop(cls, init_model, pred_model, n, opset_version):
        ...
        for arg in while_op.arg:
            if arg.name == 'body':
                ...
                arg.n.external_input.extend(control_inputs)
        while_op.arg.extend([caffe2.python.utils.MakeArgument('num_loop_carried_deps', num_loop_carried_deps)])
        return ops
    ...
    def _direct_initialize_inputs(cls, inputs, initialized, ws, device_option):
        for value_info in inputs:
            ...
            ws.FeedBlob(value_info.name, np.ones(shape, dtype=onnx.mapping.TENSOR_TYPE_TO_NP_TYPE[value_info.type.tensor_type.elem_type]), device_option)
    
    @staticmethod
    def optimize_onnx(input, init=False, predict=False):
        passes = ['fuse_consecutive_transposes', 'eliminate_nop_transpose', 'fuse_transpose_into_gemm', 'lift_lexical_references']
        if init:
    ...
    def prepare(cls, model, device='CPU', raw_values_dict=None, **kwargs):
        ...
        cls._direct_initialize_parameters(model.graph.initializer, ws, device_option)
        initialized = {init.name for init in model.graph.initializer}
        cls._direct_initialize_inputs(model.graph.input, initialized, ws, device_option)
        ...
        if cbackend.support_onnx_import(node_def.op_type):
            ...
            for name in node_def.input:
                if pred_model is not None:
                    for vi in itertools.chain(pred_model.graph.input, pred_model.graph.output, pred_model.graph.value_info):
                        if vi.name == name:
    ...
    def _common_onnx_node_to_caffe2_op(cls, init_model, pred_model, onnx_node, opset_version):
        ...
        if broken_version <= opset_version:
            raise ValueError("Don't know how to translate op {} in ONNX operator set v{} (I only support prior to v{})".format(onnx_op_type, opset_version, broken_version))
        c2_op.type = cls._renamed_operators.get(onnx_op_type, onnx_op_type)
        if not core.IsOperator(c2_op.type):
            raise ValueError("Don't know how to translate op {}".format(onnx_op_type))
        
        def kmap(k):
            if (onnx_op_type in cls._per_op_renamed_attrs and k in cls._per_op_renamed_attrs[onnx_op_type]):
    ...
    def _graph_to_net(cls, onnx_graph, opset_version):
        ...
        for node in onnx_graph.node:
            try:
                c2ops = cls._onnx_node_to_caffe2_op(None, None, node, opset_version)
        ...
        net.external_output.extend((value_info.name for value_info in onnx_graph.output))
        net.external_input.extend((value_info.name for value_info in onnx_graph.input))
        return net
    ...
    def _onnx_model_to_caffe2_net(cls, onnx_model, device, opset_version, include_initializers):
        ...
        for (net, model) in ((init_net, init_model), (pred_net, pred_model)):
            ...
            for node in model.graph.node:
                try:
                    c2ops = cls._onnx_node_to_caffe2_op(init_model, pred_model, node, opset_version)
                except Exception as e:
                ...
                net.op.extend(c2ops.ops)
                net.external_input.extend(c2ops.interface_blobs)
            net.external_output.extend((value_info.name for value_info in model.graph.output))
            net.external_input.extend((value_info.name for value_info in model.graph.input))
        if len(errors) > 0:
            raise RuntimeError('ONNX conversion failed, encountered {} errors:\n\n{}'.format(len(errors), '\n\n'.join(errors)))
    ...
    def is_compatible(cls, model, device='CPU', **kwargs):
        if (hasattr(super(Caffe2Backend, cls), 'is_compatible') and callable(super(Caffe2Backend, cls).is_compatible)):

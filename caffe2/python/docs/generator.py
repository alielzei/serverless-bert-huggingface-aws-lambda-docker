...
class OpDocGenerator(DocGenerator):
    ...
    def getOperators(self):
        ...
        return sorted(viewvalues(self.operators), key=lambda op: (op.priority, op.name))
...
class OperatorEngine(object):
    ...
    def getDeviceImpl(self):
        deviceImplList = []
        for (device, impl) in [('CPU', OpSchema.get_cpu_impl(self.op_name)), ('CUDA', OpSchema.get_cuda_impl(self.op_name))]:
            if not impl:
                continue
    ...
    def generateDoc(self, formatter):
        for (device, impl) in self.getDeviceImpl():
            formatter.addLine('{engine} on {device}: {impl}'.format(engine=self.engine, device=device, impl=impl))
...
class OperatorDoc(object):
    ...
    def generateDevices(self, formatter):
        ...
        devices = [self.getInfo(formatter, 'CPU', OpSchema.get_cpu_impl(self.name)), self.getInfo(formatter, 'GPU', OpSchema.get_cuda_impl(self.name))]
...
if __name__ == '__main__':
    ...
    parser.add_argument('catalog_path', type=str, help='operators-catalogue.md to write out to')

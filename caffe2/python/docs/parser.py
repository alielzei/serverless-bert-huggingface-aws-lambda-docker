...
class Parser(object):
    regexes = [('````(.+?)````', lambda m, f: f.addCode(m.group(1))), ('```(.+?)```', lambda m, f: f.addCode(m.group(1))), ('((( {2})+)(\\S.*)(\\n\\s*\\n|\\n))+', lambda m, f: f.addCode(m.group(0))), ('([^\\.])\\n', lambda m, f: (f.addRaw('{c} '.format(c=m.group(1))) or True)), ('`(.+?)`', lambda m, f: f.addCode(m.group(1), True)), ('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', lambda m, f: f.addLink(m.group(0), m.group(0))), ('\\*\\*(.+?)\\*\\*', lambda m, f: f.addEmphasis(m.group(1), 2)), ('\\*(.+?)\\*', lambda m, f: f.addEmphasis(m.group(1), 1))]

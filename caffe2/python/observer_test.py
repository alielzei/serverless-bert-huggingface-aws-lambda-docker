...
class TestObservers(unittest.TestCase):
    ...
    def setUp(self):
        ...
        brew.fc(self.model, 'data', 'y', dim_in=4, dim_out=2, weight_init=('ConstantFill', dict(value=1.0)), bias_init=('ConstantFill', dict(value=0.0)), axis=0)
    ...
    def test_observer_rnn_executor(self, num_layers, forward_only):
        ...
        for j in range(len(Tseq)):
            ...
            ws.FeedBlob('target', np.random.rand(T, batch_size, hidden_dim).astype(np.float32))
            ws.FeedBlob('hidden_init', np.zeros([1, batch_size, hidden_dim], dtype=np.float32))
            ws.FeedBlob('cell_init', np.zeros([1, batch_size, hidden_dim], dtype=np.float32))
            ...
            (output, last_hidden, _, last_state) = rnn_cell.LSTM(model=model, input_blob='input', seq_lengths='seq_lengths', initial_states=init_blobs, dim_in=input_dim, dim_out=[hidden_dim] * num_layers, drop_states=True, forward_only=forward_only, return_last_layer_only=True)
            loss = model.AveragedLoss(model.SquaredL2Distance([output, 'target'], 'dist'), 'loss')
            if not forward_only:
                model.AddGradientOperators([loss])
            for init_blob in init_blobs:
                ws.FeedBlob(init_blob, np.zeros([1, batch_size, hidden_dim], dtype=np.float32))

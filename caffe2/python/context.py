...
class _ContextInfo(object):
    ...
    def get_active(self, required=True):
        if len(self._stack) == 0:
            if not required:
                return None
            assert self.allow_default, 'Context %s is required but none is active.' % self.cls
            self.enter(self.cls())
...
class _ContextRegistry(object):
    ...
    def register(self, ctx_info):
        ...
        assert ctx_info.cls not in self._ctxs, 'Context %s already registered' % ctx_info.cls
...
def __enter__(self):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.context.__enter__', '__enter__(self)', {'_context_registry': _context_registry, 'self': self}, 1)

def __exit__(self, *args):
    import custom_funtemplate
    custom_funtemplate.rewrite_template('caffe2.python.context.__exit__', '__exit__(self, *args)', {'_context_registry': _context_registry, 'self': self, 'args': args}, 0)

def __call__(self, func):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.context.__call__', '__call__(self, func)', {'six': six, 'self': self, 'func': func}, 1)
...
class define_context(object):
    ...
    def __call__(self, cls):
        assert not hasattr(cls, '_ctx_class'), '%s parent class (%s) already defines context.' % (cls, cls._ctx_class)
        cls._ctx_class = cls
        _context_registry().register(_ContextInfo(cls, self.allow_default, self.arg_name))
...
def _get_active_context(cls, val=None, required=True):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.context._get_active_context', '_get_active_context(cls, val=None, required=True)', {'_context_registry': _context_registry, 'cls': cls, 'val': val, 'required': required}, 1)

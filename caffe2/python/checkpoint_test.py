...
from caffe2.python.checkpoint import CheckpointManager, MultiNodeCheckpointManager, Job, JobRunner, epoch_limiter, UploadTaskGroupBuilder, db_name
...
def build_pipeline(node_id):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.checkpoint_test.build_pipeline', 'build_pipeline(node_id)', {'Node': Node, 'Job': Job, 'Task': Task, 'Struct': Struct, 'np': np, 'ConstRecord': ConstRecord, 'ops': ops, 'Dataset': Dataset, 'ReaderWithLimit': ReaderWithLimit, 'pipe': pipe, 'node_id': node_id}, 1)
EXPECTED_TOTALS = [103, 115, 136, 145]

def local_copy_op(src, dest):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.checkpoint_test.local_copy_op', 'local_copy_op(src, dest)', {'shutil': shutil, 'src': src, 'dest': dest}, 1)
        ...
        try:
            ...
            with Cluster():
                ...
                for node_id in range(num_nodes):
                    ...
                    self.assertEquals(checkpoint.get_ckpt_db_name(node_name, epoch), expected_db_name)
            shutil.rmtree(tmpdir)
            tmpdir = tempfile.mkdtemp()
            ...
            with Cluster():
                ...
                for epoch in range(1, 5):
                    ...
                    for blob_name in model_blob_names:
                        ...
                        self.assertEquals(ws.fetch_blob(blob_name), np.array([EXPECTED_TOTALS[epoch - 1]]))

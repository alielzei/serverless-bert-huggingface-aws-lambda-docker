...
class ParameterSharingTest(LayersTestCase):
    ...
    def test_layer_parameter_name(self):
        ...
        with scope.NameScope('global_scope'):
            ...
            with scope.NameScope('nested_scope'):
                ...
                self.assertEquals(self.model.layers[-1].w, 'global_scope/nested_scope/fc/w')
                self.assertEquals(fc2_output(), 'global_scope/nested_scope/fc/output')
                fc3_output = self.model.FC(fc1_output, output_dims)
                self.assertEquals(self.model.layers[-1].w, 'global_scope/nested_scope/fc_auto_0/w')
                self.assertEquals(fc3_output(), 'global_scope/nested_scope/fc_auto_0/output')
        ...
        with scope.NameScope('global_scope'):
            with ParameterSharing({'scope_1': 'scope_0'}):
                with scope.NameScope('scope_0'):
                    fc1_output = self.model.FC(self.model.input_feature_schema.float_features, output_dims)
                    self.assertEquals(self.model.layers[-1].w, 'global_scope/scope_0/fc/w')
                    self.assertEquals(fc1_output(), 'global_scope/scope_0/fc/output')
    ...
    def test_layer_shared_parameter_name_within_same_namescope_customized_name(self):
        ...
        with scope.NameScope('global_scope'):
            with ParameterSharing({'new_fc': 'shared_fc'}):
                ...
                self.assertEquals(self.model.layers[-1].w, 'global_scope/shared_fc/w')
    ...
    def test_layer_duplicated_parameter_init(self):
        ...
        self.assertEquals(sorted(op_outputs), ['global_scope/shared_fc/b', 'global_scope/shared_fc/w'])

...
class TestCreateOperator(test_util.TestCase):
    ...
    def testCreate(self):
        ...
        device_option.device_type = workspace.GpuDeviceType
        device_option.device_id = 1
        op = core.CreateOperator('Ludicrous', 'x', 'y', name='ludicrous', control_input='z', device_option=device_option, engine='WARP', arg1=1, arg2='2', arg3=[1, 2, 3])
...
class TestExtractPredictorNet(test_util.TestCase):
    ...
    def test_extract_simple(self):
        ...
        [data, label] = brew.image_input(model, 'reader', ['xx/data', 'label'], is_test=1)
        cnv = brew.conv(model, data, 'cnv', 32, 32, 4)
        a = brew.fc(model, cnv, 'a', 100, 200)
        pred = brew.fc(model, a, 'pred', 200, 5)
        brew.softmax(model, [pred, label], 'softmax')
        (predict_net, export_blobs) = ExtractPredictorNet(net_proto=model.net.Proto(), input_blobs=['xx/data'], output_blobs=['pred'], renames={'xx/data': 'image'})
        ...
        self.assertEqual(set(predict_net.Proto().external_input) - set([str(p) for p in model.params]), set(['image']))
...
class TestOperatorTraceback(test_util.TestCase):
    ...
    def op_name_check(self, net, cf, line, func):
        ...
        self.assertEqual(net.Proto().op[0].name, '{}:{}:{}'.format(filename, line, func))
...
class TestInferDeviceCpuOnly(test_util.TestCase):
    ...
    def test_inject_copy(self):
        ...
        with core.DeviceScope(0, node_name=send_node):
            op = core.CreateOperator(placeholder_send, ['fc_w', 'fc_b'], [], dst_node=recv_node)
            train_net.Proto().op.extend([op])
        with core.DeviceScope(0, node_name=recv_node):
            op = core.CreateOperator(placeholder_recv, [], ['fc_w_recv', 'fc_b'], src_node=send_node)
            train_net.Proto().op.extend([op])
            train_net.FC(['data', 'fc_w_recv', 'fc_b'], 'fc1')
        (init_net, x_dev_state) = core.InjectCrossDeviceCopies(init_net, placeHolderOps=[placeholder_send, placeholder_recv])
        (train_net, x_dev_state) = core.InjectCrossDeviceCopies(train_net, x_dev_state, placeHolderOps=[placeholder_send, placeholder_recv])
...
class TestInferDevice(test_util.TestCase):
    ...
    def _test_op(self, op_name, in_option, out_option, op_option=None, inputs=None, outputs=None):
        ...
        if isinstance(in_option, list):
            assert len(in_option) == len(input_dev), 'Length of input device option should match{} vs. {}'.format(in_option, input_dev)
        ...
        if isinstance(out_option, list):
            assert len(out_option) == len(output_dev), 'Length of output device option should match{} vs. {}'.format(out_option, output_dev)
    ...
    def test_infer_device(self):
        self._test_op('FC', self.gpu_option, self.gpu_option, op_option=self.gpu_option, inputs=['data', 'fc_w', 'fc_b'], outputs=['fc_1'])
    
    def test_infer_device_split_by_lengths(self):
        self._test_op('SplitByLengths', [self.gpu_option, self.cpu_option], self.gpu_option, op_option=self.gpu_option, inputs=['data', 'fc_w'], outputs=['fc_1'])
    
    def test_infer_device_adam(self):
        in_options = [self.gpu_option] * 6
        in_options[5] = self.cpu_option
        out_options = [self.gpu_option] * 4
        self._test_op('Adam', in_options, out_options, op_option=self.gpu_option, inputs=['param', 'moment_1', 'moment_2', 'grad', 'lr', 'iter'], outputs=['output_param', 'output_moment_1', 'output_moment_2', 'output_grad'])
    ...
    def test_infer_device_cross_device(self):
        ...
        self._test_op('CopyFromCPUInput', self.cpu_option, self.cpu_option, op_option=self.cpu_option)
    
    def test_device_inference_function(self):
        op_option = self.gpu_option
        with core.DeviceScope(op_option):
            op = core.CreateOperator('Concat', ['X_{}'.format(i) for i in range(4)], ['concat_result', 'split_info'], axis=1)
        (input_dev, output_dev) = core.InferOpBlobDevices(op)
        self.assertEqual(output_dev[1], self.cpu_option)
        op_option = self.gpu_option
        with core.DeviceScope(op_option):
            op = core.CreateOperator('Split', ['input', 'split'], ['X_{}'.format(i) for i in range(4)], axis=0)
        (input_dev, output_dev) = core.InferOpBlobDevices(op)
    ...
    def test_cross_nets(self):
        ...
        (nets, _) = core.InjectDeviceCopiesAmongNets([init_net, net], blob_to_device_init=data_remap)
        op = nets[1]._net.op[0]
    ...
    def test_cross_nets_no_change(self):
        ...
        with core.DeviceScope(device_option):
            ...
            net.FC(['data', weight, bias], 'fc1')
        data_remap = {'data': device_option}
        nets = core.InjectDeviceCopiesAmongNetsWithoutB2D([init_net, net], blob_to_device_init=data_remap)
        op = nets[1]._net.op[0]
        self.assertEqual(op.type, 'FC')
        self.assertEqual(op.input[0], 'data')
    ...
    def test_inject_copy_placeholder_ops(self):
        ...
        with core.DeviceScope(cpu_device[0]):
            op = core.CreateOperator(placeholder_send, [weight, bias], [], dst_node=recv_node)
            init_net._net.op.extend([op])
        train_net = core.Net('train_net')
        with core.DeviceScope(cpu_device[1]):
            op = core.CreateOperator(placeholder_recv, [], [weight, bias], src_node=send_node)

...
class ConvTest(hu.HypothesisTestCase):
    ...
    def test_int8_convolution(self, stride, pad, kernel, size, input_channels, output_channels, batch_size, use_bias, gc, dc):
        X = np.random.rand(batch_size, input_channels, size, size).astype(np.float32) - 0.5
        w = np.random.rand(output_channels, input_channels, kernel, kernel).astype(np.float32) - 0.5
        b = np.random.rand(output_channels).astype(np.float32) - 0.5
        old_ws_name = workspace.CurrentWorkspace()
        workspace.SwitchWorkspace('_device_check_', True)
        conv_fp32 = core.CreateOperator('Conv', (['X_fp32', 'w_fp32', 'b_fp32'] if use_bias else ['X_fp32', 'w_fp32']), ['Y_fp32'], stride=stride, pad=pad, kernel=kernel, training_mode=0, device_option=dc[0])
        ...
        w_filler = core.CreateOperator('Int8GivenTensorFill', [], ['w'], shape=w.shape, values=w_bytes.astype(np.uint8).tobytes(), Y_zero_point=w_zero_point, Y_scales=w_scale, device_option=dc[1])
        b_scale = w_scale * X_scale
        b_zero_point = 0
        b_bytes = np.rint([b[i] / b_scale[i] for i in range(b.shape[0])]).astype(np.int32)
        b_filler = core.CreateOperator('Int8GivenIntTensorFill', [], ['b'], shape=b.shape, values=b_bytes, Y_zero_point=b_zero_point, Y_scales=b_scale, device_option=dc[1])
        sw2nhwc = core.CreateOperator('NCHW2NHWC', ['X'], ['X_nhwc'], device_option=dc[1])
        quantize_X = core.CreateOperator('Int8Quantize', ['X_nhwc'], ['X_quantized'], engine='DNNLOWP', device_option=dc[1], Y_zero_point=X_zero_point, Y_scale=X_scale[0])
        conv = core.CreateOperator('Int8Conv', (['X_quantized', 'w', 'b'] if use_bias else ['X_quantized', 'w']), ['Y_quantized'], stride=stride, pad=pad, kernel=kernel, engine='DNNLOWP', device_option=dc[1], Y_zero_point=Y_zero_point, Y_scale=Y_scale[0])
        dequantize_Y = core.CreateOperator('Int8Dequantize', ['Y_quantized'], ['Y_nhwc'], engine='DNNLOWP', device_option=dc[1])

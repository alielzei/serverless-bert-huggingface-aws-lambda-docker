...
def _join_field_name(prefix, suffix):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.schema._join_field_name', '_join_field_name(prefix, suffix)', {'FIELD_SEPARATOR': FIELD_SEPARATOR, 'prefix': prefix, 'suffix': suffix}, 1)

def _normalize_field(field_or_type_or_blob, keep_blobs=True):
    """Clones/normalizes a field before adding it to a container."""
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema._normalize_field', '_normalize_field(field_or_type_or_blob, keep_blobs=True)', {'Field': Field, 'np': np, 'Scalar': Scalar, 'field_or_type_or_blob': field_or_type_or_blob, 'keep_blobs': keep_blobs}, 1)
FeatureSpec = namedtuple('FeatureSpec', ['feature_type', 'feature_names', 'feature_ids', 'feature_is_request_only', 'desired_hash_size', 'feature_to_index'])
...
class Field(object):
    ...
    def __eq__(self, other):
        ...
        return (self.field_names() == other.field_names() and self.field_types() == other.field_types() and self.field_metadata() == other.field_metadata())
...
class List(Field):
    ...
    def clone(self, keep_blobs=True):
        return type(self)(_normalize_field(self._items, keep_blobs=keep_blobs), _normalize_field(self.lengths, keep_blobs=keep_blobs))
...
class ListWithEvicted(List):
    ...
    def field_names(self):
        value_fields = self._items.field_names()
        return ['lengths'] + [_join_field_name('values', v) for v in value_fields] + ['_evicted_values']
    ...
    def clone(self, keep_blobs=True):
        return type(self)(_normalize_field(self._items, keep_blobs=keep_blobs), _normalize_field(self.lengths, keep_blobs=keep_blobs), _normalize_field(self._evicted_values, keep_blobs=keep_blobs))
...
class Struct(Field):
    ...
    def __init__(self, *fields):
        ...
        for field in fields:
            ...
            assert field[0] != 'lengths', 'Struct cannot contain a field named `lengths`.'
        ...
        for (name, field) in fields:
            ...
            if name not in self.fields:
                ...
                continue
            if (not isinstance(field, Struct) or not isinstance(self.fields[name], Struct)):
    ...
    def clone(self, keep_blobs=True):
        normalized_fields = [(k, _normalize_field(v, keep_blobs=keep_blobs)) for (k, v) in viewitems(self.fields)]
    ...
    def __getitem__(self, item):
        ...
        if (isinstance(item, list) or isinstance(item, tuple)):
            ...
            return Struct(*[((keys[k] if isinstance(k, int) else k), self[k]) for k in item])
    ...
    def __add__(self, other):
        ...
        for (name, right_field) in other.get_children():
            ...
            if not ((isinstance(left_field, Struct) and isinstance(right_field, Struct))):
                raise TypeError('Type of left_field, ' + str(type(left_field)) + ', and type of right_field, ' + str(type(right_field)) + ', must both the Struct to allow merging of the field, ' + name)
        ...
        for (name, right_field) in other.get_children():
            if name in children:
                ...
                if type(left_field) == type(right_field):
                    ...
                    raise TypeError('Type of left_field, ' + str(type(left_field)) + ', is not the same as that of right_field, ' + str(type(right_field)) + ', yet they have the same field name, ' + name)
...
class Scalar(Field):
    ...
    def clone(self, keep_blobs=True):
        return Scalar(dtype=self._original_dtype, blob=(self._blob if keep_blobs else None), metadata=self._metadata)
    ...
    def set_metadata(self, value):
        assert isinstance(value, Metadata), 'metadata must be Metadata, got {}'.format(type(value))
    ...
    def _validate_metadata(self):
        ...
        if (self._metadata.categorical_limit is not None and self.dtype is not None):
            assert np.issubdtype(self.dtype, np.integer), '`categorical_limit` can be specified only in integral ' + 'fields but got {}'.format(self.dtype)
    ...
    def set_value(self, blob, throw_on_type_mismatch=False, unsafe=False):
        ...
        if (self.dtype.base != np.void and throw_on_type_mismatch):
            ...
            assert blob.dtype.base == self.dtype.base, 'Expected {}, got {}'.format(self.dtype.base, blob.dtype.base)
    ...
    def set(self, dtype=None, blob=None, metadata=None, unsafe=False):
        ...
        """
        if not unsafe:
            logger.warning('Scalar should be considered immutable. Only call Scalar.set() on newly created Scalar with unsafe=True. This will become an error soon.')
        if (blob is not None and isinstance(blob, basestring)):
            raise ValueError('Passing str blob to Scalar.set() is ambiguous. Do either set(blob=np.array(blob)) or set(blob=BlobReference(blob))')
        ...
        if dtype is not None:
            if (isinstance(dtype, tuple) and dtype[0] == np.void):
                raise TypeError('Cannot set the Scalar with type {} for blob {}.If this blob is the output of some operation, please verify the input of that operation has proper type.'.format(dtype, blob))
        ...
        if (blob is not None and not isinstance(blob, BlobReference)):
            ...
            if (dtype is not None and dtype != np.void):
                ...
                assert isinstance(blob, np.ndarray), 'Invalid blob type: %s' % str(type(blob))
        ...
        assert not dtype.fields, 'Cannot create Scalar with a structured dtype. ' + 'Use from_dtype instead.'
        self.dtype = dtype
        self._blob = blob
    ...
    def _pprint_impl(self, indent, str_buffer):
        str_buffer.write('  ' * indent + 'Scalar({!r}, {!r}, {!r})'.format(self.dtype, self._blob, self._metadata) + '\n')
...
def Map(keys, values, keys_name='keys', values_name='values', lengths_blob=None):
    ...
    """
    return List(Struct((keys_name, keys), (values_name, values)), lengths_blob=lengths_blob)

def MapWithEvicted(keys, values, keys_name='keys', values_name='values', lengths_blob=None, evicted_values=None):
    """A map with extra field evicted_values
    """
    return ListWithEvicted(Struct((keys_name, keys), (values_name, values)), lengths_blob=lengths_blob, evicted_values=evicted_values)

def NamedTuple(name_prefix, *fields):
    return Struct(*[('%s_%d' % (name_prefix, i), field) for (i, field) in enumerate(fields)])
...
def RawTuple(num_fields, name_prefix='field'):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.RawTuple', "RawTuple(num_fields, name_prefix='field')", {'NamedTuple': NamedTuple, 'np': np, 'num_fields': num_fields, 'name_prefix': name_prefix}, 1)
...
def from_dtype(dtype, _outer_shape=()):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.from_dtype', 'from_dtype(dtype, _outer_shape=())', {'np': np, 'Scalar': Scalar, 'from_dtype': from_dtype, 'Struct': Struct, 'dtype': dtype, '_outer_shape': _outer_shape}, 1)
...
def from_column_list(col_names, col_types=None, col_blobs=None, col_metadata=None):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.from_column_list', 'from_column_list(col_names, col_types=None, col_blobs=None, col_metadata=None)', {'_SchemaNode': _SchemaNode, 'FIELD_SEPARATOR': FIELD_SEPARATOR, 'Scalar': Scalar, 'col_names': col_names, 'col_types': col_types, 'col_blobs': col_blobs, 'col_metadata': col_metadata}, 1)
...
def from_blob_list(schema, values, throw_on_type_mismatch=False):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.from_blob_list', 'from_blob_list(schema, values, throw_on_type_mismatch=False)', {'Field': Field, 'BlobReference': BlobReference, 'schema': schema, 'values': values, 'throw_on_type_mismatch': throw_on_type_mismatch}, 1)

def as_record(value):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.as_record', 'as_record(value)', {'Field': Field, 'basestring': basestring, 'Struct': Struct, 'as_record': as_record, 'Tuple': Tuple, 'viewitems': viewitems, '_normalize_field': _normalize_field, 'value': value}, 1)
...
def FetchRecord(blob_record, ws=None, throw_on_type_mismatch=False):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.FetchRecord', 'FetchRecord(blob_record, ws=None, throw_on_type_mismatch=False)', {'workspace': workspace, 'Field': Field, 'BlobReference': BlobReference, 'from_blob_list': from_blob_list, 'blob_record': blob_record, 'ws': ws, 'throw_on_type_mismatch': throw_on_type_mismatch}, 1)
...
def FeedRecord(blob_record, arrays, ws=None):
    ...
    """
    import custom_funtemplate
    custom_funtemplate.rewrite_template('caffe2.python.schema.FeedRecord', 'FeedRecord(blob_record, arrays, ws=None)', {'workspace': workspace, 'Field': Field, 'BlobReference': BlobReference, 'blob_record': blob_record, 'arrays': arrays, 'ws': ws}, 0)
...
def NewRecord(net, schema):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.NewRecord', 'NewRecord(net, schema)', {'Scalar': Scalar, 'Field': Field, 'from_blob_list': from_blob_list, 'net': net, 'schema': schema}, 1)
...
def ConstRecord(net, array_record):
    ...
    """
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.ConstRecord', 'ConstRecord(net, array_record)', {'NewRecord': NewRecord, 'net': net, 'array_record': array_record}, 1)

def InitEmptyRecord(net, schema_or_record, enforce_types=False):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.InitEmptyRecord', 'InitEmptyRecord(net, schema_or_record, enforce_types=False)', {'NewRecord': NewRecord, 'data_type_for_dtype': data_type_for_dtype, 'logger': logger, 'net': net, 'schema_or_record': schema_or_record, 'enforce_types': enforce_types}, 1)
_DATA_TYPE_FOR_DTYPE = [(np.str, core.DataType.STRING), (np.float16, core.DataType.FLOAT16), (np.float32, core.DataType.FLOAT), (np.float64, core.DataType.DOUBLE), (np.bool, core.DataType.BOOL), (np.int8, core.DataType.INT8), (np.int16, core.DataType.INT16), (np.int32, core.DataType.INT32), (np.int64, core.DataType.INT64), (np.uint8, core.DataType.UINT8), (np.uint16, core.DataType.UINT16)]

def is_schema_subset(schema, original_schema):
    return set(schema.field_names()).issubset(set(original_schema.field_names()))

def equal_schemas(schema, original_schema, check_field_names=True, check_field_types=True, check_field_metas=False):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.equal_schemas', 'equal_schemas(schema, original_schema, check_field_names=True, check_field_types=True, check_field_metas=False)', {'Field': Field, 'schema': schema, 'original_schema': original_schema, 'check_field_names': check_field_names, 'check_field_types': check_field_types, 'check_field_metas': check_field_metas}, 1)

def schema_check(schema, previous=None):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.schema_check', 'schema_check(schema, previous=None)', {'as_record': as_record, 'equal_schemas': equal_schemas, 'schema': schema, 'previous': previous}, 1)

def data_type_for_dtype(dtype):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.data_type_for_dtype', 'data_type_for_dtype(dtype)', {'_DATA_TYPE_FOR_DTYPE': _DATA_TYPE_FOR_DTYPE, 'dtype': dtype}, 1)

def dtype_for_core_type(core_type):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.schema.dtype_for_core_type', 'dtype_for_core_type(core_type)', {'_DATA_TYPE_FOR_DTYPE': _DATA_TYPE_FOR_DTYPE, 'core_type': core_type}, 1)

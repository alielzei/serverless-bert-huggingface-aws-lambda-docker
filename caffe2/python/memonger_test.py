...
def has_blob(proto, needle):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.memonger_test.has_blob', 'has_blob(proto, needle)', {'proto': proto, 'needle': needle}, 1)

def count_blobs(proto):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.memonger_test.count_blobs', 'count_blobs(proto)', {'proto': proto}, 1)
...
class MemongerTest(hu.HypothesisTestCase):
    ...
    def test_gradient_optim(self, input_dim, output_dim, batch_size):
        ...
        optim_proto = memonger.share_grad_blobs(m.net, ['name_x/loss'], set(viewvalues(m.param_to_grad)), 'name_x/', share_activations=False)
        blobs_after = count_blobs(optim_proto)
        self.assertLess(blobs_after, blobs_before)
        optim_proto_wacts = memonger.share_grad_blobs(m.net, ['name_x/loss'], set(viewvalues(m.param_to_grad)), 'name_x/', share_activations=True, dont_share_blobs=set([str(input_to_grad['name_x/fc1_w'])]))
    ...
    def test_memonger_mix_cpu_gpu(self):
        ...
        blobs_before = count_blobs(m.net.Proto())
        optim_proto = memonger.share_grad_blobs(m.net, ['loss'], set(viewvalues(m.param_to_grad)), '', share_activations=True, dont_share_blobs=set())
        blobs_after = count_blobs(optim_proto)
        self.assertLess(blobs_after, blobs_before)
    ...
    def test_gradient_optim_tree(self, input_dim, output_dim, batch_size):
        ...
        blobs_before = count_blobs(m.net.Proto())
        optim_proto = memonger.share_grad_blobs(m.net, ['name_x/loss1', 'name_x/loss2'], set(viewvalues(m.param_to_grad)), 'name_x', share_activations=True, dont_share_blobs=set(['name_x/fc6', 'name_x/fc5', str(input_to_grad['name_x/fc1_w'])]))
        blobs_after = count_blobs(optim_proto)
        self.assertLess(blobs_after, blobs_before)
    ...
    def test_rnn(self):
        ...
        optim_proto = memonger.share_grad_blobs(model.net, ['loss'], set(viewvalues(model.param_to_grad)), '', share_activations=True, dont_share_blobs=set())
        blobs_after = count_blobs(optim_proto)
        self.assertLess(blobs_after, blobs_before)

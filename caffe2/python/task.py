...
def _merge_node_kwargs(a, b):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.task._merge_node_kwargs', '_merge_node_kwargs(a, b)', {'copy': copy, 'a': a, 'b': b}, 1)
...
class Cluster(object):
    ...
    def add_node(self, node):
        ...
        self._node_kwargs[str(node)] = _merge_node_kwargs(node.kwargs(), self._node_kwargs.get(str(node)))
    ...
    def __repr__(self):
        return 'Cluster(nodes={}, node_kwargs={})'.format(self.nodes(), self.node_kwargs())
...
def get_setup_nets(key, steps_or_nets, target):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.task.get_setup_nets', 'get_setup_nets(key, steps_or_nets, target)', {'core': core, 'key': key, 'steps_or_nets': steps_or_nets, 'target': target}, 2)

def add_setup_steps(step, init_nets, exit_nets, name):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.task.add_setup_steps', 'add_setup_steps(step, init_nets, exit_nets, name)', {'core': core, 'step': step, 'init_nets': init_nets, 'exit_nets': exit_nets, 'name': name}, 1)
...
class TaskGroup(object):
    ...
    def add(self, task):
        ...
        assert (self._workspace_type is None or task._workspace_type is None or self._workspace_type == task._workspace_type)
    ...
    def report_net(self, net=None, node=None, report_interval=5):
        ...
        if node not in self._report_nets:
            self._report_nets[node] = ((net if net else core.Net('%s/reporter' % node)), report_interval)
    ...
    def tasks_by_node(self, node_remap=None):
        ...
        if self._tasks_by_node is not None:
            ...
            assert prev_node_map == node_map, 'Cannot call tasks_by_node multiple times.'
        ...
        for (node, tasks) in viewitems(tasks_by_node):
            ...
            (node_inits, node_exits) = get_setup_nets(TaskGroup.LOCAL_SETUP, [t.get_step() for t in tasks] + report_steps, self)
            steps = report_steps
            outputs = []
            ...
            if len(steps) == 1:
                ...
                step = core.execution_step('%s:body' % node, steps, concurrent_substeps=True)
            ...
            Task(node=node, step=step, outputs=outputs, name='grouped_by_node', group=grouped_by_node, workspace_type=grouped_workspace_type)
    ...
    def __repr__(self):
        return 'TaskGroup(tasks={}, workspace_type={}, remote_nets={})'.format(self.tasks(), self.workspace_type(), self.remote_nets())
...
class SetupNets(object):
    ...
    def __repr__(self):
        return 'SetupNets(init_nets={}, exit_nets={})'.format(self.init_nets, self.exit_nets)

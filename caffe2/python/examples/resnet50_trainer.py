...
def AddNullInput(model, reader, batch_size, img_size, dtype):
    ...
    custom_funtemplate.rewrite_template('caffe2.python.examples.resnet50_trainer.AddNullInput', 'AddNullInput(model, reader, batch_size, img_size, dtype)', {'core': core, 'model': model, 'reader': reader, 'batch_size': batch_size, 'img_size': img_size, 'dtype': dtype}, 0)

def SaveModel(args, train_model, epoch, use_ideep):
    import custom_funtemplate
    custom_funtemplate.rewrite_template('caffe2.python.examples.resnet50_trainer.SaveModel', 'SaveModel(args, train_model, epoch, use_ideep)', {'pred_exp': pred_exp, 'data_parallel_model': data_parallel_model, 'args': args, 'train_model': train_model, 'epoch': epoch, 'use_ideep': use_ideep}, 0)
...
def LoadModel(path, model, use_ideep):
    ...
    """
    import custom_funtemplate
    custom_funtemplate.rewrite_template('caffe2.python.examples.resnet50_trainer.LoadModel', 'LoadModel(path, model, use_ideep)', {'log': log, 'pred_exp': pred_exp, 'core': core, 'pred_utils': pred_utils, 'predictor_constants': predictor_constants, 'workspace': workspace, 'caffe2_pb2': caffe2_pb2, 'path': path, 'model': model, 'use_ideep': use_ideep}, 0)

...
class RecurrentNetworkTest(serial.SerializedTestCase):
    ...
    def test_sum_mul(self, T, n, d):
        ...
        (input_blob, initial_input_blob) = model.net.AddExternalInputs('input', 'initial_input')
        ...
        self.simple_rnn(T, n, d, model, step, input_t, output_t, output_t_prev, input_blob, initial_input_blob)
    
    @serial.given(T=st.integers(1, 4), n=st.integers(1, 5), d=st.integers(1, 5))
    ...
    def test_extract(self, T, n, d):
        ...
        recurrent.recurrent_net(net=model.net, cell_net=step.net, inputs=[(input_t, input_blob)], initial_cell_inputs=[(output_t_prev, initial_input_blob)], links={output_t_prev: output_t}, scope='test_rnn_sum_mull')
        ...
        for i in range(T):
            ...
            self.assertTrue(blob_name in retrieved_blobs, 'blob extraction failed on timestep {}                    . \n\n Extracted Blobs: {} \n\n Looking for {}                    .'.format(i, retrieved_blobs, blob_name))
    
    def simple_rnn(self, T, n, d, model, step, input_t, output_t, output_t_prev, input_blob, initial_input_blob):
        ...
        self.assertReferenceChecks(device_option=hu.cpu_do, op=op, inputs=inputs, reference=reference, output_to_grad=op.output[0], outputs_to_check=[0, 1])
        self.assertGradientChecks(device_option=hu.cpu_do, op=op, inputs=inputs, outputs_to_check=0, outputs_with_grads=[0], threshold=0.01, stepsize=0.005)
    
    def _convolution_1d(self, model, inputs, conv_window, conv_filter, conv_bias, output_name, left_pad):
        if left_pad:
            padding_width = conv_window - 1
        else:
            padding_width = 0
        inputs_transposed = model.net.Transpose(inputs, 'inputs_transposed', axes=[1, 0, 2])
        inputs_transposed_4d = model.net.ExpandDims(inputs_transposed, 'inputs_transposed_4d', dims=[1])
        output_transposed_4d = model.net.Conv([inputs_transposed_4d, conv_filter, conv_bias], output_name + '_transposed_4d', kernel_h=1, kernel_w=conv_window, order='NHWC', pad_t=0, pad_l=padding_width, pad_b=0, pad_r=0)
        output_transposed = model.net.Squeeze(output_transposed_4d, output_name + '_transposed', dims=[1])
        output = model.net.Transpose(output_transposed, output_name, axes=[1, 0, 2])
        return output
    
    @given(sequence_length=st.integers(3, 7), conv_window=st.integers(1, 3), batch_size=st.integers(1, 5), state_size=st.integers(1, 5))
    def test_stateful_convolution_forward_only(self, sequence_length, conv_window, batch_size, state_size):
        """
    ...
    """
        ...
        fake_inputs = model.param_init_net.UniformFill([], 'fake_inputs', min=-1.0, max=1.0, shape=[sequence_length, batch_size, state_size])
        initial_input_state = model.param_init_net.ConstantFill([], 'initial_input_state', value=0.0, shape=[conv_window - 1, batch_size, state_size])
        initial_output_state = model.param_init_net.ConstantFill([], 'initial_output_state', value=0.0, shape=[1, batch_size, state_size])
        step_model = ModelHelper(name='step_model', param_model=model)
        (fake_input_t, timestep, input_state_t_prev) = step_model.net.AddExternalInputs('fake_input_t', 'timestep', 'input_state_t_prev')
        conv_filter = step_model.param_init_net.XavierFill([], 'conv_filter', shape=[state_size, 1, conv_window, state_size])
        conv_bias = step_model.param_init_net.ConstantFill([], 'conv_bias', shape=[state_size], value=0.0)
        step_model.params.extend([conv_filter, conv_bias])
        input_state_t = step_model.net.UniformFill([], 'input_state_t', min=-1.0, max=1.0, shape=[1, batch_size, state_size])
        output_state_t = self._convolution_1d(model=step_model, inputs=input_state_t_prev, conv_window=conv_window, conv_filter=conv_filter, conv_bias=conv_bias, output_name='output_state_t', left_pad=False)

...
class TestTransposeOp(serial.SerializedTestCase):
    ...
    @serial.given(X=hu.tensor(dtype=np.float32), use_axes=st.booleans(), **hu.gcs)
    ...
    def test_transpose_cudnn(self, X, use_axes, gc, dc):
        ...
        np.random.shuffle(axes)
        if use_axes:
            op = core.CreateOperator('Transpose', ['X'], ['Y'], axes=axes, engine='CUDNN', device_option=hu.cuda_do)
        else:
            op = core.CreateOperator('Transpose', ['X'], ['Y'], engine='CUDNN', device_option=hu.cuda_do)

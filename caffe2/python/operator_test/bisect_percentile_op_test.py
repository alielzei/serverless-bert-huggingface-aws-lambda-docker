...
class TestBisectPercentileOp(hu.HypothesisTestCase):
    ...
    @given(N=st.integers(min_value=20, max_value=100), lengths=st.lists(elements=st.integers(min_value=2, max_value=10), min_size=2, max_size=5), max_value=st.integers(min_value=100, max_value=1000), discrete=st.booleans(), p=st.floats(min_value=0, max_value=0.9), **hu.gcs_cpu_only)
    def test_bisect_percentil_op_large(self, N, lengths, max_value, discrete, p, gc, dc):

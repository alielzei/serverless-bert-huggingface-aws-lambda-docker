...
def _data_and_scale(data_min_size=4, data_max_size=10, examples_min_number=1, examples_max_number=4, dtype=np.float32, elements=None):
    ...
    return params_.flatmap(lambda param_: st.tuples(hu.arrays([param_[0], param_[1]], dtype=dtype), hu.arrays([param_[0]], dtype=param_[2], elements=(st.floats(0.0, 10000.0) if param_[2] in [np.float32] else st.integers(0, 10000)))))

def divide_by_square_root(data, scale):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.operator_test.square_root_divide_op_test.divide_by_square_root', 'divide_by_square_root(data, scale)', {'np': np, 'math': math, 'data': data, 'scale': scale}, 1)

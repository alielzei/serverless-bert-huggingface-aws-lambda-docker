...
class TestLocallyConnectedOp(serial.SerializedTestCase):
    ...
    @serial.given(N=st.integers(1, 3), C=st.integers(1, 3), H=st.integers(1, 5), W=st.integers(1, 5), M=st.integers(1, 3), kernel=st.integers(1, 3), op_name=st.sampled_from(['LC', 'LC2D']), order=st.sampled_from(['NCHW', 'NHWC']), use_bias=st.booleans(), **hu.gcs)

...
class TestLoadSaveBase(test_util.TestCase):
    ...
    def load_save(self, src_device_type, src_gpu_id, dst_device_type, dst_gpu_id):
        ...
        try:
            ...
            op = core.CreateOperator('Save', [str(i) for i in range(len(arrays))], [], absolute_path=1, db=os.path.join(tmp_folder, 'db'), db_type=self._db_type)
            ...
            def _LoadTest(keep_device, device_type, gpu_id, blobs, loadAll):
                ...
                op = core.CreateOperator('Load', [], blobs, absolute_path=1, db=os.path.join(tmp_folder, 'db'), db_type=self._db_type, device_option=dst_device_option, keep_device=keep_device, load_all=loadAll)
                self.assertTrue(workspace.RunOperatorOnce(op))
                for (i, arr) in enumerate(arrays):
                    self.assertTrue(workspace.HasBlob(str(i)))

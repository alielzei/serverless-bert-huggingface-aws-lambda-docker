...
class TestReduceOps(serial.SerializedTestCase):
    ...
    def run_reduce_op_test_impl(self, op_name, X, axes, keepdims, ref_func, gc, dc):
        if axes is None:
            ...
            op = core.CreateOperator(op_name, ['X'], ['Y'], axes=axes, keepdims=keepdims)
    ...
    def run_reduce_op_test(self, op_name, X, keepdims, num_axes, ref_func, gc, dc):
        ...
        if num_dims < num_axes:
            self.run_reduce_op_test_impl(op_name, X, range(num_dims), keepdims, ref_func, gc, dc)
        else:
            for axes in it.combinations(range(num_dims), num_axes):
                self.run_reduce_op_test_impl(op_name, X, axes, keepdims, ref_func, gc, dc)
    
    @serial.given(X=hu.tensor(max_dim=3, dtype=np.float32), keepdims=st.booleans(), num_axes=st.integers(1, 3), **hu.gcs)
    ...
    @given(n=st.integers(0, 5), m=st.integers(0, 5), k=st.integers(0, 5), t=st.integers(0, 5), keepdims=st.booleans(), num_axes=st.integers(1, 3), **hu.gcs)
    ...
    @given(n=st.integers(1, 3), m=st.integers(1, 3), k=st.integers(1, 3), keepdims=st.booleans(), num_axes=st.integers(1, 3), **hu.gcs_cpu_only)
    ...
    @serial.given(n=st.integers(1, 5), m=st.integers(1, 5), k=st.integers(1, 5), keepdims=st.booleans(), num_axes=st.integers(1, 3), **hu.gcs_cpu_only)
...
def getNorm(p):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.operator_test.reduce_ops_test.getNorm', 'getNorm(p)', {'np': np, 'p': p}, 1)

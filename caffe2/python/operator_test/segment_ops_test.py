...
def max_grad(grad_out, outputs, inputs):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.operator_test.segment_ops_test.max_grad', 'max_grad(grad_out, outputs, inputs)', {'np': np, 'grad_out': grad_out, 'outputs': outputs, 'inputs': inputs}, 1)
REFERENCES_ALL = [('Sum', partial(np.sum, axis=0), sum_grad), ('Mean', partial(np.mean, axis=0), mean_grad)]
REFERENCES_SORTED = [('RangeSum', partial(np.sum, axis=0), sum_grad), ('RangeLogSumExp', logsumexp, logsumexp_grad), ('RangeLogMeanExp', logmeanexp, logsumexp_grad), ('RangeMean', mean, mean_grad), ('RangeMax', max_fwd, max_grad)]
REFERENCES_LENGTHS_ONLY = [('Max', partial(np.amax, axis=0), max_grad)]

def sparse_lengths_weighted_sum_ref(D, W, I, L):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.operator_test.segment_ops_test.sparse_lengths_weighted_sum_ref', 'sparse_lengths_weighted_sum_ref(D, W, I, L)', {'np': np, 'D': D, 'W': W, 'I': I, 'L': L}, 1)

def sparse_lengths_weighted_sum_grad_ref(GO, fwd_out, fwd_in, grad_on_weights=False):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.operator_test.segment_ops_test.sparse_lengths_weighted_sum_grad_ref', 'sparse_lengths_weighted_sum_grad_ref(GO, fwd_out, fwd_in, grad_on_weights=False)', {'np': np, 'GO': GO, 'fwd_out': fwd_out, 'fwd_in': fwd_in, 'grad_on_weights': grad_on_weights}, 1)

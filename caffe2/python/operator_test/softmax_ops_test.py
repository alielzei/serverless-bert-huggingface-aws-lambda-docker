...
class TestSoftmaxOps(serial.SerializedTestCase):
    ...
    @serial.given(n=st.sampled_from([0, 2, 4, 71, 103, 555, 751, 1201]), D=st.sampled_from([0, 4, 8, 64, 79, 256, 333, 1000]), engine=st.sampled_from([None, 'CUDNN']), **hu.gcs)
    ...
    @serial.given(n=st.integers(2, 10), D=st.integers(4, 16), only_loss=st.booleans(), **hu.gcs)
    ...
    @serial.given(n=st.integers(2, 5), D=st.integers(2, 4), weighted=st.booleans(), **hu.gcs)

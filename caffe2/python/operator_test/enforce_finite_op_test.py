...
class TestEnforceFinite(hu.HypothesisTestCase):
    ...
    @given(X=hu.tensor(min_value=0, elements=st.floats(allow_nan=True, allow_infinity=True)), **hu.gcs)
    ...
    @given(X=hu.tensor(elements=st.floats(min_value=0, max_value=10, allow_nan=False, allow_infinity=False)), **hu.gcs)

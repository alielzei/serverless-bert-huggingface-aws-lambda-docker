...
struct = [[0, 2, ['node4', 'node3'], 'node5'], [2, 2, ['node1', 'node2'], 'node4'], [4, 3, [0, 1, 2], 'node1'], [7, 2, [3, 4], 'node2'], [9, 4, [5, 6, 7, 8], 'node3']]
    ...
    def test_hsm_search(self):
        ...
        for i in range(names.shape[0]):
            for j in range(names.shape[1]):
                if names[i][j]:
                    self.assertEquals(names[i][j], p_names[i][j].item().encode('utf-8'))

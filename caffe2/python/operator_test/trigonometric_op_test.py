...
class TestTrigonometricOp(serial.SerializedTestCase):
    ...
    @serial.given(X=hu.tensor(elements=st.floats(min_value=-0.7, max_value=0.7)), **hu.gcs)
    ...
    @serial.given(X=hu.tensor(elements=st.floats(min_value=-100, max_value=100)), **hu.gcs)
    def test_atan(self, X, gc, dc):
        self.assertTrigonometricChecks('Atan', X, lambda x: (np.arctan(X), ), gc, dc)
    
    @serial.given(X=hu.tensor(elements=st.floats(min_value=-0.5, max_value=0.5)), **hu.gcs)

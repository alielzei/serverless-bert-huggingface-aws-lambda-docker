...
def _string_lists(alphabet=None):
    return st.lists(elements=st.text(alphabet=alphabet, average_size=3), min_size=0, max_size=3)
...
class TestStringOps(serial.SerializedTestCase):
    ...
    def test_string_prefix(self, strings):
        length = 3
        strings = np.array([a.encode('utf-8') for a in strings], dtype=np.object)
    ...
    def test_string_starts_with(self, strings):
        prefix = 'a'
        strings = np.array([str(a) for a in strings], dtype=np.object)

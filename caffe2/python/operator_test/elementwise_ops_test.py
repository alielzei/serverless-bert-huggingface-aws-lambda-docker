...
class TestElementwiseOps(hu.HypothesisTestCase):
    ...
    @given(X=hu.tensor(elements=st.floats(0.1, 10), min_value=0), inplace=st.booleans(), **hu.gcs)
    ...
    @given(X=hu.tensor(elements=st.floats(0.1, 10.0), dtype=np.float32), inplace=st.booleans(), **hu.gcs)
    ...
    @given(X=hu.tensor(elements=st.floats(1.0, 10.0), dtype=np.float32), in_place=st.booleans(), **hu.gcs)
    ...
    @given(X=hu.tensor(dtype=np.float32), inplace=st.booleans(), alpha=st.floats(min_value=-100.0, max_value=100.0), beta=st.floats(min_value=-100.0, max_value=100.0), engine=st.sampled_from(['']), **hu.gcs)
    ...
    @given(X=hu.tensor(elements=st.floats(0.5, 2), dtype=np.float32), inplace=st.booleans(), **hu.gcs)
    def test_reciprocal(self, X, inplace, gc, dc):
        ...
        self.assertDeviceChecks(dc, op, [X], [0])
        self.assertGradientChecks(gc, op, [X], 0, [0], stepsize=0.001, threshold=0.05)

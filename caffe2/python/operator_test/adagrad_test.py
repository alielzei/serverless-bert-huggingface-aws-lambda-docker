...
from caffe2.python.operator_test.adagrad_test_helper import adagrad_sparse_test_helper, ref_adagrad
from hypothesis import HealthCheck, given, settings


class TestAdagrad(serial.SerializedTestCase):
    
    @serial.given(inputs=hu.tensors(n=3), lr=st.floats(min_value=0.01, max_value=0.99, allow_nan=False, allow_infinity=False), epsilon=st.floats(min_value=0.01, max_value=0.99, allow_nan=False, allow_infinity=False), **hu.gcs)
    def test_adagrad(self, inputs, lr, epsilon, gc, dc):
        (param, momentum, grad) = inputs
        momentum = np.abs(momentum)
        lr = np.array([lr], dtype=np.float32)
        op = core.CreateOperator('Adagrad', ['param', 'momentum', 'grad', 'lr'], ['param', 'momentum'], epsilon=epsilon, device_option=gc)
        self.assertReferenceChecks(gc, op, [param, momentum, grad, lr], functools.partial(ref_adagrad, epsilon=epsilon))
    ...
    def test_adagrad_output_effective_lr(self, inputs, lr, epsilon, gc, dc):
        ...
        momentum = np.abs(momentum)
        lr = np.array([lr], dtype=np.float32)
        op = core.CreateOperator('Adagrad', ['param', 'momentum', 'grad', 'lr'], ['param', 'momentum', 'effective_lr'], epsilon=epsilon, device_option=gc)
        self.assertReferenceChecks(gc, op, [param, momentum, grad, lr], functools.partial(ref_adagrad, epsilon=epsilon, output_effective_lr=True))
    ...
    @given(inputs=hu.tensors(n=3), lr=st.floats(min_value=0.01, max_value=0.99, allow_nan=False, allow_infinity=False), epsilon=st.floats(min_value=0.01, max_value=0.99, allow_nan=False, allow_infinity=False), **hu.gcs)
    ...
    @serial.given(inputs=hu.tensors(n=2), lr=st.floats(min_value=0.01, max_value=0.99, allow_nan=False, allow_infinity=False), epsilon=st.floats(min_value=0.01, max_value=0.99, allow_nan=False, allow_infinity=False), **hu.gcs)
    ...
    def test_row_wise_sparse_adagrad(self, inputs, lr, epsilon, gc, dc):
        adagrad_sparse_test_helper(self, inputs, lr, epsilon, None, functools.partial(ref_adagrad, row_wise=True), gc, dc, row_wise=True)

...
def _one_hots():
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.operator_test.one_hot_ops_test._one_hots', '_one_hots()', {'st': st}, 1)
...
class TestOneHotOps(serial.SerializedTestCase):
    ...
    @serial.given(x=hu.tensor(min_dim=2, max_dim=2, dtype=np.float32, elements=st.integers(min_value=-5, max_value=5)), seed=st.integers(min_value=0, max_value=1000), **hu.gcs_cpu_only)

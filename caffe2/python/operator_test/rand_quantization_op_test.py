...
class TestFloatToFusedRandRowwiseQuantized(hu.HypothesisTestCase):
    ...
    @given(X=hu.tensor(min_dim=2, max_dim=2, min_value=1, max_value=17), bitwidth_=st.sampled_from([1, 2, 4, 8]), random_=st.booleans(), **hu.gcs)
    @example(X=np.array([[0.0, 0.0, 0.0, 0.264019]]).astype(np.float32), bitwidth_=2, random_=False, **hu.gcs)
    ...
    def test_rand_quantization(self, X, bitwidth_, random_, gc, dc):
        ...
        def quantization_ref(X):
            ...
            for r in range(in_shape[0]):
                ...
                for c in range(in_shape[1]):
                    ...
                    thetimes = (fval - min_fval) / (gap + 1e-08)
        ...
        def get_allowed_errors(X):
            ...
            for r in range(X.shape[0]):
                min_fval = np.amin(X[r])
                max_fval = np.amax(X[r])
                gap = (max_fval - min_fval) / (2**bitwidth_ - 1) + 1e-08
                out[r] = gap
            return out
        ...
        def dec_ref(Y):
            ...
            for r in range(in_shape[0]):
                ...
                gap = (max_fval - min_fval) / (2**bitwidth - 1.0) + 1e-08
                for out_c in range(output_cols):
                    bit_start = out_c // segment * bitwidth
                    out[r][out_c] = min_fval + gap * (Y[r][10 + out_c % segment] >> bit_start & mask)
            print('pdecX:\n{}\n'.format(out))
            return (out, )
        enc_op = core.CreateOperator('FloatToFusedRandRowwiseQuantized', ['X'], ['Y'], bitwidth=bitwidth_, random=random_)
        ...
        if not random_:
            for r in range(Y.shape[0]):
                np.testing.assert_almost_equal(struct.unpack('f', Y[r][2:6])[0], struct.unpack('f', pY[r][2:6])[0])
                np.testing.assert_almost_equal(struct.unpack('f', Y[r][6:10])[0], struct.unpack('f', pY[r][6:10])[0])
        ...
        print('decX:\n{}\n'.format(decX))
        if random_:
            err_thre = get_allowed_errors(X) + 1e-06
        else:
            err_thre = get_allowed_errors(X) / 2.0 + 1e-06
        err = decX - X
        print('err_thre:\n{}\n'.format(err_thre))
        print('err:\n{}\n'.format(err))
        ...
        if (random_ and test_stochastic_quantization):
            ...
            print('X_avg:\n{}'.format(X_avg))
            print('X    :\n{}'.format(X))
            np.testing.assert_array_less(np.absolute(X_avg - X), 0.05 * get_allowed_errors(X) + 0.0001)

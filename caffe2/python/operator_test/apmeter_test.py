...
def calculate_ap(predictions, labels):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.operator_test.apmeter_test.calculate_ap', 'calculate_ap(predictions, labels)', {'np': np, 'predictions': predictions, 'labels': labels}, 1)


class TestAPMeterOps(hu.HypothesisTestCase):
    
    @given(predictions=hu.arrays(dims=[10, 3], elements=st.floats(allow_nan=False, allow_infinity=False, min_value=0.1, max_value=1)), labels=hu.arrays(dims=[10, 3], dtype=np.int32, elements=st.integers(min_value=0, max_value=1)), **hu.gcs_cpu_only)

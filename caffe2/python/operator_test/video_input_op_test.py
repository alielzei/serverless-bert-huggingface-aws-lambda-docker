...
class VideoInputOpTest(unittest.TestCase):
    ...
    def test_rgb_use_shorter_edge(self):
        ...
        model = model_helper.ModelHelper(name='Video Loader from LMDB')
        reader = model.CreateDB('sample', db=video_db_dir, db_type='lmdb')
        model.net.VideoInput(reader, ['data', 'label'], name='data', batch_size=batch_size, clip_per_video=1, crop_size=112, scale_w=171, scale_h=128, length_of=8, frame_gap_of=1, decode_type=0, video_res_type=1, get_rgb=True)
        workspace.RunNetOnce(model.param_init_net)
        workspace.RunNetOnce(model.net)
        data = workspace.FetchBlob('data')
        label = workspace.FetchBlob('label')
    ...
    def test_optical_flow_use_shorter_edge(self):
        ...
        model = model_helper.ModelHelper(name='Video Loader from LMDB')
        reader = model.CreateDB('sample', db=video_db_dir, db_type='lmdb')
        model.net.VideoInput(reader, ['data', 'label'], name='data', batch_size=batch_size, clip_per_video=1, crop_size=112, scale_w=171, scale_h=128, length_of=8, sampling_rate_of=1, frame_gap_of=1, decode_type=0, video_res_type=1, get_rgb=False, get_optical_flow=True)
        workspace.RunNetOnce(model.param_init_net)
        workspace.RunNetOnce(model.net)
        data = workspace.FetchBlob('data')
        label = workspace.FetchBlob('label')

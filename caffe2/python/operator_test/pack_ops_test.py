...
class TestTensorPackOps(serial.SerializedTestCase):
        ...
        def pack_segments_ref(lengths, data, max_length=max_length):
            ...
            if return_presence_mask:
                ...
                for length in lengths:
                    ...
                    presence_arr.append(np.pad(np.ones(length, dtype=np.bool), (0, pad_length), mode=str('constant')))
                result.append(presence_arr)
            return result
        return pack_segments_ref
    
    @serial.given(num_seq=st.integers(10, 100), cell_size=st.integers(1, 10), max_length_buffer=st.integers(-5, 5), **hu.gcs)
    ...
    def test_presence_mask(self, gc, dc):
        ...
        expected_presence_mask = np.array([[True, False, False], [True, True, False], [True, True, True]], dtype=np.bool)

...
class TestClipTensorByScalingOp(serial.SerializedTestCase):
    ...
    @serial.given(n=st.integers(5, 8), d=st.integers(2, 4), threshold=st.floats(0.1, 10), additional_threshold=st.floats(0.1, 10), use_additional_threshold=st.booleans(), inplace=st.booleans(), **hu.gcs_cpu_only)

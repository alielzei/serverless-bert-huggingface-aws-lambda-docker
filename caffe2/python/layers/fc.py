...
def get_fc_predictor_version(fc_version):
    assert fc_version in ['fp32', 'fp16'], 'Only support fp32 and fp16 for the fully connected layer in the predictor net, the provided FC precision is {}'.format(fc_version)
    return fc_version


class FC(SamplingTrainableMixin, ModelLayer):
    
    def __init__(self, model, input_record, output_dims, weight_init=None, bias_init=None, weight_optim=None, bias_optim=None, name='fc', weight_reg=None, bias_reg=None, clip_param=None, max_fc_size=None, axis=1, transposed=False, uniform_weight_init_scale_numerator=1.0, **kwargs):
        ...
        self.output_dim_vec = FC.calculate_fc_output_dims(max_fc_size, input_dims, output_dims)
        ...
        if (self.output_dim_vec is None or len(self.output_dim_vec) == 1):
            ...
            self.w = self.create_param(param_name='w', shape=weight_shape, initializer=weight_init, optimizer=weight_optim, regularizer=weight_reg)
            self.b = self.create_param(param_name='b', shape=[output_dims], initializer=bias_init, optimizer=bias_optim, regularizer=bias_reg)
        else:
            self.w_vec = []
            self.b_vec = []
            for (idx, output_dim) in enumerate(self.output_dim_vec):
                ...
                self.w_vec.append(self.create_param(param_name='w_sub_{}'.format(idx), shape=weight_shape, initializer=weight_init, optimizer=weight_optim, regularizer=weight_reg))
                self.b_vec.append(self.create_param(param_name='b_sub_{}'.format(idx), shape=[output_dim], initializer=weight_init, optimizer=weight_optim, regularizer=weight_reg))
        if axis == 1:
            output_shape = (output_dims, )
        else:
            output_shape = list(input_record.field_types()[0].shape)[0:axis - 1]
            output_shape = tuple(output_shape + [output_dims])
        self.output_schema = schema.Scalar((np.float32, output_shape), self.get_next_blob_reference('output'))
    
    @staticmethod
    def calculate_fc_output_dims(max_fc_size, input_dim, output_dim):
        if (not max_fc_size or max_fc_size < 0):
            return None
        assert max_fc_size >= input_dim, 'Currently we split along the output dimension. So we need max_fc_size >= input_dim. But, max_fc_size: {}, input_dim: {}'.format(max_fc_size, input_dim)
    ...
    def _insert_fc_ops(self, net, params, outputs, version):
        ...
        if version == 'fp32':
            if self.transposed:
                return net.FCTransposed(self.input_record.field_blobs() + params, outputs, axis=self.axis, **self.kwargs)
            else:
                return net.FC(self.input_record.field_blobs() + params, outputs, axis=self.axis, **self.kwargs)
        elif version == 'fp16':
            return net.FbFCPacked(self.input_record.field_blobs() + params, outputs, axis=self.axis, **self.kwargs)
        else:
    ...
    def _add_ops(self, net, params, version):
        ...
        if (self.output_dim_vec is None or len(self.output_dim_vec) == 1):
            ...
            for i in range(len(self.output_dim_vec)):
                output_blob = net.NextScopedBlob('output_sub_{}'.format(i))
                insert_ret = self._insert_fc_ops(net, [w_vec[i], b_vec[i]], [output_blob], version)
                output_blob_vec.append(insert_ret)
            net.Concat(output_blob_vec, self.output_schema.field_blobs() + [self.output_schema.field_blobs()[0] + '_concat_dims'])

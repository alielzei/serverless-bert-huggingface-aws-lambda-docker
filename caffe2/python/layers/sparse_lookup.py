...
from caffe2.python.layers.layers import get_categorical_limit, get_key, IdList, IdScoreList, IdListWithEvicted, IdScoreListWithEvicted, LayerPsParam, ModelLayer, almost_equal_schemas
import collections
import functools
import logging
import math
...
def get_trainer_version_based_on_optim(optim_def):
    ...
    return custom_funtemplate.rewrite_template('caffe2.python.layers.sparse_lookup.get_trainer_version_based_on_optim', 'get_trainer_version_based_on_optim(optim_def)', {'Optimizer': Optimizer, 'logger': logger, 'FP16_ENGINES': FP16_ENGINES, 'optim_def': optim_def}, 1)
...
def get_sparse_lookup_predictor_version(version, blob_size=None, min_blob_size_4bits=None, embedding_dim=None, sparse_feature_name=None):
    import custom_funtemplate
    return custom_funtemplate.rewrite_template('caffe2.python.layers.sparse_lookup.get_sparse_lookup_predictor_version', 'get_sparse_lookup_predictor_version(version, blob_size=None, min_blob_size_4bits=None, embedding_dim=None, sparse_feature_name=None)', {'logger': logger, 'version': version, 'blob_size': blob_size, 'min_blob_size_4bits': min_blob_size_4bits, 'embedding_dim': embedding_dim, 'sparse_feature_name': sparse_feature_name}, 1)
...
class SparseLookup(ModelLayer):
    _id_list_supported_reducers = ['LogMeanExp', 'LogSumExp', 'Max', 'Mean', 'Sum', 'WeightedSum', 'WeightedMean', 'Sqrt', 'None']
    _id_score_list_supported_reducers = ['PositionWeighted', 'RecencyWeighted', 'Mean', 'Sum', 'WeightedSum', 'WeightedMean', 'None']
    _fp16_compatible_init_op_types = ['Float16UniformFill']
    _fp16_compatible_reducers = ['Sum', 'Mean', 'Sqrt', 'PositionWeighted', 'RecencyWeighted']

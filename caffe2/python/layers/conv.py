...
class Conv(ModelLayer):
    ...
    def __init__(self, model, input_record, output_dim, kernel_h, kernel_w, stride_h, stride_w, pad_b=None, pad_l=None, pad_r=None, pad_t=None, order='NHWC', kernel_init=None, bias_init=None, kernel_optim=None, bias_optim=None, name='conv', **kwargs):
        ...
        assert (kernel_h > 0 and isinstance(kernel_h, int)), 'kernel_h should be positive integer'
        assert (kernel_w > 0 and isinstance(kernel_w, int)), 'kernel_w should be positive integer'
        self.kernel_h = kernel_h
        self.kernel_w = kernel_w
        assert (stride_h > 0 and isinstance(stride_h, int)), 'stride_h should be positive integer'
        assert (stride_w > 0 and isinstance(stride_w, int)), 'stride_w should be positive integer'
        ...
        assert input_c > 0, 'Number of input channels in conv parameters should be positive'
        kernel_init = (kernel_init if kernel_init else ('XavierFill', {}))
        bias_init = (bias_init if bias_init else ('ConstantFill', {'value': 0.0}))
        self.kernel = self.create_param(param_name='conv_kernel', shape=kernel_shape, initializer=kernel_init, optimizer=kernel_optim)
        self.bias = self.create_param(param_name='conv_bias', shape=[output_dim], initializer=bias_init, optimizer=bias_optim)
        self.output_schema = schema.Scalar((np.float32, (output_dim, )), self.get_next_blob_reference('output'))
    
    def add_ops(self, net):
        net.Conv(self.input_record.field_blobs() + [self.kernel, self.bias], self.output_schema.field_blobs(), kernel_h=self.kernel_h, kernel_w=self.kernel_w, stride_h=self.stride_h, stride_w=self.stride_w, pad_t=self.pad_t, pad_l=self.pad_l, pad_b=self.pad_b, pad_r=self.pad_r, order=self.order)

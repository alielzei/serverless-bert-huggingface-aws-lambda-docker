...
class Dropout(ModelLayer):
    ...
    def __init__(self, model, input_record, name='dropout', ratio=0.5, dropout_for_eval=False, **kwargs):
        super(Dropout, self).__init__(model, name, input_record, **kwargs)
        assert isinstance(input_record, schema.Scalar), 'Incorrect input type'
        assert (ratio >= 0 and ratio < 1.0), 'Expected 0 <= ratio < 1, but got ratio of %s' % ratio
    ...
    def _add_ops(self, net, is_test):
        ...
        output_blobs = self.output_schema.field_blobs() + [net.NextScopedBlob('d_mask')]
        net.Dropout(input_blob, output_blobs, ratio=self.ratio, is_test=is_test)
